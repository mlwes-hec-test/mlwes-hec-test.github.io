...
function rc5SourceFamilyChoice(context,query){
  if(!context.source)return null;const key=(C8?.norm?.(query)||normalise(query)).replace(/\s+/g,'-'),choices=rc4SourceFoods(context.source.id).filter(food=>food.choiceFamily===key).sort((a,b)=>(Number(a.choiceOrder)||0)-(Number(b.choiceOrder)||0)||a.name.localeCompare(b.name));
