...
test('44. production controller carries generic Margarine teaspoon state into final Review',()=>{const app=runtimeController(progressiveAudit.afcdFoods,{query:'Margarine'});assert.equal(app.api.state().session.genericConcept.name,'Margarine');app.clickLabel('Monounsaturated');app.clickLabel('Reduced fat');app.clickLabel('Regular salt');assert.equal(app.api.state().session.stage,guided.stages.MEASURE);app.clickLabel('Teaspoon (5 g)');app.active().querySelector('[data-gpr-amount]').value='1';app.clickLabel('Continue');assert.equal(app.api.state().session.stage,guided.stages.CONFIRMATION);assert.match(app.active().innerHTML,/Ready for review/);app.clickLabel('Review And Confirm');assert.equal(app.review.current.options.amount,1);assert.equal(app.review.current.options.unit,'tsp');assert.equal(app.review.current.options.consumedPortion.baseQuantity,5);assert.equal(app.review.current.food.units.tsp,.05);});
