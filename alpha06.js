...
let alpha0633Conversation=CONVERSATION?.createConversation?.()||{state:'idle',pendingAction:null},alpha0633ListenMode='request',alpha0633VoiceSaveLocked=false,alpha0633PromptTimer=null,alpha0633ResponseTimer=null,alpha0633GestureSession=false;
...
function alpha0633Speak(text,onEnd=null){const companion=alpha0633Companion();if(!companion.enabled||!companion.speech||!text){if(onEnd)setTimeout(onEnd,0);return null;}alpha0633CancelSpeech();const utterance=window.HECSpeakText?.(text);if(onEnd&&utterance){let finished=false;const done=()=>{if(finished)return;finished=true;if(alpha0633PromptTimer)clearTimeout(alpha0633PromptTimer);alpha0633PromptTimer=null;onEnd();};if(typeof utterance.addEventListener==='function'){utterance.addEventListener('end',done,{once:true});utterance.addEventListener('error',done,{once:true});}else{utterance.onend=done;utterance.onerror=done;}alpha0633PromptTimer=setTimeout(()=>{window.speechSynthesis?.cancel?.();done();},6000);}else if(onEnd)setTimeout(onEnd,0);return utterance;}
...
function alpha0633ShowAnswerFallback(message='Tap to Answer when you are ready.'){alpha0633GestureSession=false;const button=by('voice-response-mic');button?.classList.remove('hidden');if(by('voice-status'))by('voice-status').textContent=message;}
function alpha0633AutoListenForResponse(){if(!alpha0633GestureSession||alpha0633Conversation.state!=='confirmation-ready')return false;if(by('voice-status'))by('voice-status').textContent='Listening for your answer…';const started=alpha0633StartListening('response',true);if(!started)alpha0633ShowAnswerFallback('Automatic listening is not available here. Tap to Answer.');return started;}
...
function rc6ResolveRemovalPending(intent){
  const entries=entriesForDate(intent.localDate).filter(entry=>entry.meal===intent.meal&&entry.status!=='skipped');intent.items=alpha0633ResolveIntentFood(intent).slice(0,1);if(intent.items[0]){intent.canonicalId=intent.items[0].canonicalId;intent.items[0].amount=intent.removeQuantity||intent.items[0].amount;}
  const match=CONVERSATION?.matchRemoval?.(entries,intent)||{status:'none',matches:[],message:'No matching Diary food was found.'};intent.diaryMatch={status:match.status,totalQuantity:match.totalQuantity||0,ids:(match.matches||[]).map(entry=>entry.id)};intent.unresolved=(intent.unresolved||[]).filter(item=>!['diary-match','quantity'].includes(item.field));
