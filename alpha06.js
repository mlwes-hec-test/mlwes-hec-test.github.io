...
function au633ConceptMember(food,conceptId){if(!conceptId)return true;const entry=window.HECBrandCatalogue?.entries.get('off:'+food.barcode);return entry?entry.conceptIds.includes(conceptId):S23.conceptCompatibility(food,conceptId).compatible;}
...
function retailerRender633(state){
  ...
  const R=window.HECRetailerCatalogue,model=state.result||R.directory(state.retailerId,state),retailer=model?.retailer||R.entity(state.retailerId),commercial=state.scope==='commercial-identity',collection=['source-declared-brand','source-declared-store'].includes(retailer?.collectionMode);
