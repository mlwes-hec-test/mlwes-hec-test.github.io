...
(function(global){
  ...
  function register(index,{loadJSON,timeoutMs=15000}={}){
    const entries=new Map(index.entries.map(entry=>[entry.id,entry])),files=new Set(index.files.map(file=>file.path)),cache=new Map();
    const read=loadJSON|| (typeof window==='undefined'&&typeof require==='function'?async file=>JSON.parse(require('node:fs').readFileSync(require('node:path').join(__dirname,index.base,file),'utf8')):async(file,{signal})=>{const response=await fetch(index.base+file,{signal});if(!response.ok)throw Error('Retailer product facts could not be loaded');return response.json();});
    async function boundedRead(file){
      const controller=new AbortController();let timer;
      try{return await Promise.race([Promise.resolve().then(()=>read(file,{signal:controller.signal})),new Promise((_,reject)=>{timer=setTimeout(()=>{reject(Error('Retailer product facts took too long to load'));controller.abort();},timeoutMs);})]);}
      finally{clearTimeout(timer);}
    }
    async function load(file){if(!files.has(file))throw Error('Unlisted retailer evidence shard');if(!cache.has(file)){const pending=boundedRead(file);cache.set(file,pending);pending.catch(()=>{if(cache.get(file)===pending)cache.delete(file);});if(cache.size>16)cache.delete(cache.keys().next().value);}return cache.get(file);}
