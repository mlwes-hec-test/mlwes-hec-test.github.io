...
function rc6SyncFoodSearchViewport(){const input=by('food-search'),vv=window.visualViewport,height=vv?.height||window.innerHeight;document.documentElement.style.setProperty('--hec-visual-viewport-height',`${Math.round(height)}px`);document.body.classList.toggle('food-search-focused',document.activeElement===input);document.body.classList.toggle('food-search-keyboard-open',document.activeElement===input&&!!vv&&height<window.innerHeight-80);}
