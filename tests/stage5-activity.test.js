...
const circlePositions=()=>Array.from({length:9},(_,index)=>{const n=index+1,match=styles.match(new RegExp(`\\.nine-room-circle \\.r${n}\\{left:([\\d.]+)%!important;top:([\\d.]+)%!important\\}`));assert.ok(match,`r${n} position`);return {x:Number(match[1]),y:Number(match[2])};});
function assertOrbit(viewport,circleWidth,roomPercent){
  const size=Math.min(circleWidth(viewport),590),diameter=Math.max(62,size*roomPercent/100),points=circlePositions();
  for(let i=0;i<points.length;i++)for(let j=i+1;j<points.length;j++){const distance=Math.hypot((points[i].x-points[j].x)*size/100,(points[i].y-points[j].y)*size/100);assert.ok(distance>diameter,`${viewport.w}x${viewport.h}: rooms ${i+1}/${j+1}`);}
  points.forEach(({x,y},index)=>{const radius=diameter/2;assert.ok(x*size/100-radius>=-1&&x*size/100+radius<=size+1,`r${index+1} horizontal`);assert.ok(y*size/100-radius>=-1&&y*size/100+radius<=size+1,`r${index+1} vertical`);});
}

test("1. Home contains exactly nine room controls",()=>{assert.equal((html.match(/class="room r\d+"/g)||[]).length,9);assert.equal((html.match(/data-room=/g)||[]).length,9);});
...
test("29. nine-room orbit geometry works on 375x667 and 430x932 iPhones",()=>{assertOrbit({w:375,h:667},v=>Math.min(.96*v.w,470),18.5);assertOrbit({w:430,h:932},v=>Math.min(.96*v.w,470),18.5);});
test("30. nine-room orbit geometry works on 768x1024 and 820x1180 iPad portrait",()=>{assertOrbit({w:768,h:1024},v=>Math.min(.76*v.w,v.h-245,590),18);assertOrbit({w:820,h:1180},v=>Math.min(.76*v.w,v.h-245,590),18);});
test("31. nine-room orbit geometry works on 1024x768 and 1180x820 iPad landscape",()=>{assertOrbit({w:1024,h:768},v=>Math.min(.62*v.w,v.h-205,575),18);assertOrbit({w:1180,h:820},v=>Math.min(.62*v.w,v.h-205,575),18);});
