...
function rc4ApplySearchSurface(){const input=by('food-search'),raw=input?.value.trim()||'',exact=rc4ExactProduct(raw);let state=ext.ui.foodSourceBrowse||null,named=null;if(rc4SourceOnly(raw))named=typeof rc4NamedRestaurantSource==='function'?rc4NamedRestaurantSource(raw,{only:true}):{id:'mcdonalds-au',displayName:"McDonald's Australia"};if(named){state={sourceId:named.id,label:named.displayName,query:raw,shown:state?.sourceId===named.id?state.shown||20:20};ext.ui.foodSourceBrowse=state;}if(state){if(C8?.corrected?.(state.query)!==C8?.corrected?.(raw)){state.query=raw;state.shown=20;}rc4RenderSourceSurface(state,raw);return;}if(exact)rc4RenderExactSurface(raw,exact);}
...
function rc5SourceFamilyChoice(context,query){
  if(!context.source)return null;const family=s23Parsed(query).food,choices=rc4SourceFoods(context.source.id).filter(food=>s23Parsed(String(food.choiceFamily||'').replace(/-/g,' ')).food===family).sort((a,b)=>(Number(a.choiceOrder)||0)-(Number(b.choiceOrder)||0)||a.name.localeCompare(b.name));
  if(choices.length<2)return null;return {kind:'choice',primary:null,choices,context,reason:`Choose the ${context.source.displayName} ${String(query).trim()} option that matches the serving you had.`};
}
...
renderLibrary=function(){rc6RenderLibraryBase();const input=by('food-search'),raw=input?.value.trim()||'';if(rc4SourceOnly(raw)){const source=rc4NamedRestaurantSource(raw,{only:true});ext.ui.foodSourceBrowse={sourceId:source.id,label:source.displayName,query:raw,shown:ext.ui.foodSourceBrowse?.sourceId===source.id?ext.ui.foodSourceBrowse.shown||20:20};delete ext.ui.foodSourceBrowse.category;rc5RenderSourceSurface(ext.ui.foodSourceBrowse,raw);const live=by('food-live-results');if(live){live.innerHTML='';live.classList.add('hidden');}input?.blur?.();}else rc6GroupGenericFries(raw);rc6SyncFoodSearchViewport();};
window.addEventListener('input',event=>{const input=event.target,source=input?.id==='food-search'?rc4NamedRestaurantSource(input.value,{only:true}):null;if(!source)return;setTimeout(()=>{const current=rc4NamedRestaurantSource(input.value,{only:true});if(!current)return;ext.ui.foodSourceBrowse={sourceId:current.id,label:current.displayName,query:input.value,shown:20};saveExt();renderLibrary();input.blur();rc6ScrollSourceHub();},0);},true);
...
function ps33BrandFamily(raw){const context=rc5SearchContext(raw);if(rc5SourceCountedExact(context,raw,context.product))return null;const value=C8?.brandFamilyResults?.(allFoods().filter(s23ProductLike),raw);return value?.intent?.kind==='brand-family'?value:null;}
