...
test('explicit source tap commits and dismisses the keyboard through the shared filter transition',()=>{
  const inputBlock=runtime.slice(runtime.indexOf("window.addEventListener('input',event=>{const input=event.target;if(input?.id!=='food-search')"),runtime.indexOf("by('food-search')?.addEventListener('focus'")),clickBlock=runtime.slice(runtime.indexOf("const source=event.target.closest?.('[data-rc4-source-browse]')"),runtime.indexOf("if(event.target.closest?.('[data-rc4-clear-source-filter]'))"));assert.doesNotMatch(inputBlock,/\.blur/);assert.match(clickBlock,/rc4SetSourceFilter\(query,.*'source-tap'/);
  const app=sourceInputHarness();app.type('KFC');app.ext.ui.pendingMeal='Breakfast';const context={input:app.input,ext:app.ext,searchSession633:{rawQuery:'KFC',revision:1},by:()=>app.input,ss633BeginTyping(input){context.searchSession633.rawQuery=input.value;context.searchSession633.revision++;delete app.ext.ui.foodSourceBrowse;},ss633Commit(reason,intent){context.commit={reason,intent};},saveExt(){}};
  vm.runInNewContext(`${productionFunction('rc4SetSourceFilter')}\ntransition=rc4SetSourceFilter;`,context);context.transition('',{sourceId:'kfc-au',category:'Burgers'},'source-tap');assert.equal(app.input.blurCount,1);assert.equal(context.searchSession633.rawQuery,'');assert.equal(context.searchSession633.revision,2);assert.equal(context.commit.reason,'source-tap');assert.equal(app.ext.ui.foodSourceBrowse.sourceId,'kfc-au');assert.equal(app.ext.ui.foodSourceBrowse.category,'Burgers');assert.equal(app.ext.ui.pendingMeal,'Breakfast');
});
...
test('KFC source inventory exposes all 12 categories and 148 products including the reviewed supplement',()=>{const source=sources.getCatalogue('kfc-au').source;assert.equal(source.inventory.browseCategories.length,12);assert.equal(source.inventory.protectedUniqueProducts,126);assert.equal(kfc.length,148);assert.equal(new Set(kfc.map(food=>food.canonicalId)).size,148);});
...
test('Big Mac remains one burger with official McDonald’s provenance',()=>{const food=mcd.find(item=>item.name==='Big Mac'),session=consumed(guided.chooseCustomisation(guided.createSession([food],'Big Mac',{intent:{kind:'exact-product'}}),'standard'),'burger',1);assert.equal(session.nutrition.energyKj,2600);assert.equal(food.sourceDisplayName,"McDonald's Australia");});
...
test('control: protected KFC snapshot remains 126 unique products without invented standalone Corn Chips',()=>{assert.equal(sources.getCatalogue('kfc-au').source.inventory.protectedUniqueProducts,126);assert.equal(require('../scripts/audit_kfc_au.js').manifest().hashMatches,true);assert.equal(kfc.some(food=>/^Corn Chips$/i.test(food.name)),false);});
