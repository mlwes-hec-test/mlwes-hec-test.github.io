...
function us633SubmittedMarkup(model){if(!model?.groups?.length)return `<section class="universal-search-empty"><strong>No reliable match is ready to select.</strong><p>HEC will not guess nutrition. Scan the barcode, read the Nutrition Panel, or create the food.</p><div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button><button type="button" class="secondary" data-open-feature="custom-food">Create Food</button></div></section>`;return `<section class="universal-search-submitted" aria-label="Submitted food search results"><header><strong>Results for “${esc(model.rawQuery)}”</strong><small>Frozen at Search · choose a result to continue</small></header>${model.groups.map(group=>`<section class="universal-search-group" data-universal-group="${esc(group.key)}"><h3>${esc(group.label)}</h3>${group.items.slice(0,24).map(item=>item.kind==='generic-concept'?`<button type="button" class="universal-search-result universal-generic-result" data-universal-generic="${esc(item.conceptKey)}"><span><strong>${esc(item.name)}</strong><small>Generic food · choose where it came from</small></span><b>Choose</b></button>`:us633ResultRow(item)).join('')}</section>`).join('')}</section>`;}
...
function us633UniversalControl(target){return target?.closest?.('[data-universal-generic],[data-universal-source],[data-universal-back-results],[data-universal-back-source],[data-universal-result]')||null;}
