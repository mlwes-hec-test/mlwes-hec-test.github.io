...
(function(global){
  ...
  function isBrandMember(food,key){return !!food&&!!admittedKeys.get(C.canonicalKey(food))?.brandKeys.includes(key);}
  ...
  async function hydrate(rows,{isCurrent=()=>true}={}){
    ...
    const foods=await Promise.all(rows.map(async e=>{
      ...
      if(!food||food.barcode!==e.barcode||!['audited-first-wave','audited-round-two'].includes(food.brandAdmission?.status)||!C.productEligibility(food).addability.normalLoggingAllowed)throw Error('Brand admission mismatch: '+e.id);
  ...
  const api={index,pageSize:PAGE_SIZE,brands,entries,loadedFoods,entryFor,isAdmitted,isBrandMember,ordinaryChoice,recognise,conceptBrands,directory,page,search,hydrate};global.HECBrandCatalogue=api;if(typeof module!=='undefined'&&module.exports)module.exports=api;
