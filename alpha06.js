...
function updateEntryPreview(){
  ...
  const selectionBlocked=window.HECFoodCatalogue&&!window.HECFoodCatalogue.canLog(food);if(by("entry-selection-summary"))by("entry-selection-summary").innerHTML=`<strong>${editorState?.libraryOnly?'Food To Save':'You Are Adding'}: ${formatNaturalAmount(amount)} ${esc(friendlyUnitLabel(food,unit,amount))}</strong><span>${selectionBlocked?esc(food.entryBlockedReason||'This selection cannot be saved with fixed nutrition.'):`${energyText(values.calories,values.energyKj)} · ${editorState?.libraryOnly?'Save this source record to My Foods for later use.':'Will be added only after you confirm Add to Diary.'}`}</span>`;
}
...
function ps34GuidedMarkup(raw,family,{compact=false}={}){
  ...
  if(session.exactProduct&&session.resolutionState===GUIDED_PRODUCTS.states.INCOMPLETE)return `<section class="guided-resolution-card"><strong>${esc(session.exactProduct.name)}</strong><p>${esc(session.identityIssue||'Complete this product’s nutrition and portion conversion.')}</p><div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-feature="custom-food">Create Food</button></div>${ps34GuidedNav(session)}</section>`;
  ...
  const calories=session.nutrition?.calories,reviewLabel=GUIDED_PRODUCTS.reviewLabel?.(session)||`${formatNaturalAmount(session.amount)} ${session.selectedMeasure?.label||session.selectedMeasure?.key||''}`;return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${identity}<div class="guided-resolution-question"><strong>Ready for review</strong><small>${esc(reviewLabel)}${Number.isFinite(Number(calories))?` · ${energyText(calories,session.nutrition?.energyKj)}`:''}</small></div><div class="guided-resolution-edit-actions"><button type="button" class="secondary" data-gpr-edit-amount>Change amount</button><button type="button" class="secondary" data-gpr-edit-measure>Change measure</button><button type="button" class="secondary" data-gpr-edit-identity>Change food</button></div><button type="button" class="primary guided-resolution-continue" data-gpr-review>Review And Confirm</button>${ps34GuidedNav(session)}</section>`;
}
...
function ps34PositionActiveCard(){const session=ps34GuidedSession,signature=session?`${session.stage}|${session.nextQuestion?.key||''}|${session.selectedMeasure?.key||''}|${session.transitionRevision||0}`:'';if(!signature||signature===ps34PositionSignature)return;ps34PositionSignature=signature;(window.requestAnimationFrame||((callback)=>callback()))(()=>{const card=document.querySelector?.('#food-results .guided-resolution-card:not(.guided-resolution-compact)')||document.querySelector?.('.guided-resolution-card'),viewport=window.visualViewport,rect=card?.getBoundingClientRect?.(),top=Number(viewport?.offsetTop)||0,bottom=top+(Number(viewport?.height)||window.innerHeight);if(rect&&(rect.top<top+8||rect.bottom>bottom-8))card.scrollIntoView?.({block:'nearest',behavior:window.matchMedia?.('(prefers-reduced-motion: reduce)').matches?'auto':'smooth'});});}
...
function ps34UpdateAmountPreview(input){const target=input?.closest?.('.guided-resolution-card')?.querySelector?.('[data-gpr-amount-preview]'),amount=Number(input?.value),measure=ps34GuidedSession?.selectedMeasure,food=ps34GuidedSession?.exactProduct;if(!target)return;if(!(amount>0)||!measure||!food){target.textContent='Enter an amount to preview nutrition.';return;}const factor=Number(measure.multiplier)*amount,calories=Number(food.nutrients?.calories),energyKj=Number(food.nutrients?.energyKj);target.textContent=Number.isFinite(calories)?`Nutrition preview · ${energyText(calories*factor,Number.isFinite(energyKj)?energyKj*factor:null)}`:'Nutrition preview will be checked at Review.';}
function ps34SelectGuidedProductChoice(choiceId,answerKey){const session=ps34GuidedSession,question=session?.nextQuestion,key=String(answerKey||question?.key||''),value=String(choiceId||''),valid=session?.stage===GUIDED_PRODUCTS?.stages?.IDENTITY&&question?.key===key&&question.options?.some(option=>String(option.value)===value);if(!valid)return false;GUIDED_PRODUCTS.answerDistinction(session,key,value);if(session.exactNutritionalIdentity&&typeof ss633RetireOwnedError==='function')ss633RetireOwnedError();ps34RenderGuided();return true;}
...
function us633ResultRow(item){const food=item.food||{},usable=C8?.canLog?.(food),status=usable?'Nutrition available · choose an amount':'Incomplete or identity-only · review before Diary',meta=[food.brand,item.provenance?.label,cleanMeasureText(food.serving||''),status].filter(Boolean).join(' · ');return `<button type="button" class="universal-search-result" data-universal-result="${esc(item.recordId||item.id)}"><span><strong>${esc(item.name||food.name||'Food')}</strong><small>${esc(meta)}</small></span><b>${usable?'Review':'Complete'}</b></button>`;}
...
const us633CommitCurrentBase=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){const input=by('food-search'),raw=input?.value.trim()||'';if(!raw)return false;const parsed=S23?.parseQuery?.(raw),concept=S23?.conceptFromQuery?.(parsed),bare=!!concept&&(concept.aliases||[]).some(alias=>S23?.singular?.(alias)===S23?.singular?.(parsed?.food));if(!bare&&(rc4SourceOnly(raw)||ps33BrandFamily(raw)||ps34GenericFamily(raw)))return us633CommitCurrentBase(reason);ss633Commit(reason,{kind:'universal-search'});searchSession633.quantity=S23?.parseQuantityLanguage?.(raw,{candidates:allFoods()})||null;searchSession633.submittedModel=C8?.submittedResultModel?.(allFoods(),raw,{savedIds:ext.savedFoodIds||[],online:psFederatedSearchState?.online||[]})||null;searchSession633.ownedAsync={previewRevision:searchSession633.previewRevision,committedRevision:searchSession633.committedRevision,query:searchSession633.normalisedQuery};searchSession633.sourceContext=null;searchSession633.selectedResult=null;ext.ui.foodSearch=raw;saveExt();input?.blur?.();us633RenderSubmitted();return true;};
...
window.addEventListener('pointerup',event=>{const pending=us633PendingPointer;us633PendingPointer=null;if(!pending||pending.pointerId!==event.pointerId||Math.hypot(event.clientX-pending.x,event.clientY-pending.y)>12)return;event.preventDefault();event.stopImmediatePropagation();us633ActivateControl(pending.control);},true);
window.addEventListener('pointercancel',()=>{us633PendingPointer=null;},true);
window.addEventListener('click',event=>{const control=us633UniversalControl(event.target);if(!control)return;event.preventDefault();event.stopImmediatePropagation();us633ActivateControl(control);},true);
