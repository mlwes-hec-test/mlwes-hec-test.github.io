...
function s23CacheProductMatches(cacheKey,scores){
    ...
    let ranked=scored.map(x=>x.food);const policy=C8?.partitionSearchRecords?.(allFoods());if(policy){const primary=new Set(policy.primary.map(food=>C8.canonicalKey(food)));ranked=ranked.filter(food=>primary.has(C8.canonicalKey(food)));}
...
function fc633Render(){
  ...
  session.question=S23.nextConceptQuestion(session,candidates);session.stage='identity';
  ...
  else if(session.question){const question=session.question;content=`<div class="food-concept-question"><strong>${esc(question.question)}</strong></div><div class="guided-resolution-choices">${question.options.map(option=>`<button type="button" class="guided-resolution-choice" data-fc-answer="${esc(question.key)}" data-fc-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('')}</div>`;}
...
async function fc633Enrich(session,revision){
  ...
  try{
    ...
    const results=await Promise.all(queries.map(query=>external.search(query,{limit:500})));
...
const fc633TypingBase=ss633BeginTyping;ss633BeginTyping=function(input){if(String(input?.value||'')!==searchSession633.rawQuery){fc633Session=null;fc633Revision++;}return fc633TypingBase(input);};
...
const fc633ActivateBase=us633ActivateControl;us633ActivateControl=function(control){if(fc633Session&&control?.matches('[data-fc-answer],[data-fc-back],[data-fc-cancel],[data-fc-base]'))return fc633Activate(control);if(fc633Session&&control?.hasAttribute('data-universal-result')){const food=(fc633Session.stage==='shortlist'?fc633Session.records:fc633Candidates()).find(item=>item.id===control.dataset.universalResult);if(!food)return false;fc633Session.stage='resolved';fc633Session.selectedId=food.id;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(fc633Session.rawQuery,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}return fc633ActivateBase(control);};
...
function au633LoadBrand(query){
  ...
  state.promise=(async()=>{
    ...
    try{const result=await window.HECOpenFoodFactsAU.search(query,{limit:500,isCurrent});if(!isCurrent())return;state.records=C8.canonicaliseRecords([...allFoods().filter(food=>C8.consumerBrandMembership(entity,food).matches),...result.foods]);state.catalogueTotal=result.total;state.catalogueLoaded=result.foods.length;}
