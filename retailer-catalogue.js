...
(function(global){
  ...
  function conceptMembership(food,source){if(source.retailer.collectionMode==='private-testing-evidence')return source.retailer.id==='aldi'?browseMembership(food,source).length>0:C.commercialIdentityMembership(source.retailer,food).matches;return source.retailer.collectionMode==='source-declared-store'?browseMembership(food,source).length>0:C.commercialIdentityMembership(source.retailer,food).matches;}
  function registerCatalogue({retailer,categories=[],entries=[],loadRecords,selectableOnly=false}){
    ...
    const all=[...groups.values()].filter(group=>group.categories.size),postings=new Map([['*',all]]),commercial=new Map();
  ...
  async function page(retailerId,{categoryId='*',scope='retailer',conceptId='',offset=0,limit=PAGE_SIZE,isCurrent=()=>true}={}){
    ...
    const groups=scope==='commercial-identity'?source?.commercial.get(conceptId)||[]:source?.postings.get(categoryId)||[];
    const selected=groups.slice(start,start+size),foods=[];
    if(!isCurrent())return null;
    const key=JSON.stringify([scope,conceptId,categoryId,start,size]),cached=source?.cache.get(key);
    if(cached)return clone(cached);
    if(selected.length){
      foods.push(...await hydrateGroups(source,selected,{scope,isCurrent}));if(!isCurrent())return null;
    }
    const result={...model,categoryId,categoryLabel:categoryId==='*'?'All Items':source?.categories.get(categoryId)?.label||'',offset:start,total:groups.length,hasMore:start+selected.length<groups.length,foods};
  ...
  async function search(query,{offset=0,limit=PAGE_SIZE,isCurrent=()=>true}={}){
    ...
    matches.sort((a,b)=>b.score-a.score||a.group.key.localeCompare(b.group.key));const selected=matches.slice(start,start+size),bySource=new Map(),foods=[];
  ...
  async function load(session,{categoryId=session.categoryId,offset=0,isCurrent=()=>true}={}){
    const request=++session.request,owns=()=>session.active&&session.request===request&&isCurrent();session.categoryId=categoryId;session.offset=offset;session.loading=true;session.result=null;session.error='';
