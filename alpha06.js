...
function editorMeasure(food,unit){const carried=(editorState?.portionMeasures||[]).find(item=>item.key===unit||item.id===unit);if(carried)return carried;const profile=window.HECServingFoundation?.servingMeasureProfile?.(food),profileMeasure=profile?.measures?.find(item=>item.key===unit||item.id===unit);if(profileMeasure)return profileMeasure;const saved=editorState?.consumedPortion;if(saved?.measureId!==unit)return null;return {key:unit,id:unit,label:saved.measureLabel,displayLabel:saved.measureLabel,multiplier:saved.conversion?.nutritionMultiplier,conversionToBase:{baseQuantity:saved.conversion?.perMeasureQuantity,baseUnit:saved.conversion?.baseUnit||saved.baseUnit,basis:saved.conversion?.basis||''},conversionBasis:saved.conversion?.basis||'',...(saved.conversionProvenance||{})};}
function syncEditorConsumedPortion(food=editorFood(),amount=by("entry-amount")?.value,unit=by("entry-unit")?.value){if(!editorState||!food||!unit)return null;const measure=editorMeasure(food,unit),factory=window.HECServingFoundation?.consumedPortionState;if(!measure||!factory)return editorState.consumedPortion||null;const prior=editorState.consumedPortion,portion=factory({measure,amount,sourceServing:prior?.sourceServing||food.manufacturerServing||null,nutritionBasis:prior?.nutritionBasis||food.nutritionBasis||null});if(portion)editorState.consumedPortion=portion;return portion;}
function prepareEntry(food,{entry=null,date=null,meal=null,status="eaten",source=null,amount=null,unit=null,consumedPortion=null,portionMeasures=null}={}){
  ...
  editorState={foodId:food.id,entryId:entry?.id||null,returnTo:selectedMeal?"food-diary":"food-library",source:source||food.source,variantSelections:entry?.variantSelections||{},libraryOnly:!entry&&!selectedMeal,pendingDiarySave:false,reviewFood,portionMeasures:clone(portionMeasures||[]),consumedPortion:carriedPortion};
...
function alpha0633CountIdentityFood(intent){
  const count=Number(intent?.quantity?.amount),query=C8?.norm?.(intent?.foodText)||normalise(intent?.foodText);if(!intent?.quantity?.explicit||!Number.isInteger(count)||count<2||!query)return null;
...
const searchSession633={inputNode:by('food-search')||null,rawQuery:'',normalisedQuery:'',focused:false,mode:'idle',commitReason:'',intentKind:'none',revision:0,caretStart:0,caretEnd:0,stableTimer:0,errorOwner:null};
