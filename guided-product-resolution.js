...
(function(global){
  ...
  const DIMENSIONS=Object.freeze(['fatLevel','saltLevel','texture','grain','format','dietaryVariant','variant','flavour','productLine','family','size','packSize','category']);
  const LABELS=Object.freeze({category:'product category',family:'product family',productLine:'product line',variant:'variant',flavour:'flavour',fatLevel:'type',saltLevel:'salt level',texture:'texture',grain:'bread or grain type',format:'format',dietaryVariant:'dietary variant',packSize:'pack size',size:'size',product:'product'});
  ...
  function workflowState(session){
    ...
    if(candidates.length===1){const food=candidates[0],quality=CATALOGUE?.exactProductQuality?.(food,{candidates:session.allCandidates})||{exactEligible:true,reason:'legacy-specific-product'},type=SEMANTICS?.classify?.(food)?.type;session.exactProductQuality=quality;if(!quality.exactEligible){session.exactProduct=null;session.resolutionState=STATES.INCOMPLETE;session.nextQuestion=null;session.identityIssue=quality.reason;session.stage=STAGES.IDENTITY;return session;}session.exactProduct=food;session.resolutionState=type===SEMANTICS?.types?.CONFIGURABLE?STATES.CONFIGURABLE:STATES.EXACT;session.nextQuestion=null;session.servingProfile=servingProfile(food);session.stage=session.resolutionState===STATES.CONFIGURABLE?STAGES.IDENTITY:STAGES.MEASURE;return session;}
  ...
  function candidatePool(records,query,externalIntent=null){
    const all=canonicalizeCandidates(records).filter(food=>!isNonSpecificBrandRecord(food)),queries=identityQueries(query),intent=externalIntent||CATALOGUE?.queryIntent?.(query)||{kind:'product'},family=externalIntent?null:CATALOGUE?.brandFamilyResults?.(all,query);
  ...
  function createSession(records,query,{knownAttributes={},consumption=null,destination=null,intent=null}={}){
    let parsed=consumption?{...parseConsumption(query),...consumption,identityQuery:consumption.identityQuery||parseConsumption(query).identityQuery}:parseConsumption(query),pool=candidatePool(records,parsed.identityQuery,intent);const packageIdentity=parsed.explicit&&parsed.measure&&pool.candidates.length===1&&norm(productAttributes(pool.candidates[0]).packSize)===norm(parsed.phrase);if(packageIdentity){parsed={...parsed,identityQuery:norm(query),amount:null,measure:'',explicit:false,phrase:''};pool=candidatePool(records,parsed.identityQuery,intent);}const allCandidates=pool.candidates,selectableCandidates=allCandidates.filter(food=>(CATALOGUE?.exactProductQuality?.(food,{candidates:allCandidates})||{exactEligible:true}).exactEligible),initial=selectableCandidates.length?selectableCandidates:allCandidates,incompleteCandidates=allCandidates.filter(food=>!selectableCandidates.includes(food)),known=knownAttributesFromQuery(initial,parsed.identityQuery,knownAttributes),session={version:VERSION,query:String(query||''),identityQuery:parsed.identityQuery,intent:pool.intent,allCandidates,selectableCandidates,initialCandidates:initial,incompleteCandidates,candidates:initial,initialResolutionState:pool.state,resolutionState:pool.state,stage:STAGES.IDENTITY,knownAttributes:known,answers:[],nextQuestion:null,exactProduct:null,exactProductQuality:null,identityIssue:'',servingProfile:null,selectedMeasure:null,amount:null,nutrition:null,nutritionIntegrity:null,destination:clone(destination||null),consumption:parsed};
  ...
  const api={version:VERSION,states:STATES,stages:STAGES,dimensions:DIMENSIONS,labels:LABELS,directProductLimit:DIRECT_PRODUCT_LIMIT,norm,normalizeMeasure,parseConsumption,productAttributes,canonicalProductKey,canonicalizeCandidates,isNonSpecificBrandRecord,knownAttributesFromQuery,facetQuality,getNextProductDistinction,presentationForSession,servingProfile,createSession,answerDistinction,changeAnswer,selectMeasure,selectAmount,calculateNutrition,back,resolveRequest,separationAudit,summary};
