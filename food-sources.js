...
(function(global){
  ...
  function toFoodRecord(item,source,variant=null){
    ...
    const entity=variant||item,unitKey=entity.serving.unitKey,unitLabel=entity.serving.unitLabel,sourceItemKey=`${source.id}:${item.id}${variant?`:${variant.id}`:''}`,retailerSource=/retailer/i.test(source.sourceType||'');
    if(retailerSource&&!String(entity.brand||item.brand||'').trim())throw new Error('Retailer source records require an explicit consumer brand');
    ...
    const record={
      ...
      familyId:item.id,familyName:variant?(item.familyDisplayName||item.name):item.name,variantLabel:variant?.variantLabel||'',name:entity.name,brand:entity.brand||item.brand||source.displayName,category:item.category,categoryMemberships:clone(item.categoryMemberships),browseCategory:entity.browseCategory||item.browseCategory||'Other',browseTags:unique([...(item.browseTags||[]),...(entity.browseTags||[])]),country:source.country,market:source.market,
    ...
    for(const key of ['sourceProvenance','canonicalEvidence','evidenceConflicts','currentState','choiceFamilyAliases','retailerMemberships','commercialIdentities'])if(entity[key]!==undefined||item[key]!==undefined)record[key]=clone(entity[key]??item[key]);
    if(retailerSource){record.recordType='packaged';record.barcode=entity.barcode||item.barcode||null;record.sourceCatalogueId=source.id;delete record.foodSourceId;delete record.sourceAliases;delete record.sourceDisplayName;record.source=`Official ${source.displayName} retailer product information`;record.sourceProvenance={...(record.sourceProvenance||{}),trustClass:'official-au-retailer',sourceType:'retailer'};}
