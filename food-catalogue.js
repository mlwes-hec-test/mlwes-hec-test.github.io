...
(function(global){
  ...
  function recordType(food){
    ...
    if(RECORD_TYPE_VALUES.has(food?.recordType))return food.recordType;
  ...
  // Retailer evidence establishes a listing, never consumer-brand ownership.
  function verifiedRetailEvidence(evidence){return !!evidence&&['official-au-retailer','official-au-manufacturer','package-verified-au'].includes(evidence.trustClass)&&!!evidence.sourceId&&!!evidence.recordId&&/^https:\/\//.test(evidence.url||'')&&Number.isFinite(Date.parse(evidence.retrievedAt||''));}
  function retailerMemberships(food){return marketFor(food)==='AU'?(food?.retailerMemberships||[]).filter(item=>item.retailerId&&item.market==='AU'&&item.scope==='food'&&item.verified===true&&verifiedRetailEvidence(item.evidence)):[];}
  function retailerMembership(food,retailerId){return retailerMemberships(food).filter(item=>item.retailerId===retailerId);}
  // Reviewed, pinned community store evidence stays distinct from verified
  // listings and from brand ownership. Only literal source store tokens count.
  function sourceDeclaredRetailerMembership(food,retailerId){
  ...
  function commercialIdentityMembership(entity,food){
    if(entity?.type==='brand')return consumerBrandMembership(entity,food);
    const matches=entity?.type==='retailer'&&(food?.commercialIdentities||[]).some(item=>item.entityId===entity.id&&item.market==='AU'&&item.verified===true&&['own-label','private-label'].includes(item.relationship)&&brandKey(item.consumerBrand)===brandKey(food.brand)&&!!food.brand&&verifiedRetailEvidence(item.evidence));
    const declared=entity?.type==='retailer'&&privateLabelCollectionMembership(food,entity.id).length>0;
    return {matches:!!matches||declared,reason:matches?'verified-commercial-identity':declared?'source-declared-private-label-identity':'no-verified-commercial-identity'};
  }
  ...
  // profile. Repeated discovery passes commonly receive fresh copies of the
  // same catalogue record. Key the small decision by ALL record content, never
  // just an id/name: nutrition, source evidence and nested edits must invalidate.
  // Optional undefined adapter metadata is represented distinctly from absent
  // fields and null. Reserved marker input, explicit profiles and unsupported
  // values bypass caching; no input field is silently discarded from the key.
  const undefinedCacheKey='\u0000hec-undefined',undefinedCacheValue=Object.freeze({[undefinedCacheKey]:true});
  const catalogueAddabilityCache=new Map();let catalogueAddabilityEvaluator=null;
  function catalogueAddability(food,central,profile){
    if(profile||!food)return central.evaluateAddability(food,{profile});
    if(catalogueAddabilityEvaluator!==central.evaluateAddability){catalogueAddabilityCache.clear();catalogueAddabilityEvaluator=central.evaluateAddability;}
    let signature,cacheable=true;
    try{signature=JSON.stringify(food,function(key,value){const type=typeof value;if(typeof this[key]?.toJSON==='function')cacheable=false;if(type==='undefined')return undefinedCacheValue;if(type==='function'||type==='symbol'||type==='bigint'||type==='number'&&!Number.isFinite(value)||value&&type==='object'&&(Object.prototype.hasOwnProperty.call(value,undefinedCacheKey)||!Array.isArray(value)&&Object.getPrototypeOf(value)!==Object.prototype))cacheable=false;return value;});}catch{cacheable=false;}
    if(!cacheable||!signature||signature.length>32768)return central.evaluateAddability(food,{profile});
    if(catalogueAddabilityCache.has(signature))return JSON.parse(catalogueAddabilityCache.get(signature));
    const decision=central.evaluateAddability(food,{profile});catalogueAddabilityCache.set(signature,JSON.stringify(decision));
    if(catalogueAddabilityCache.size>1024)catalogueAddabilityCache.delete(catalogueAddabilityCache.keys().next().value);
    return decision;
  }
  ...
  // Titles contribute explicit material attributes, not a string-equality veto.
  // No default variant is inferred when a source omits an attribute.
  function titleIdentityEvidence(food){
    const name=norm(canonicalName(food)),raw=String(food?.name||''),out={};
    // "Sandwich slice bread" describes a loaf, not a filled sandwich. Keep the
    // original title for evidence while resolving this specific bread phrase.
    const familyTitle=String(food?.name||'').replace(/\bsandwich\s+(?:slic(?:e|ed)\s+)?bread\b/ig,'bread');
    const concept=SEARCH.foodConceptEvidence({name:familyTitle});
    if(concept.conceptId&&concept.conceptId!=='unknown')out.family=concept.conceptId;
    for(const key of SEARCH.foodConceptRegistry[concept.conceptId]?.facets||[]){const match=SEARCH.conceptFacets[key]?.rules?.find(([,pattern])=>pattern.test(name));if(match)out[key]=norm(match[0]);}
    const rules={
      flavour:[['chocolate chip',/\bchocolate chip\b/],['chocolate',/\bchocolate\b/],['vanilla',/\bvanilla\b/],['strawberry',/\bstrawberr(?:y|ies)\b/],['chilli',/\bchill?i\b/],['plain',/\b(?:original|regular|plain|classic)\b/]],
      fatLevel:[['reduced fat',/\b(?:reduced|low) fat\b/],['regular fat',/\b(?:full cream|whole milk|full fat|regular fat)\b/],['skim',/\bskim(?:med)?\b/],['light',/\b(?:light|lite)\b/]],
      dietaryVariant:[['gluten free',/\bgluten free\b/],['regular',/\b(?:regular|original|classic)\b/]],
      saltLevel:[['reduced salt',/\b(?:salt reduced|reduced salt|low salt|low sodium)\b/],['unsalted',/\b(?:unsalted|salt free)\b/],['regular salt',/\b(?:salted|regular salt)\b/]],
      sweetening:[['unsweetened',/\b(?:unsweetened|no added sugar|sugar free)\b/],['sweetened',/\bsweetened\b/]],
      size:[['small',/\bsmall\b/],['medium',/\bmedium\b/],['large',/\blarge\b/],['mini',/\bmini\b/]],
      format:[['powder',/\b(?:powder|powdered)\b/],['ready to drink',/\bready to drink\b/],['sliced',/\bsliced\b/],['whole',/\bwhole loaf\b/]],
      texture:[['smooth',/\bsmooth\b/],['crunchy',/\bcrunchy\b/]]
    };
    for(const [key,options] of Object.entries(rules)){const match=options.find(([,pattern])=>pattern.test(name));if(match)out[key]=match[0];}
    // Use the existing shared flavour vocabulary for additional supported tastes.
    if(!out.flavour){const match=SEARCH.patterns.flavour?.find(([,pattern])=>pattern.test(name));if(match)out.flavour=norm(match[0]);}
    const multi=raw.match(/\b(\d+)\s*[x×]\s*(\d+(?:\.\d+)?)\s*(kg|g|ml|l)\b/i),count=raw.match(/\b(\d+)\s*(?:pack|pk|pieces?|count)\b/i),pack=metricEvidence(raw);
    if(multi){out.count=Number(multi[1]);const each=metricEvidence(multi[2]+' '+multi[3]);out.pack={amount:each.amount*out.count,unit:each.unit};}else{if(count)out.count=Number(count[1]);if(pack)out.pack=pack;}
    return out;
  }
  function sameGtinIdentityConflicts(left,right){
    if(!left?.barcode||!right?.barcode||canonicalKey(left)!==canonicalKey(right))return [];
    const fields=[],knownBrand=food=>!['','brandnotlisted','unknown'].includes(brandKey(food?.brand));
    if(knownBrand(left)&&knownBrand(right)&&brandKey(left.brand)!==brandKey(right.brand))fields.push('brand');
    const titles=[titleIdentityEvidence(left),titleIdentityEvidence(right)];
    const a=metricEvidence(left.pack||left.packageSize||left.quantity||left.packSize)||titles[0].pack,b=metricEvidence(right.pack||right.packageSize||right.quantity||right.packSize)||titles[1].pack;
    if(a&&b&&(a.amount!==b.amount||a.unit!==b.unit))fields.push('pack');
    for(const key of new Set([...Object.keys(titles[0]),...Object.keys(titles[1])]))if(titles[0][key]!==undefined&&titles[1][key]!==undefined&&JSON.stringify(titles[0][key])!==JSON.stringify(titles[1][key]))fields.push('title-'+key);
    const form=food=>{const value=servingFoundation()?.physicalForm?.(food)?.form||'unknown';return ({'solid-countable':'countable','solid-weight':'weight','packaged-single':'packaged-item'})[value]||value;},x=form(left),y=form(right);
    if(x!=='unknown'&&y!=='unknown'&&x!==y)fields.push('physical-form');
    for(const aliases of [['flavour','flavor'],['productVariant','variantLabel'],['fatLevel'],['saltLevel'],['productSize','size'],['productFormat','format'],['dietaryVariant'],['semanticCount','packCount'],['productFamily','familyId'],['texture'],['grainType','grain']]){
      const titleKey={productSize:'size',productFormat:'format',semanticCount:'count',productFamily:'family',grainType:'grain'}[aliases[0]]||aliases[0],value=(food,title)=>aliases.map(key=>food[key]).find(v=>v!==undefined&&v!==null&&v!==''&&v!==0)??title[titleKey],x=value(left,titles[0]),y=value(right,titles[1]);
      if(x!==undefined&&y!==undefined&&norm(x)!==norm(y))fields.push(aliases[0]);
    }
    if(!fields.length)return [];
    const identity=food=>({recordId:food.id,brand:food.brand,name:food.name,barcode:food.barcode,pack:food.pack||food.packageSize||food.quantity||food.packSize||null,physicalForm:form(food),variantAttributes:Object.fromEntries(['flavour','flavor','productVariant','variantLabel','fatLevel','saltLevel','productSize','size','productFormat','format','dietaryVariant','semanticCount','packCount'].filter(key=>food[key]!==undefined).map(key=>[key,food[key]])),source:sourceEvidence(food)});
    return [{code:'same-gtin-identity-conflict',field:'identity',severity:'material',resolution:'unresolved',fields,evidence:{left:{...identity(left),titleAttributes:titles[0]},right:{...identity(right),titleAttributes:titles[1]}}}];
  }
  function strongDuplicateEvidence(left,right){
    if(sameGtinIdentityConflicts(left,right).length)return {duplicate:false,reason:'same-gtin-identity-conflict'};
  ...
  function canonicaliseRecords(records){
    ...
    return groups.map(group=>{
      ...
      const ordered=[...group].sort((a,b)=>sourceEvidence(a).trustRank-sourceEvidence(b).trustRank||Number(sourceConflicts(a).length>0)-Number(sourceConflicts(b).length>0)||String(b.sourceProvenance?.publishedDate||'').localeCompare(String(a.sourceProvenance?.publishedDate||''))||quality(b)-quality(a)||canonicalKey(a).localeCompare(canonicalKey(b))||String(a.id).localeCompare(String(b.id))),winner=ordered[0],conflicts=group.flatMap(food=>sourceConflicts(food).map(conflict=>food!==winner&&!sourceConflicts(winner).length&&conflict.code!=='same-gtin-identity-conflict'?{...conflict,resolution:conflict.resolution||'quarantined-source-record'}:conflict)),evidence=group.map(food=>({source:sourceEvidence(food),recordId:food.id,canonicalId:canonicalKey(food),name:food.name,brand:food.brand,barcode:food.barcode,physicalForm:food.physicalForm,pack:food.pack||food.packageSize||food.quantity||null,serving:food.manufacturerServing||null,nutrients:food.nutrients||null,nutritionPer100:food.nutritionPer100||null,retailerMemberships:food.retailerMemberships||[]}));
      for(let a=0;a<group.length;a++)for(let b=a+1;b<group.length;b++)conflicts.push(...sameGtinIdentityConflicts(group[a],group[b]));
      ...
      const uniqueEvidence=values=>[...new Map(values.map(value=>[JSON.stringify(value),value])).values()];
      return {...winner,...(unresolved?{loggable:false,nutritionStatus:'needs-review',entryBlockedReason:'Sources disagree about this product identity or nutrition. Review the evidence before logging.'}:{aliases:[...new Set(group.flatMap(food=>[...(food.aliases||[]),food.name]).filter(name=>name&&name!==winner.name))]}),canonicalEvidence:uniqueEvidence([...group.flatMap(food=>food.canonicalEvidence||[]),...evidence]),retailerMemberships:uniqueEvidence(group.flatMap(food=>[...retailerMemberships(food),...(food.retailerMemberships||[]).filter(item=>sourceDeclaredRetailerMembership(food,item.retailerId).includes(item))])),privateLabelCollections:uniqueEvidence(group.flatMap(food=>(food.privateLabelCollections||[]).filter(item=>privateLabelCollectionMembership(food,item.retailerId).includes(item)))),commercialIdentities:uniqueEvidence(group.flatMap(food=>food.commercialIdentities||[])),evidenceConflicts:uniqueEvidence(conflicts),mergedRecordIds:[...new Set(group.flatMap(food=>[food.id,...(food.mergedRecordIds||[])]))]};
    });
  }
  function sourceQuality(food,decision=addability(food)){const evidence=sourceEvidence(food),foreign=/\b(?:beurre|lait|fromage|yaourt|biscottes|portionen|stuck|stueck|unite|unites)\b/.test(norm(`${food?.name||''} ${food?.servingSize||''} ${food?.packageServingText||''}`));return {trust:evidence.trustRank,complete:decision.normalLoggingAllowed,foreignFacing:foreign,current:food?.currentState==='listed-at-retrieval'||food?.itemStatus==='current',au:marketFor(food)==='AU'};}
  ...
  function sortedAustralianProducts(records){const qualities=new Map(records.map(food=>[food,sourceQuality(food)]));return [...records].sort((a,b)=>{const x=qualities.get(a),y=qualities.get(b);return Number(y.au)-Number(x.au)||Number(y.complete)-Number(x.complete)||x.trust-y.trust||Number(x.foreignFacing)-Number(y.foreignFacing)||Number(y.current)-Number(x.current)||norm(a.name).localeCompare(norm(b.name))||canonicalKey(a).localeCompare(canonicalKey(b));});}
  const brandProductCache=new WeakMap();
  async function forEachSearchChunk(records,visit,isCurrent=()=>true){
    const now=()=>global.performance?.now?.()??Date.now();let start=now();
    for(const record of records){
      if(!isCurrent())return false;visit(record);
      if(now()-start>=8){await new Promise(resolve=>setTimeout(resolve,0));start=now();}
    }
    return isCurrent();
  }
  function brandModelCandidates(records,entity){return canonicaliseRecords(records||[]).filter(food=>consumerBrandMembership(entity,food).matches&&!food.legacyPreviewOnly&&food.itemStatus!=='retired');}
  function cacheBrandModel(records,entity,candidates,decisions){
    const qualityMap=new Map(candidates.map(food=>[food,sourceQuality(food,decisions.get(food))]));
    candidates.sort((a,b)=>{const x=qualityMap.get(a),y=qualityMap.get(b);return Number(y.au)-Number(x.au)||Number(y.complete)-Number(x.complete)||x.trust-y.trust||Number(x.foreignFacing)-Number(y.foreignFacing)||Number(y.current)-Number(x.current)||norm(a.name).localeCompare(norm(b.name))||canonicalKey(a).localeCompare(canonicalKey(b));});
    const cached={brand:brandKey(entity.name),products:candidates,decisions};if(records)brandProductCache.set(records,cached);return cached;
  }
  async function prepareBrandResultModel(records,query,{isCurrent=()=>true}={}){
    const intent=queryIntent(query),entity=intent.entity;if(intent.kind!=='brand-family'||!entity||!isCurrent())return false;
    if(brandProductCache.get(records)?.brand===brandKey(entity.name))return true;
    const candidates=brandModelCandidates(records,entity),decisions=new Map();
    if(!await forEachSearchChunk(candidates,food=>decisions.set(food,productEligibility(food,{candidates}).addability),isCurrent))return false;
    cacheBrandModel(records,entity,candidates,decisions);return true;
  }
  function brandResultModel(records,query,{limit=20,offset=0,total=null}={}){
    ...
    let cached=brandProductCache.get(records);if(!cached||cached.brand!==brandKey(entity.name)){
      const candidates=brandModelCandidates(records,entity),decisions=new Map(candidates.map(food=>[food,productEligibility(food,{candidates}).addability]));
      cached=cacheBrandModel(records,entity,candidates,decisions);
    }
  ...
  function resolveQueryIntent(query){
    ...
    const retailer=global.HECRetailerCatalogue?.recognise?.(raw)||null;
    if(retailer)return {kind:'retailer',query:raw,normalised:normal,entity:retailer,productQuery:'',reason:'explicit-retailer-browse'};
    if(exact&&(exact.type!=='brand'||brandFamilyAlias))return {kind:exact.type==='restaurant'?'source':exact.type==='retailer'?'retailer':'brand-family',query:raw,normalised:normal,entity:exact,productQuery:'',reason:exact.type==='retailer'?'explicit-retailer-browse':exact.type==='restaurant'?'recognised-source-only':'recognised-brand-only'};
  ...
  function consumerBrandMembership(entity,food){
    ...
    if(food.brandAdmission?.status==='audited-first-wave'&&(food.sourceBrandTokens||[]).some(token=>[entity.name,...(entity.aliases||[])].some(name=>brandKey(name)===token.key)))return {matches:true,reason:'audited-source-brand-token'};
  ...
  function identityCandidateIndex(candidates){
    const names=new Map(),words=new Map(),brands=new Map(),occurrences=new Map(),rows=[];
    for(const food of candidates){if(!food)continue;const name=norm(food.name),brand=norm(food.brand),row={food,name,brand,key:canonicalKey(food),source:identityLabelSource(food),lines:[food.familyName,food.productFamily,food.productLine].map(norm).filter(Boolean)};rows.push(row);occurrences.set(food,(occurrences.get(food)||0)+1);
      for(const [map,key] of [[names,name+'\u0000'+brand+'\u0000'+row.source],[brands,brand],...[...new Set(tokens(name))].map(word=>[words,word])]){const values=map.get(key)||[];values.push(row);map.set(key,values);}
    }
    return {names,words,brands,rows,occurrences};
  }
  function indexedIdentityPeers(food,index){
    const name=norm(food.name),brand=norm(food.brand),nameTokens=tokens(name),key=canonicalKey(food),sameBrand=(brand?index.brands.get(brand)||[]:index.rows).filter(row=>row.food!==food),residual=brand&&name.startsWith(brand+' ')?name.slice(brand.length+1):name;
    return {sameName:(index.names.get(name+'\u0000'+brand+'\u0000'+identityLabelSource(food))||[]).some(row=>row.food!==food&&row.key!==key&&!(food.foodSourceId&&food.sourceItemId&&food.verified!==false)&&!(food.barcode&&row.food.barcode&&String(food.barcode)!==String(row.food.barcode))),containedByPeer:nameTokens.length===1&&(index.words.get(name)||[]).some(row=>row.food!==food&&row.name!==name),lineLabels:new Set([food.familyName,food.productFamily,food.productLine].map(norm).filter(Boolean).concat(sameBrand.flatMap(row=>row.lines))),linePeer:sameBrand.some(row=>{const peer=brand&&row.name.startsWith(brand+' ')?row.name.slice(brand.length+1):row.name;return peer!==residual&&(' '+peer+' ').includes(' '+residual+' ');}),peerCount:index.rows.length-(index.occurrences.get(food)||0)};
  }
  function exactProductQuality(food,{candidates=[],identityIndex=null}={}){
    if(identityIndex){
      const base=productIdentityQuality(food),name=norm(food?.name),nameTokens=tokens(name),brand=norm(food?.brand),peers=indexedIdentityPeers(food,identityIndex),metadata=[food?.genericName,food?.familyName,food?.productFamily,food?.productLine,food?.categoryFacet,food?.category,...(food?.categories||[]),...(food?.categoryMemberships||[])].map(norm).filter(Boolean),residual=brand&&name.startsWith(brand+' ')?name.slice(brand.length+1):name,lineShell=![RECORD_TYPES.FOOD_SOURCE,RECORD_TYPES.AFCD,RECORD_TYPES.RECIPE].includes(recordType(food))&&peers.lineLabels.has(residual)&&peers.linePeer,sourceIdentity=!!(food?.foodSourceId&&food?.sourceItemId&&food?.verified===true&&meaningfulProductName(food));
      ...
      if(brand&&name===brand){exactEligible=false;reason='brand-reference';}
      else if(base.classification===PRODUCT_QUALITY.BARCODE_ONLY){exactEligible=false;reason='barcode-known-name-incomplete';}
      else if((WEAK_IDENTITY_LABELS.has(name)&&!sourceIdentity)||peers.sameName||lineShell||(!sourceIdentity&&nameTokens.length===1&&(peers.containedByPeer||metadata.includes(name)))){exactEligible=false;reason=peers.sameName?'ambiguous-duplicate-label':WEAK_IDENTITY_LABELS.has(name)?'generic-product-label':'family-or-line-placeholder';}
      return {...base,exactEligible,meaningful:exactEligible,reason,contextCandidates:1+peers.peerCount};
    }
  ...
  function rank(food,query,{saved=false,locallyVerified=false,exactOnly=false}={}){
    const raw=norm(query);if(!raw)return {score:100,tier:'browse'};
    // Exact-preview callers discard every other tier. Avoid calculating fuzzy,
    // category and prefix scores for records that cannot enter that shortlist.
    if(exactOnly&&(corrected(raw)!==raw||![food?.name,`${food?.brand||''} ${food?.name||''}`,...(food?.aliases||[])].some(value=>norm(value)===raw)))return {score:0,tier:'none'};
  ...
  function duplicateIdentityEvidence(left,right){
    ...
    if(sameGtinIdentityConflicts(left,right).length)return {duplicate:false,reason:'same-gtin-identity-conflict',sameCanonical:true};
  ...
  function dedupe(records){
    ...
    for(const food of records||[]){if(!food)continue;const keys=[`canonical:${canonicalKey(food)}`],equivalent=equivalentDisplayKey(food);if(equivalent)keys.push(`equivalent:${equivalent}`);const found=keys.map(key=>indexes.get(key)).find(index=>index!==undefined);if(found===undefined){const index=rows.length;rows.push(food);keys.forEach(key=>indexes.set(key,index));}else{const old=rows[found];if(old!==food&&food.barcode&&old.barcode&&canonicalKey(old)===canonicalKey(food))rows[found]=canonicaliseRecords([old,food])[0];else if(quality(food)>quality(old))rows[found]=food;keys.forEach(key=>indexes.set(key,found));}}
    return rows;
  }
  function dedupeRanked(items){
    const rows=[],indexes=new Map();
    for(const item of items||[]){if(!item?.food)continue;const keys=[`canonical:${canonicalKey(item.food)}`],equivalent=equivalentDisplayKey(item.food);if(equivalent)keys.push(`equivalent:${equivalent}`);const found=keys.map(key=>indexes.get(key)).find(index=>index!==undefined);if(found===undefined){const index=rows.length;rows.push(item);keys.forEach(key=>indexes.set(key,index));}else{const old=rows[found],preferred=Number(item.rank)>Number(old.rank)||(Number(item.rank)===Number(old.rank)&&quality(item.food)>quality(old.food))?item:old;rows[found]=old.food!==item.food&&old.food.barcode&&item.food.barcode&&canonicalKey(old.food)===canonicalKey(item.food)?{...preferred,food:canonicaliseRecords([old.food,item.food])[0]}:preferred;keys.forEach(key=>indexes.set(key,found));}}
    return rows;
  }
  ...
  function partitionSearchRecords(records){
    const list=dedupe(records||[]),authorities=list.filter(authoritativePackagedRecord),legacy=[],legacyKeys=new Set(),byName=new Map();
    // Only shared names can pass sameNamedProduct. Index them once instead of
    // normalising every authority's aliases for every unrelated catalogue row.
    authorities.forEach((food,index)=>{
      for(const name of new Set([food.name,`${food.brand||''} ${food.name||''}`,...(food.aliases||[])].map(norm).filter(Boolean))){
        if(!byName.has(name))byName.set(name,[]);byName.get(name).push(index);
      }
    });
    for(const food of list){
      if(authoritativePackagedRecord(food))continue;
      const matches=new Set();for(const name of [food.name,`${food.brand||''} ${food.name||''}`,...(food.aliases||[])].map(norm))for(const index of byName.get(name)||[])matches.add(index);
      // Preserve the first authority in input order and the existing strong
      // identity evidence check; a shared display name alone never merges rows.
      const authority=[...matches].sort((a,b)=>a-b).map(index=>authorities[index]).find(item=>sameNamedProduct(item,food));if(!authority)continue;
  ...
  // snapshot, not sales or user popularity. Reuse this request's concept facts,
  // but not an array/length cache: records can change brand/category in place.
  // Never scan the catalogue on typing.
  function shortlistBrandCounts(records,conceptId,facts=null){
    const brands=new Map(),seen=new Set();
    for(const food of records){
      if(recordType(food)!==RECORD_TYPES.EXTERNAL||marketFor(food)!=='AU'||!(facts?.get(food)?.compatibility||SEARCH.conceptCompatibility(food,conceptId)).compatible)continue;
  ...
  function conceptShortlist(records,intent){
    ...
    // For bare intent every compatible record is in eligible. Unrelated records
    // cannot contribute a brand count; supplied facets retain the full scope.
    const brandCounts=shortlistBrandCounts(hasKnown?records:eligible,intent.conceptId,hasKnown?null:facts),identityIndex=identityCandidateIndex(eligible);
    for(const food of dedupe(eligible)){
      const evidence=facts.get(food)?.evidence||SEARCH.foodConceptEvidence(food),compatibility=facts.get(food)?.compatibility||SEARCH.conceptCompatibility(food,intent,evidence),compatible=compatibility.compatible,reference=recordType(food)==='afcd';
      ...
      if(!reference&&!exactProductQuality(food,{candidates:eligible,identityIndex}).exactEligible)continue;
      ...
      buckets[group].push({kind:'exact-product',recordId:food.id,name:consumerDisplayName(food),food,addability:addability(food),decisionTrace:{concept:evidence,compatibility,sourceTrust:sourceTier(food),quality:shortlistQuality(food,intent,brandCounts),group}});
    }
    ...
    for(const group of groups)for(const item of group.items)if(item.food)item.decisionTrace.rank=rank(item.food,intent.identityQuery);
