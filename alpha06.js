...
let rc6FoodViewportFrame=0;function rc6SyncFoodSearchViewport(){if(rc6FoodViewportFrame)return;rc6FoodViewportFrame=requestAnimationFrame(()=>{rc6FoodViewportFrame=0;const input=by('food-search'),vv=window.visualViewport,height=vv?.height||window.innerHeight,offsetTop=vv?.offsetTop||0;document.documentElement.style.setProperty('--hec-visual-viewport-height',`${Math.round(height)}px`);document.documentElement.style.setProperty('--hec-visual-viewport-offset',`${Math.round(offsetTop)}px`);document.documentElement.style.setProperty('--hec-keyboard-top',`${Math.round(offsetTop+height)}px`);document.body.classList.toggle('food-search-focused',document.activeElement===input);document.body.classList.toggle('food-search-keyboard-open',document.activeElement===input&&!!vv&&height<window.innerHeight-80);});}
