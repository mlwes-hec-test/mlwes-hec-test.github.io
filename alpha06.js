...
function rc4NamedRestaurantSource(raw,options){
  ...
  const padded=` ${value} `,sources=(registry.allCatalogues?.()||[]).map(catalogue=>catalogue.source).filter(source=>/restaurant|fast food/i.test(source.sourceType||''));
...
function rc4ApplySearchSurface(){const input=by('food-search'),raw=input?.value.trim()||'',exact=rc4ExactProduct(raw),state=ext.ui.foodSourceBrowse||null;if(state){if(C8?.corrected?.(state.query)!==C8?.corrected?.(raw)){state.query=raw;state.shown=20;}rc4RenderSourceSurface(state,raw);return;}if(exact)rc4RenderExactSurface(raw,exact);}
...
function ss633Current(revision,raw){return revision===searchSession633.revision&&(C8?.corrected?.(raw)||normalise(raw))===searchSession633.normalisedQuery;}
...
const us633CommitCurrentWithContext=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){const input=by('food-search'),normal=C8?.corrected?.(input?.value)||normalise(input?.value||'');if(input&&normal!==searchSession633.normalisedQuery)ss633BeginTyping(input);const state=us633CommitCurrentWithContext(reason);if(state){searchSession633.suppliedSourceWords=S23?.sourceModeFromQuery?.(searchSession633.rawQuery)||'';searchSession633.suppliedPreparationWords=S23?.classifyText?.(searchSession633.rawQuery)?.prep||'';searchSession633.destination={date:ext.ui.diaryDate||isoToday(),meal:ext.ui.pendingMeal||''};}return state;};
...
function us633ActivateControl(control){if(!control)return false;if(control.hasAttribute('data-universal-generic'))return us633OpenGeneric(control.dataset.universalGeneric);if(control.hasAttribute('data-universal-source'))return us633OpenSource(control.dataset.universalSource);if(control.hasAttribute('data-universal-back-results')){searchSession633.sourceContext=null;searchSession633.selectedResult=null;return us633RenderSubmitted();}if(control.hasAttribute('data-universal-back-source'))return us633OpenGeneric(searchSession633.sourceContext?.conceptKey);if(control.hasAttribute('data-universal-result')){const food=getFood(control.dataset.universalResult)||window.HECOpenFoodFactsAU?.getLoaded?.(control.dataset.universalResult);if(!food)return false;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(searchSession633.rawQuery||food.name,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}return false;}
...
async function fc633StartExplicit(raw,intent,reason){
  fc633Session=null;const revision=++fc633Revision;ps34GuidedSession=null;ps34GuidedFamily=null;ps34SyncGuidedUiState();ss633Commit(reason,intent);searchSession633.sourceContext=null;searchSession633.selectedResult=null;searchSession633.rawQuery=raw;ext.ui.foodSearch=raw;saveExt();by('food-search')?.blur?.();
  const identity=intent.identityQuery||raw,sourceIntent=C8.queryIntent(identity),entity=sourceIntent.entity,records=allFoods();
  const render=()=>{const matches=records.filter(food=>C8.explicitIdentityMatch(food,identity,sourceIntent,intent));const model=C8.submittedResultModel(matches,raw);searchSession633.submittedModel={...model,groups:model.groups.filter(group=>group.key!=='generic')};us633RenderSubmitted();if(!matches.length)by('food-results').innerHTML='<section class="universal-search-empty" role="status"><p>This item isn’t currently in HEC’s verified catalogue.</p><p>Try another search, or cancel.</p></section>';};
...
function fc633Candidates(){
  ...
  return session.records.filter(food=>S23.conceptCompatibility(food,session).compatible).filter(food=>{
    ...
    if(source==='ready-to-eat')return !!food.foodSourceId&&food.verified!==false;
...
function fc633Render(){
  ...
  if(session.known.sourceContext==='home-prepared'){session.question=null;content='<p>Build the home recipe from its ingredients, or choose Typical for an available Australian reference.</p><button type="button" class="secondary" data-open-feature="recipe-builder">Build From Ingredients</button>';}
    ...
    const groups=new Map();for(const food of candidates){const label=C8.recordType(food)==='afcd'?'Generic Australian reference':food.sourceDisplayName||food.brand||'Brand not listed',group=groups.get(label)||[];group.push(food);groups.set(label,group);}
...
async function fc633Enrich(session,revision){
  ...
  try{
    ...
    if(fc633Session!==session||revision!==fc633Revision)return;
  ...
  }catch(error){if(fc633Session===session)session.catalogueError=String(error?.message||error);}
  finally{if(fc633Session===session&&revision===fc633Revision){session.loading=false;fc633Render();}}
}
function fc633Start(raw,intent,reason){
  ...
  fc633Session={...intent,known:{...intent.known},records:allFoods(),history:[],loading:true,question:null,stage:'shortlist'};
  searchSession633.rawQuery=raw;searchSession633.normalisedQuery=C8.corrected(raw);searchSession633.sourceContext=null;searchSession633.selectedResult=null;
...
const fc633CommitBase=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){
  const input=by('food-search'),raw=input?.value.trim()||'';if(!raw)return false;
  ...
  if(intent.generic){if(C8.corrected(raw)!==searchSession633.normalisedQuery)ss633BeginTyping(input);return fc633Start(raw,intent,reason);}
  fc633Session=null;fc633Revision++;return fc633CommitBase(reason);
};
const fc633TypingBase=ss633BeginTyping;ss633BeginTyping=function(input){if(C8.corrected(input?.value)!==searchSession633.normalisedQuery){fc633Session=null;fc633Revision++;}return fc633TypingBase(input);};
const fc633LibraryBase=renderLibrary;renderLibrary=function(){if(fc633Session&&!ps34GuidedSession&&C8.corrected(by('food-search')?.value)===C8.corrected(fc633Session.rawQuery)){fc633Render();return;}return fc633LibraryBase();};
function fc633Activate(control){
  if(!fc633Session||!control)return false;
