...
async function sourceBreadth(page){
  ...
  for(const source of ['ready-to-eat','packaged-frozen']){await page.locator('[data-fc-answer="sourceContext"][data-fc-value="'+source+'"]').click();const candidates=await page.evaluate(()=>HEC_FOOD_CONCEPT_TEST.candidates()),ids=await page.locator('[data-universal-result]').evaluateAll(ns=>ns.map(n=>n.dataset.universalResult));assert.deepEqual([...ids].sort(),candidates.map(f=>f.id).sort());assert(!candidates.some(f=>/quiche|burger|wrap/i.test(f.name)));
    ...
    else{const raw=require('./audit_open_food_facts_au.js').allProducts(),search=require('../search-foundation.js'),rawIds=[...raw,...require('../australian-catalogue-data').packagedProducts].filter(f=>search.conceptCompatibility(f,'hash-brown').compatible).map(f=>f.id),retained=await page.evaluate(ids=>ids.map(id=>{const food=HEC_CANONICAL_CACHE_TEST.food(id);return {rawId:id,id:food?.id||id,represented:food?.id===id||food?.mergedRecordIds?.includes(id)};}),rawIds);assert(retained.every(row=>row.represented),'Every original packaged identity remains reachable through its retained canonical record');assert.deepEqual([...ids].sort(),[...new Set(retained.map(row=>row.id))].sort());assert(candidates.some(f=>/McCain/i.test(f.brand)));}
...
async function deltaChecks(page){
  ...
  for(const query of ['Milk','Hash Brown']){await submit(page,query);await settled(page);assert.equal(await page.locator('[data-fc-answer]').count(),0);await page.locator('[data-fc-base]').click();const generic=page.locator('[data-fc-answer="catalogueOrigin"][data-fc-value="generic"]');if(await generic.count())await generic.click();assert.equal((await capture(page)).concept.nextFacet,query==='Milk'?'milkSource':'sourceContext');}
...
async function run({baseline=false,flows=false,targeted=false,outputDirectory=fs.mkdtempSync(path.join(os.tmpdir(),'hec-concept-edge-')),viewports=[{width:390,height:844}]}={}){
  ...
  try{for(const viewport of viewports){const diagnostic=qa.evidence(),context=await qa.contextFor(browser,viewport,diagnostic),row={viewport,diagnostic,scenarios:[]};report.contexts.push(row);const page=await context.newPage();await qa.openLibrary(page);
    for(const item of cases.filter(item=>!targeted||['Bread','Whole meal bread','Milk','Hash Brown','Big Mac','Flora ProActiv Light','KFC Wicked Wings'].includes(item.query))){console.log('First screen '+viewport.width+' '+item.query);await submit(page,item.query);await settled(page);const screen=await capture(page),failures=[];
      ...
      if(!baseline)await settled(page);if(item.guidedQuestion){await page.locator('[data-fc-base]').click();const generic=page.locator('[data-fc-answer="catalogueOrigin"][data-fc-value="generic"]');if(await generic.count())await generic.click();assert.match((await capture(page)).question,item.guidedQuestion);}
    }
