...
test('audited brand generation is byte deterministic and committed outputs match',()=>{
  ...
  const report=JSON.parse(first['build-report.json']);assert.equal(report.products,1922);assert.equal(report.brands,342);assert.equal(report.retailerCanonicalReuse,634);assert.equal(report.newCanonicalIdentities,0);
});
