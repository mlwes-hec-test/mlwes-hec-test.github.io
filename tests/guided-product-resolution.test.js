...
test('27. cereal-like family selects a named product and skips its sole measure',()=>{const s=guided.createSession(cereal,'Grain Co Cereal');assert.equal(s.nextQuestion.key,'product');guided.answerDistinction(s,'product','cereal-honey');assert.equal(s.stage,guided.stages.AMOUNT);assert.equal(s.measureAutoSelected,true);});
...
test('46. module loads before alpha and is cached offline',()=>{assert.ok(index.indexOf('guided-product-resolution.js')<index.indexOf('alpha06.js'));assert(require('../release-manifest.json').files['guided-product-resolution.js']);});
test('47. UI presents identity, measure and amount before one final Review editor',()=>{for(const text of ['Which exact product?','How are you measuring it?','How much?','function ps34OpenFinalReview'])assert.match(runtime,new RegExp(text.replace(/[?]/g,'\\?')));assert.doesNotMatch(runtime,/Ready for review|Review And Confirm/);});
