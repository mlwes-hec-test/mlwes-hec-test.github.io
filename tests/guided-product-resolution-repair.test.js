...
test('11. actual rendered Flora Light control follows the delegated click path',()=>{const app=runtimeController();const event=app.clickLabel('Flora Light');assert.equal(event.prevented,true);assert.match(app.live.innerHTML,/Exact canonical product[\s\S]*Flora Light[\s\S]*Choose the serving measure/);});
test('12. actual rendered Flora ProActiv Light control follows the delegated click path',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');assert.equal(app.api.state().session.exactProduct.canonicalId,'packaged:flora-proactiv-light-au');assert.match(app.live.innerHTML,/Choose the serving measure/);});
...
test('19. serving UI is absent before exact product and present after tap',()=>{const app=runtimeController();assert.doesNotMatch(app.live.innerHTML,/Choose the serving measure/);app.clickLabel('Flora Light');assert.match(app.live.innerHTML,/Choose the serving measure/);});
