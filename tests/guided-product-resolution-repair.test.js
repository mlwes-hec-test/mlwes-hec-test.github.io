...
test('11. actual rendered Flora Light control follows the delegated click path',()=>{const app=runtimeController();const event=app.clickLabel('Flora Light');assert.equal(event.prevented,true);assert.match(app.live.innerHTML,/Exact canonical product[\s\S]*Flora Light[\s\S]*How are you measuring it\?/);});
test('12. actual rendered Flora ProActiv Light control follows the delegated click path',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');assert.equal(app.api.state().session.exactProduct.canonicalId,'packaged:flora-proactiv-light-au');assert.match(app.live.innerHTML,/How are you measuring it\?/);});
...
test('19. serving UI is absent before exact product and present after tap',()=>{const app=runtimeController();assert.doesNotMatch(app.live.innerHTML,/How are you measuring it\?/);app.clickLabel('Flora Light');assert.match(app.live.innerHTML,/How are you measuring it\?/);});
...
test('24. rendered grams control accepts amount 5 through the live event path',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');app.clickLabel('g');const amount=app.live.querySelector('[data-gpr-amount]');amount.value='5';app.clickLabel('Continue');assert.equal(app.api.state().session.amount,5);});
test('25. rendered 5 g flow calculates about 18.5 Cal and 77 kJ',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');app.clickLabel('g');app.live.querySelector('[data-gpr-amount]').value='5';app.clickLabel('Continue');assert.match(app.live.innerHTML,/18\.5 Cal \/ 77 kJ/);});
test('26. one manufacturer serve calculates 37 Cal and 154 kJ',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');app.clickLabel('Manufacturer Serve (10 g)');app.live.querySelector('[data-gpr-amount]').value='1';app.clickLabel('Continue');assert.match(app.live.innerHTML,/37 Cal \/ 154 kJ/);});
...
test('31. synthetic family narrowing remains data driven',()=>{const a=packaged({id:'a',canonicalId:'a',name:'Example Original',brand:'Example',productFamily:'Core',variantLabel:'Original'}),b=packaged({id:'b',canonicalId:'b',name:'Example Light',brand:'Example',productFamily:'Core',variantLabel:'Light'}),session=guided.createSession([a,b],'Example');assert.equal(session.nextQuestion.key,'variant');});
