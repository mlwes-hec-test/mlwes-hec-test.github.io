...
function createFoodLibraryHarness(){
  ...
  const context={
    console,foods,catalogue,input,live,results,
  ...
  const integrated=[
    ...
    'alpha0633StripWake','alpha0633ResolvePart','alpha0633VoiceRequest'
  ].map(productionFunction).join('\n');
  const rankAssignment=productionAssignment('searchRank');
  vm.runInContext(`
    const REG29=window.HECAustralianEntityRegistry,S23=window.HECSearchFoundation,C8=window.HECFoodCatalogue;
  ...
  return{context,input,live,results,dispatch};
}
...
test('Quick Voice continues to resolve the corrected shared search identity',()=>{
  const ui=createFoodLibraryHarness(),request=ui.context.alpha0633VoiceRequest('Hey Percy I had Mega breaky wrap for breakfast');
