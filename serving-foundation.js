...
(function(global){
  ...
  function evaluateAddability(food,{profile=null}={}){
    ...
    if(!hasEnergy||recognisedOnly||nutritionBlocked||measureBlocked){const completable=isPackageFood(resolved)||resolved?.private===true||String(resolved?.recordType||'').toLowerCase()==='private';if(completable)return {status:ADDABILITY_STATUSES.COMPLETION,label:'Needs Nutrition Completion',reasonCode:measureBlocked?'portion-conversion-incomplete':!hasEnergy?'energy-missing':recognisedOnly?'identity-recognised-only':'nutrition-review-required',message:String(!hasEnergy?'Energy is missing from the selected nutrition basis. Open Review Nutrition to complete it.':recognisedOnly?'Review Nutrition and confirm the package values before adding this food.':resolved?.entryBlockedReason||(measureBlocked?'Choose a supported serving, g or mL amount in Review Nutrition.':'Review and correct the nutrition values before adding this food.')),actions:completionActions(resolved),normalLoggingAllowed:false};return {status:ADDABILITY_STATUSES.DETAILS,label:'Details only',reasonCode:measureBlocked?'portion-conversion-unavailable':'nutrition-unavailable',message:String(resolved?.entryBlockedReason||'Nutrition is not available for ordinary logging.'),actions:[{id:'details',label:'View Details'}],normalLoggingAllowed:false};}
