...
(function(global){
  ...
  function servingPolicy(food={},semantics=classify(food)){
    ...
    if(type===TYPES.COUNTED&&count>0&&semantics.individualScaling===false)return {...base,defaultAmount:1,defaultUnit:'portion',allowedUnits:['portion'],allowedUnitFamily:'fixed-order',units:{portion:1},unitLabels:{portion:semantics.standardOrderLabel||`${count}-Piece Order`},naturalServingDisplay:`1 order: ${semantics.standardOrderLabel||food.name}`,countMeaningful:false,partialEditingSafe:false};
  ...
  function validate(food,policy=servingPolicy(food)){
    ...
    if(type===TYPES.COUNTED){const wholeOrder=classify(food).individualScaling===false;if(wholeOrder){if(policy.defaultAmount!==1||policy.defaultUnit!=='portion'||policy.allowedUnits.length!==1||policy.countMeaningful)issues.push('fixed-order-semantics');}else if(!policy.countMeaningful||policy.defaultAmount<2)issues.push('count-semantics');}
