...
function ps33RenderBrandFamilyLive(raw,family){const box=by('food-live-results');if(!box)return;box.innerHTML=ps34GuidedMarkup(raw,family,{compact:true});box.classList.toggle('hidden',!box.innerHTML);}
function ps33RenderBrandFamilySurface(raw,family){const results=by('food-results');if(!results)return;results.className='food-results brand-family-results';results.innerHTML=ps34GuidedMarkup(raw,family);}
function ps34RenderGuided(){const raw=by('food-search')?.value.trim()||ps34GuidedQuery,family=ps33BrandFamily(raw);if(!family)return;ps33RenderBrandFamilyLive(raw,family);ps33RenderBrandFamilySurface(raw,family);rc6SyncFoodSearchViewport();}
document.addEventListener('click',event=>{
...
const ps33RenderLiveBase=s23RenderLive;s23RenderLive=function(raw){const term=String(raw||'').trim(),family=ps33BrandFamily(term);if(ps34GuidedQuery&&ps34GuidedQuery!==term){ps34GuidedSession=null;ps34GuidedQuery='';}if(family&&document.activeElement===by('food-search')){ps33RenderBrandFamilyLive(term,family);return;}ps33RenderLiveBase(term);};renderFoodLiveMatches=s23RenderLive;
