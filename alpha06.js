...
const ps33ExactProductBase=rc4ExactProduct;rc4ExactProduct=function(raw){if(ps33BrandFamily(raw))return null;const exact=ps33ExactProductBase(raw);if(!exact||C8?.recordType?.(exact)==='online-candidate')return null;const intent=C8.queryIntent(raw);if(intent.entity?.type==='brand'){const matches=allFoods().filter(food=>C8.explicitIdentityMatch(food,raw,intent)),model=C8.submittedResultModel(matches,raw),best=model.groups.find(group=>group.key==='best')?.items.find(item=>item.addability?.normalLoggingAllowed);if(best)return best.food;}return C8.productEligibility(exact).identityQuality.exactEligible?exact:null;};
