...
function retailerRender633(state){
  ...
  else if(state.categoryId===null)content=`<div class="guided-resolution-choices">${[...model.categories,{id:'*',label:'All Items',count:model.total}].map(category=>`<button type="button" class="guided-resolution-choice" data-retailer-category="${esc(category.id)}"><span>${esc(category.label)} (${category.count})</span><b>›</b></button>`).join('')}</div>`;
  else content=`<h3>${esc(commercial?'Supported products':model.categoryLabel)}</h3>${items.map(us633ResultRow).join('')}${model.offset?'<button type="button" class="secondary" data-retailer-previous>Previous Products</button>':''}${model.hasMore?'<button type="button" class="secondary wide" data-retailer-more>More Products</button>':''}`;
...
document.addEventListener('click',event=>{
  const control=event.target.closest('[data-retailer-category],[data-retailer-more],[data-retailer-previous],[data-retailer-back],[data-retailer-exit],[data-retailer-retry]'),state=retailerBrowse633;
  ...
  if(control.hasAttribute('data-retailer-back')){if(state.scope==='commercial-identity'||state.categoryId===null)return retailerExit633({back:true});return void retailerLoad633(state,{categoryId:null});}
  if(state.loading)return;
  if(control.hasAttribute('data-retailer-category'))return void retailerLoad633(state,{categoryId:control.dataset.retailerCategory});
...
window.HEC_RETAILER_TEST={state:()=>retailerBrowse633?{retailerId:retailerBrowse633.retailerId,scope:retailerBrowse633.scope,categoryId:retailerBrowse633.categoryId,ownerQuery:retailerBrowse633.ownerQuery,ownerRevision:retailerBrowse633.ownerRevision,loading:retailerBrowse633.loading,error:retailerBrowse633.error,result:retailerBrowse633.result}:null};
