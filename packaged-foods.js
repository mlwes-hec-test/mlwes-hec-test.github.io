...
(function(global){
  ...
  const NUTRIENTS=['energyKj','calories','protein','carbs','fat','satFat','fibre','sugar','sodium','calcium','iron','potassium'];
  const METRIC_UNITS=new Set(['g','kg','mL','L']);
  function finiteOrNull(value){if(value===null||value===undefined||typeof value==='string'&&value.trim()==='')return null;const number=Number(value);return Number.isFinite(number)?number:null;}
  ...
  function basisModel({perServing={},per100={},servingAmount=null,servingUnit='',servingText='',manufacturerServing=false,per100Unit='',selectedBasis='',servingsPerPack=null,servingCount=null,servingCountUnit=''}={}){
    const amount=finiteOrNull(servingAmount),unit=String(servingUnit||'');
    return {perServing:nutrientSet(perServing),per100:nutrientSet(per100),servingAmount:amount,servingUnit:unit,servingText:String(servingText||''),manufacturerServing:!!manufacturerServing&&amount!==null&&amount>0&&['g','mL'].includes(unit),per100Unit:per100Unit||unit,selectedBasis,servingsPerPack:finiteOrNull(servingsPerPack),servingCount:finiteOrNull(servingCount),servingCountUnit:String(servingCountUnit||'')};
  }
