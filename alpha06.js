...
let rc6FoodViewportFrame=0;function rc6SyncFoodSearchViewport(){if(rc6FoodViewportFrame)return;rc6FoodViewportFrame=requestAnimationFrame(()=>{rc6FoodViewportFrame=0;const input=by('food-search'),vv=window.visualViewport,height=vv?.height||window.innerHeight,width=vv?.width||window.innerWidth,offsetTop=vv?.offsetTop||0,focused=document.activeElement===input,keyboardOpen=focused&&!!vv&&height<window.innerHeight-80;document.documentElement.style.setProperty('--hec-visual-viewport-height',`${Math.round(height)}px`);document.documentElement.style.setProperty('--hec-visual-viewport-width',`${Math.round(width)}px`);document.documentElement.style.setProperty('--hec-visual-viewport-offset',`${Math.round(offsetTop)}px`);document.documentElement.style.setProperty('--hec-keyboard-top',`${Math.round(offsetTop+height)}px`);document.documentElement.style.setProperty('--hec-search-available-height',`${Math.max(180,Math.round(height-56))}px`);document.body.classList.toggle('food-search-focused',focused);document.body.classList.toggle('food-search-keyboard-open',keyboardOpen);});}
...
window.HEC_PRODUCT_SEMANTICS_TEST={
  classify:food=>PS33?.classify?.(food),policy:food=>PS33?.servingPolicy?.(food),validate:food=>PS33?.validate?.(food),rankedNames:(query,limit=30)=>ps33RankedFoods(query,limit).map(food=>food.name),
  viewport:()=>({height:window.visualViewport?.height||window.innerHeight,width:window.visualViewport?.width||window.innerWidth,focused:document.activeElement===by('food-search'),keyboardOpen:document.body.classList.contains('food-search-keyboard-open')})
};
