...
test('companion speech is gated, non-overlapping, and the written prompt is always present',()=>{
  let enabled=true,spoken=[],cancelled=0;const context={console,setTimeout,clearTimeout,Date,window:null,globalThis:null};context.window=context;context.globalThis=context;context.speechSynthesis={speaking:false,cancel(){cancelled++;}};context.HECSpeakText=text=>{spoken.push(text);return null;};context.alpha0633Companion=()=>({enabled:true,speech:enabled,name:'Shelly'});context.by=()=>null;vm.createContext(context);vm.runInContext(`let alpha0633PromptTimer=null,alpha0633SpeechToken=0,alpha0633SpeechActive=false;${productionFunction('alpha0633SpeechWatchdogDelay')}\n${productionFunction('alpha0633CancelSpeech')}\n${productionFunction('alpha0633Speak')}`,context);
