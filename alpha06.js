...
function updateEntryPreview(){
  ...
  by("entry-nutrition-preview").innerHTML=`<div class="food-detail-title"><div><h3>${esc(food.name)}</h3><p>${esc(food.brand||food.sourceDisplayName||"")} · ${esc(cleanMeasureText(food.serving||""))}</p></div></div>${nutritionCards(values)}<p class="fine"><strong>Nutrition guidance:</strong> ${esc(rc3NutritionGuidance(values))}</p>`;
...
function alpha0633StripWake(text){return S23?.stripVoiceWake?.(text,[alpha0633Companion().name])||normalise(text).replace(/^(?:hey|hi)\s+(?:companion|hec|shelly|shelley)\b\s*/,'').trim();}
function alpha0633ResolvePart(part){
  ...
  const ranked=allFoods().filter(food=>!C8?.canLog||C8.canLog(food)).map(food=>({food,rank:Math.max(searchRank(food,query),C8?.rank?.(food,query)?.score||0)})).filter(result=>result.rank>=700).sort((a,b)=>b.rank-a.rank||normalise(a.food.name).localeCompare(normalise(b.food.name)));if(!ranked.length)return null;const best=ranked[0],food=best.food,unit=defaultUnit(food);return {foodId:food.id,amount:quantity?amount:defaultAmount(food),unit,confidence:best.rank>=1400?'high':'moderate',resolverQuery:query};
}
...
function alpha0633RenderVoiceReview(){
  ...
  by('voice-review-items').innerHTML=voiceParsed.items.length?voiceParsed.items.map((item,index)=>{const food=getFood(item.foodId),values=scaledNutrients(food,item.amount,item.unit),options=Object.keys(food.units||{[item.unit]:1}).map(unit=>`<option value="${esc(unit)}" ${unit===item.unit?'selected':''}>${esc(titleUnit(unitLabel(food,unit)))}</option>`).join(''),confirmation=item.confidence==='moderate'?`<p class="fine"><strong>Check this match:</strong> Did you mean ${formatNaturalAmount(item.amount)} ${esc(friendlyUnitLabel(food,item.unit,item.amount))} of ${esc(food.name)}?</p>`:'';return `<div class="voice-review-row structured-voice-review"><div><strong>${esc(food.name)}</strong><small>${esc(food.brand||food.source||'HEC Food Library')}</small><span class="source-chip ${food.verified?'verified':'trial'}">${esc(food.source||'Source recorded')}</span>${confirmation}</div><div class="voice-review-quantity"><label>Quantity<input type="number" min="0.01" step="0.1" value="${esc(item.amount)}" data-voice-item-amount="${index}"></label><label>Unit<select data-voice-item-unit="${index}">${options}</select></label></div><small>${energyText(values.calories)} · Protein ${nutrientText(values.protein,'g',true)} · Fibre ${nutrientText(values.fibre,'g',true)} · Sodium ${nutrientText(values.sodium,'mg')}</small><button data-remove-voice-item="${index}" class="delete-action" type="button">Remove</button></div>`;}).join(''):`<p class="empty-state">No confident food match was identified for “${esc(voiceParsed.heard)}”. Correct the transcript and choose Review Request again, or use the Diary fallback.</p>`;
  if(by('confirm-voice-log'))by('confirm-voice-log').disabled=!voiceParsed.items.length;
}
