...
let alpha0633Conversation=CONVERSATION?.createConversation?.()||{state:'idle',pendingAction:null},alpha0633ListenMode='request',alpha0633VoiceSaveLocked=false,alpha0633PromptTimer=null;
...
function alpha0633CancelSpeech(){if(alpha0633PromptTimer)clearTimeout(alpha0633PromptTimer);alpha0633PromptTimer=null;window.speechSynthesis?.cancel?.();}
function alpha0633Speak(text){const companion=alpha0633Companion();if(!companion.enabled||!companion.speech||!text)return;alpha0633CancelSpeech();window.HECSpeakText?.(text);}
function alpha0633StopVoice(abort=true){const active=recognition;recognition=null;try{abort?active?.abort?.():active?.stop?.();}catch{}by('stop-voice-log')?.classList.add('hidden');by('start-voice-log')?.setAttribute('aria-pressed','false');}
...
function alpha0633ResolveIntentFood(intent){
  const query=intent.foodText;if(!query)return[];const rank=q=>allFoods().map(food=>({food,rank:Math.max(searchRank(food,q),C8?.rank?.(food,q)?.score||0)})).filter(item=>item.rank>=700).sort((a,b)=>b.rank-a.rank||normalise(a.food.name).localeCompare(normalise(b.food.name)));
...
function alpha0633RenderDetails(){const target=by('voice-review-items');if(!target)return;target.innerHTML=(voiceParsed.items||[]).length?voiceParsed.items.map((item,index)=>{const food=getFood(item.foodId),values=scaledNutrients(food,item.amount,item.unit),options=Object.keys(unitOptions(food)).map(unit=>`<option value="${esc(unit)}" ${unit===item.unit?'selected':''}>${esc(titleUnit(unitLabel(food,unit)))}</option>`).join('');return `<div class="voice-review-row structured-voice-review"><div><strong>${esc(food.name)}</strong><small>${esc(food.sourceDisplayName||food.brand||food.source||'HEC Food Library')}</small><span class="source-chip ${food.verified?'verified':'trial'}">${esc(food.source||'Source recorded')}</span></div><div class="voice-review-quantity"><label>Quantity<input type="number" min="0.01" step="0.1" value="${esc(item.amount)}" data-voice-item-amount="${index}"></label><label>Unit<select data-voice-item-unit="${index}">${options}</select></label></div><small>${energyText(values.calories)} · Protein ${nutrientText(values.protein,'g',true)} · Fibre ${nutrientText(values.fibre,'g',true)} · Sodium ${nutrientText(values.sodium,'mg')} · ${item.loggable?'Ready after confirmation':'Details only — nutrition incomplete'}</small><button data-remove-voice-item="${index}" class="delete-action" type="button">Remove</button></div>`;}).join(''):`<p class="empty-state">No confident food match was identified. Correct the transcript or use Open Diary Add Food.</p>`;}
function alpha0633RenderVoiceReview({speak=true}={}){
  const pending=voiceParsed||{},review=by('voice-review');review?.classList.remove('hidden');if(pending.meal&&by('voice-meal'))by('voice-meal').value=pending.meal;if(pending.localDate&&by('voice-date'))by('voice-date').value=pending.localDate;const summary=alpha0633Summary(pending),issues=(pending.unresolved||[]).map(item=>item.message),ambiguities=(pending.ambiguities||[]).map(item=>`${item.message} Please check the full date.`);if(by('quick-voice-summary'))by('quick-voice-summary').textContent=summary;if(by('quick-voice-clarification'))by('quick-voice-clarification').textContent=[...issues,...ambiguities].join(' ');if(by('quick-log-review-destination'))by('quick-log-review-destination').textContent=pending.meal?`${formatDate(pending.localDate)} · ${pending.meal} · ${pending.status==='planned'?'Planned':'Recorded'}`:'Choose a meal before saving.';alpha0633RenderDetails();const ready=!!pending.items?.length&&!issues.length;if(by('confirm-voice-log'))by('confirm-voice-log').disabled=!ready;if(ready)alpha0633SetState('ready',{pendingAction:pending});else{alpha0633SetState('clarify',{pendingAction:pending});by('quick-voice-manual-fallback')?.setAttribute('open','');}if(speak)alpha0633Speak(ready?summary:(issues[0]||'I need one detail before I can continue.'));
}
function alpha0633InterpretTranscript({speak=true}={}){const text=by('voice-transcript')?.value||'';if(!text.trim()){showActionToast('Say or enter a food request first.',null,3500);return;}alpha0633StopVoice();alpha0633SetState('interpret',{transcript:text});voiceParsed=alpha0633VoiceRequest(text);alpha0633RenderVoiceReview({speak});}
function alpha0633ResetConversation({focusTranscript=false}={}){alpha0633StopVoice();alpha0633CancelSpeech();voiceParsed=[];alpha0633VoiceSaveLocked=false;alpha0633SaveAdapter?.reset?.();alpha0633Conversation=CONVERSATION?.createConversation?.()||{state:'idle',pendingAction:null};alpha0633SetState('open');by('voice-review')?.classList.add('hidden');by('voice-review-items')?.classList.add('hidden');if(by('voice-transcript'))by('voice-transcript').value='';if(by('voice-status'))by('voice-status').textContent='Nothing is listening yet. Tap the microphone when you are ready.';if(by('confirm-voice-log'))by('confirm-voice-log').disabled=true;if(focusTranscript){by('quick-voice-manual-fallback')?.setAttribute('open','');by('voice-transcript')?.focus();}}
function alpha0633HandleResponse(text){const response=CONVERSATION?.classifyResponse?.(text)||'correction';if(response==='confirm'){alpha0633SavePending();return;}if(response==='cancel'){alpha0633ResetConversation();if(by('voice-status'))by('voice-status').textContent='Cancelled. Nothing was saved.';alpha0633Speak('Cancelled. Nothing was saved.');return;}if(response==='change'){alpha0633SetState('clarify',{pendingAction:voiceParsed});by('quick-voice-manual-fallback')?.setAttribute('open','');by('voice-review-items')?.classList.remove('hidden');if(by('voice-status'))by('voice-status').textContent='What would you like to change? Use the fields or tap the microphone again.';return;}const changed=CONVERSATION?.applyCorrection?.(voiceParsed,text,{today:isoToday(),selectedDate:voiceParsed.localDate,selectedMeal:voiceParsed.meal,companionNames:[alpha0633Companion().name]});if(changed?.pending){voiceParsed=changed.pending;if(changed.parsed?.quantity?.explicit)voiceParsed.items=(voiceParsed.items||[]).map(item=>({...item,amount:changed.parsed.quantity.amount}));if(changed.parsed?.recurrence)voiceParsed.entryCount=changed.parsed.recurrence.count*(voiceParsed.items?.length||1);voiceParsed.saveLockIdentity=CONVERSATION?.saveLockKey?.(voiceParsed)||voiceParsed.actionId;alpha0633RenderVoiceReview({speak:true});}}
function alpha0633StartListening(mode='request'){
  if(alpha0633Conversation.state==='listening')return;const companion=alpha0633Companion(),Recognition=window.SpeechRecognition||window.webkitSpeechRecognition;if(!companion.enabled||!Recognition){by('quick-voice-fallback')?.classList.remove('hidden');by('quick-voice-manual-fallback')?.setAttribute('open','');if(by('voice-status'))by('voice-status').textContent=companion.enabled?'Voice recognition is unavailable. Type the request or open Diary Add Food.':'Choose a companion in Settings, or type the request below.';return;}
  alpha0633ListenMode=mode;ext.ui.prefersVoiceEntry=true;saveExt();alpha0633CancelSpeech();alpha0633SetState('startListening');by('stop-voice-log')?.classList.remove('hidden');if(by('voice-status'))by('voice-status').textContent=mode==='response'?'Listening for yes, cancel or a change…':'Listening… Speak naturally.';let captured='';recognition=new Recognition();recognition.lang=mainData().preferences?.language||'en-AU';recognition.continuous=false;recognition.interimResults=true;recognition.onresult=event=>{let text='';for(let i=event.resultIndex;i<event.results.length;i++)text+=event.results[i][0].transcript;captured=text.trim()||captured;if(mode==='request'&&by('voice-transcript'))by('voice-transcript').value=captured;};recognition.onerror=event=>{const denied=event.error==='not-allowed'||event.error==='service-not-allowed';alpha0633StopVoice();alpha0633SetState('error',{error:event.error});if(by('voice-status'))by('voice-status').textContent=denied?'Microphone permission was not granted. Type the request or use Diary Add Food.':`Voice recognition stopped: ${event.error}. Try again or type the request.`;if(denied){by('quick-voice-fallback')?.classList.remove('hidden');by('quick-voice-manual-fallback')?.setAttribute('open','');}};recognition.onend=()=>{recognition=null;by('stop-voice-log')?.classList.add('hidden');if(!captured)return;if(mode==='request'){if(by('voice-transcript'))by('voice-transcript').value=captured;alpha0633SetState('captured',{transcript:captured});alpha0633InterpretTranscript({speak:true});}else{alpha0633SetState('captured',{transcript:captured});alpha0633HandleResponse(captured);}};try{recognition.start();}catch{alpha0633StopVoice();alpha0633SetState('error',{error:'start'});}
}
function alpha0633CommitPending(pending){
  ...
  for(const date of dates){for(const item of pending.items){const food=getFood(item.foodId);if(!food||!item.loggable)continue;const ref=`${pending.actionId}|${date}|${pending.meal}|${item.canonicalId||item.foodId}`;if((ext.diary[date]||[]).some(entry=>entry.voiceActionRef===ref))continue;const values=scaledNutrients(food,item.amount,item.unit),label=unitLabel(food,item.unit),status=pending.recurrence||date>isoToday()||pending.status==='planned'?'planned':'eaten';staged.push({date,entry:{id:uid('entry'),foodId:food.id,name:food.name,brand:food.brand,date,meal:pending.meal,status,amount:item.amount,unit:item.unit,unitLabel:label,time:status==='planned'?'':localClock(),notes:status==='planned'?'Planned after companion voice confirmation':'Recorded after companion voice confirmation',nutrients:values,foodSnapshot:P8?.diarySnapshot?.(food,{amount:item.amount,unit:item.unit,unitLabel:label,nutrients:values})||null,foodGroups:scaledFoodGroups(food,item.amount,item.unit),waterMl:scaledWaterMl(food,item.amount,item.unit),hydrationType:food.hydrationType||'food',score:food.score,source:`Companion Confirmed Action · ${food.source}`,localDate:date,timeZone:activeTimeZone(),voiceActionRef:ref,plannerRef:status==='planned'?`voice-plan|${ref}`:'',createdAt:new Date().toISOString()}});}}
...
function alpha0633UndoCreated(created=[]){for(const record of created){ext.diary[record.date]=(ext.diary[record.date]||[]).filter(entry=>entry.id!==record.id);if(!ext.diary[record.date].length)delete ext.diary[record.date];}saveExt();renderDiary();return created.length;}
...
function alpha0633SavePending(){
  if(alpha0633VoiceSaveLocked||!voiceParsed?.items?.length||(voiceParsed.unresolved||[]).length)return;alpha0633VoiceSaveLocked=true;alpha0633SetState('save',{pendingAction:voiceParsed});const confirm=by('confirm-voice-log');if(confirm)confirm.disabled=true;let outcome;
...
document.addEventListener('click',event=>{const control=event.target.closest('#start-voice-log,#voice-response-mic,#stop-voice-log,#parse-voice-log,#confirm-voice-log,#change-voice-log,#view-voice-details,#cancel-voice-review,#clear-voice-log,#quick-log-cancel');if(!control)return;event.preventDefault();event.stopImmediatePropagation();if(control.id==='start-voice-log'){alpha0633StartListening('request');return;}if(control.id==='voice-response-mic'){alpha0633StartListening('response');return;}if(control.id==='stop-voice-log'){alpha0633StopVoice(false);return;}if(control.id==='parse-voice-log'){alpha0633InterpretTranscript({speak:true});return;}if(control.id==='confirm-voice-log'){alpha0633SavePending();return;}if(control.id==='change-voice-log'){alpha0633HandleResponse('change');return;}if(control.id==='view-voice-details'){by('voice-review-items')?.classList.toggle('hidden');by('quick-voice-manual-fallback')?.setAttribute('open','');return;}if(control.id==='cancel-voice-review'||control.id==='clear-voice-log'){alpha0633ResetConversation({focusTranscript:control.id==='clear-voice-log'});return;}if(control.id==='quick-log-cancel'){alpha0633StopVoice();voiceParsed=[];alpha0633SetState('cancel');ext.ui.diaryDate=by('voice-date')?.value||ext.ui.diaryDate||isoToday();const dest=ext.ui.quickLogOrigin||'home';saveExt();openFeature(dest);}},true);
...
const alpha0633BeforeScreenShow=window.HECBeforeScreenShow;window.HECBeforeScreenShow=function(id){if(id!=='quick-log'){alpha0633StopVoice();alpha0633CancelSpeech();}alpha0633BeforeScreenShow?.(id);};
