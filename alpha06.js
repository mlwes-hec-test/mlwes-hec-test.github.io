...
function allFoods(){
  ...
  return [...nonRecipeFoods(),...recipes,...(window.HECOpenFoodFactsAU?.loadedFoods?.values?.()||[]),...(window.HECRetailerCatalogue?.loadedFoods?.values?.()||[])];
}
...
function fc633Render(){
  ...
  session.question=session.known.breadSource==='supermarket'?{key:'retailerIdentity',question:'Which supermarket / brand identity?',options:window.HECRetailerCatalogue.commercialOptions(session.conceptId).map(item=>({value:item.id,label:item.label})),reason:'verified-private-label-identity'}:S23.nextConceptQuestion(session,candidates);session.stage='identity';
  ...
  else if(session.question){const question=session.question;content=`<div class="food-concept-question"><strong>${esc(question.question)}</strong></div>${!question.options.length?'<p>No verified house-brand products have been added for this food yet.</p>':''}<div class="guided-resolution-choices">${question.options.map(option=>`<button type="button" class="guided-resolution-choice" data-fc-answer="${esc(question.key)}" data-fc-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('')}</div>`;}
...
function fc633Activate(control){
  ...
  fc633Session.history.push({key,value,label:control.querySelector('span')?.innerText||value,before:{...fc633Session.known}});fc633Session.known[key]=value;fc633Render();return true;
}
const fc633ActivateBase=us633ActivateControl;us633ActivateControl=function(control){if(retailerBrowse633&&retailerCurrent633(retailerBrowse633)&&control?.hasAttribute('data-universal-result'))return fc633ActivateBase(control);if(fc633Session&&control?.matches('[data-fc-answer],[data-fc-back],[data-fc-cancel],[data-fc-base]'))return fc633Activate(control);if(fc633Session&&control?.hasAttribute('data-universal-result')){const food=(fc633Session.stage==='shortlist'?fc633Session.records:fc633Candidates()).find(item=>item.id===control.dataset.universalResult);if(!food)return false;fc633Session.stage='resolved';fc633Session.selectedId=food.id;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(fc633Session.rawQuery,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}return fc633ActivateBase(control);};
window.HEC_FOOD_CONCEPT_TEST={state:fc633Snapshot,candidates:()=>fc633Candidates().map(food=>({id:food.id,name:food.name,brand:food.brand,source:food.foodSourceId,concept:S23.foodConceptEvidence(food),attributes:S23.conceptAttributes(food,fc633Session?.conceptId)}))};
...
function au633LoadBrand(query){
  if(au633BrandState&&au633BrandState.query===query&&au633BrandQueryCurrent(au633BrandState)){au633RenderBrand(au633BrandState);return au633BrandState;}
  const state={query,ownerQuery:searchSession633.rawQuery,revision:searchSession633.revision,records:[],loading:true,offset:0};au633BrandState=state;au633RenderBrand(state);
  const entity=C8.queryIntent(query).entity;
  state.promise=(async()=>{
    const isCurrent=()=>au633BrandQueryCurrent(state);
    try{const result=await catalogueSearch633(query,{limit:500,isCurrent});if(!isCurrent())return;state.records=C8.canonicaliseRecords([...allFoods().filter(food=>C8.consumerBrandMembership(entity,food).matches),...result.foods]);state.catalogueTotal=result.total;state.catalogueLoaded=result.foods.length;}
    catch(error){if(isCurrent()){state.error=String(error?.message||error);state.records=allFoods().filter(food=>C8.consumerBrandMembership(entity,food).matches);}}
    finally{if(isCurrent()&&await C8.prepareBrandResultModel(state.records,query,{isCurrent})){state.loading=false;au633RenderBrand(state);}}
  })();return state;
}
...
function retailerRender633(state){
  ...
  const R=window.HECRetailerCatalogue,model=state.result||R.directory(state.retailerId,state),retailer=model?.retailer||R.entity(state.retailerId),commercial=state.scope==='commercial-identity',collection=retailer?.collectionMode==='source-declared-brand';
  ...
  else if(!model?.total)content=`<p>${commercial?'No verified house-brand products have been added for this food yet.':'No verified retailer food catalogue has been added yet.'}</p>`;
...
const retailerLiveBase633=s23RenderLive;s23RenderLive=function(raw){const intent=C8.queryIntent(raw);if(intent.kind!=='retailer'||ps34GuidedSession)return retailerLiveBase633(raw);const box=by('food-live-results'),collection=window.HECRetailerCatalogue?.entity(intent.entity.id)?.collectionMode==='source-declared-brand';if(!box)return;box.innerHTML=`<section class="guided-resolution-card"><strong>${esc(intent.entity.name)} · ${collection?'Private-testing collection':'Retailer food catalogue'}</strong><small>${collection?'Brand records from Open Food Facts. Current availability is unknown.':'Search to browse foods with verified retailer listing evidence.'}</small></section>`;box.classList.remove('hidden','result-surface-inactive');box.removeAttribute('aria-hidden');};renderFoodLiveMatches=s23RenderLive;
