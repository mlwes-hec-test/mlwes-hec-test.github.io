...
function alpha0633SpokenSummary(pending){const itemText=(pending.items||[]).map(alpha0633ItemPhrase).join(' and ')||'that',meal=String(pending.meal||'meal').toLowerCase();if(pending.recurrence)return `Plan ${itemText} for ${meal} every day for ${pending.entryCount} ${pending.entryCount===1?'entry':'entries'}. Is that correct?`;return `${pending.status==='planned'?'Plan':'Add'} ${itemText} to ${alpha0633NaturalDate(pending)}’s ${meal}. Is that correct?`;}
...
document.addEventListener('click',event=>{
  ...
  if(event.target.closest?.('[data-rc4-leave-source]')){event.preventDefault();event.stopImmediatePropagation();if(by('food-search'))by('food-search').value='';ext.ui.foodSearch='';delete ext.ui.foodSourceBrowse;saveExt();renderLibrary();by('food-search')?.focus();return;}
...
function rc5ExactDecision(raw){
  ...
  if(context.packaged){const branded=rc5ExactCandidates(query,food=>(C8?.norm?.(food.brand)||normalise(food.brand))===context.packaged.brand);if(branded.length)return{kind:'exact',primary:branded[0],choices:branded.slice(0,1),context};}
  ...
  const exact=rc5ExactProductBase(query);return exact?{kind:'exact',primary:exact,choices:[exact],context}:{kind:'none',primary:null,choices:[],context};
}
rc4ExactProduct=function(raw){const decision=rc5ExactDecision(raw);return decision.kind==='exact'?decision.primary:null;};
rc4SourceOnly=function(raw){const value=C8?.norm?.(raw)||normalise(raw);return rc5SourceOnlyBase(raw)||new Set(['mc donalds','macca']).has(value);};
...
document.addEventListener('click',event=>{
  ...
  if(event.target.closest?.('[data-rc5-back-categories]')){event.preventDefault();event.stopImmediatePropagation();if(by('food-search'))by('food-search').value='';ext.ui.foodSearch='';if(ext.ui.foodSourceBrowse){ext.ui.foodSourceBrowse.query='';delete ext.ui.foodSourceBrowse.category;ext.ui.foodSourceBrowse.shown=20;}saveExt();renderLibrary();by('food-search')?.focus();return;}
