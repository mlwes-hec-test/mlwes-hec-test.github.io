...
function runtimeController(foods=[floraLight,floraPro],{focused=true,query='Flora'}={}){
  const search={id:'food-search',value:query},live=new Surface('food-live-results'),results=new Surface('food-results'),elements=new Map([['food-search',search],['food-live-results',live],['food-results',results]]),document=new FoodDocument(search),review={current:null};if(focused)document.body.classList.add('food-search-compact');else document.activeElement=null;
  const context={console,structuredClone,window:{},document,C8:catalogue,GUIDED_PRODUCTS:guided,allFoods:()=>foods,s23ProductLike:()=>true,rc5SearchContext:raw=>({source:null,product:String(raw||'')}),rc5SourceCountedExact:()=>null,ext:{ui:{diaryDate:'2026-08-31',pendingMeal:'Snacks',foodSearch:query}},isoToday:()=> '2026-08-31',by:id=>elements.get(id)||null,esc:value=>String(value??'').replace(/&/g,'&amp;').replace(/"/g,'&quot;').replace(/</g,'&lt;'),formatNaturalAmount:value=>String(value),energyText:(calories,energyKj)=>`${calories} Cal / ${energyKj} kJ`,rc6SyncFoodSearchViewport(){},saveExt(){},renderLibrary(){},getFood:id=>foods.find(food=>food.id===id),prepareEntry(food,options){review.current={food,options};},clone:structuredClone};
...
test('25. rendered 5 g flow calculates about 18.5 Cal and hands directly to final Review',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');app.clickLabel('g');app.active().querySelector('[data-gpr-amount]').value='5';app.clickLabel('Continue');assert.equal(app.api.state().session.nutrition.calories,18.5);assert.equal(app.api.state().session.nutrition.energyKj,77);assert.equal(app.review.current.options.amount,5);assert.equal(app.review.current.options.unit,'g');});
test('26. one manufacturer serve calculates 37 Cal and hands directly to final Review',()=>{const app=runtimeController();app.clickLabel('Flora ProActiv Light');app.clickLabel('Manufacturer serve (10 g)');app.active().querySelector('[data-gpr-amount]').value='1';app.clickLabel('Continue');assert.equal(app.api.state().session.nutrition.calories,37);assert.equal(app.api.state().session.nutrition.energyKj,154);assert.equal(app.review.current.options.amount,1);assert.equal(app.review.current.options.unit,'serve');});
...
test('32. curated data contains the two current canonical Flora records',()=>{const names=require('../data/australian-catalogue/curated-products.json').filter(food=>food.brand==='Flora').map(food=>food.name);assert.deepEqual(names,['Flora ProActiv Light','Flora Light']);});
...
test('41. actual 30-member Flora renders meaningful products and separates incomplete identities before review',()=>{const app=runtimeController(actualFlora.foods,{query:'Flora'}),session=app.api.state().session,choices=session.nextQuestion.options;assert.equal(guided.presentationForSession(session),'direct-products');assert.ok(choices.length>1);assert.ok(choices.every(choice=>!/^(?:Proactiv|Butter|Margarine|Barcode \d+)$/i.test(choice.label)));const ready=choices.find(choice=>{const food=session.initialCandidates.find(item=>guided.canonicalProductKey(item)===choice.value);return serving.evaluateAddability(food).normalLoggingAllowed;});assert.ok(ready,'a complete Flora product is offered');app.clickLabel(ready.label);assert.equal(app.api.state().session.stage,guided.stages.MEASURE);assert.equal(app.api.state().session.addability.status,'loggable-now');app.clickLabel('g');app.active().querySelector('[data-gpr-amount]').value='5';app.clickLabel('Continue');assert.equal(app.api.state().session.stage,guided.stages.CONFIRMATION);assert.equal(app.review.current.options.unit,'g');});
...
test('44. production controller carries generic Margarine teaspoon state directly into final Review',()=>{const app=runtimeController(progressiveAudit.afcdFoods,{query:'Margarine'});assert.equal(app.api.state().session.genericConcept.name,'Margarine');app.clickLabel('Monounsaturated');app.clickLabel('Reduced fat');app.clickLabel('Regular salt');assert.equal(app.api.state().session.stage,guided.stages.MEASURE);app.clickLabel('Teaspoon (5 g)');app.active().querySelector('[data-gpr-amount]').value='1';app.clickLabel('Continue');assert.equal(app.api.state().session.stage,guided.stages.CONFIRMATION);assert.equal(app.review.current.options.amount,1);assert.equal(app.review.current.options.unit,'tsp');assert.equal(app.review.current.options.consumedPortion.baseQuantity,5);assert.equal(app.review.current.food.units.tsp,.05);});
...
test('46. production controller renders generic Milk without a branded detour',()=>{const app=runtimeController(progressiveAudit.afcdFoods,{query:'Milk'});assert.equal(app.api.state().session.genericConcept.name,'Milk');assert.equal(app.api.state().session.nextQuestion.key,'milkSource');assert.match(app.active().innerHTML,/What type of milk/);app.clickLabel('Cow');assert.equal(app.api.state().session.nextQuestion.key,'fatLevel');assert.match(app.active().innerHTML,/What fat level/);});
test('47. production controller renders sliced Bread and its trusted regular-slice measure',()=>{const app=runtimeController(progressiveAudit.afcdFoods,{query:'Bread'});app.clickLabel('Wholemeal');assert.equal(app.api.state().session.stage,guided.stages.MEASURE);assert.ok(app.control('Regular slice (40 g)'));});
