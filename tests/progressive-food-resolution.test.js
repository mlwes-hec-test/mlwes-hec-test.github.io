...
test('26. only validated spread presets are enabled',()=>{const profile=margarine().servingProfile;assert.deepEqual(profile.portionPresets.map(item=>item.key),['tsp']);for(const key of ['thinSpread','regularSpread','thickSpread'])assert.ok(!profile.measures.some(item=>item.key===key));});
...
test('42. incomplete nutrition remains incomplete',()=>{const food=packaged({id:'partial',name:'Specific Partial Product',nutrients:{protein:2}}),session=guided.createSession([food],food.name);guided.selectMeasure(session,'g');guided.selectAmount(session,5);assert.equal(session.nutrition.calories,null);});
test('43. missing nutrition is not false zero',()=>{const food=packaged({id:'missing',name:'Specific Missing Product',nutrients:{protein:2}}),session=guided.createSession([food],food.name);guided.selectMeasure(session,'g');guided.selectAmount(session,5);assert.notEqual(session.nutrition.calories,0);});
