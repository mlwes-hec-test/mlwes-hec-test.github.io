...
test('47. UI presents identity, measure, amount and review as distinct stages',()=>{for(const text of ['Which exact product?','Choose the serving measure','How many?','Review And Confirm'])assert.match(runtime,new RegExp(text.replace(/[?]/g,'\\?')));});
test('48. responsive guided choices remain bounded at required mobile widths',()=>{assert.match(styles,/guided-resolution-card\{[^}]*max-width:100%/);assert.match(styles,/guided-resolution-choice\{[^}]*min-height:46px/);assert.match(styles,/food-search-compact[\s\S]*?guided-resolution-choice\{min-height:44px/);});
