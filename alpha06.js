...
function availableValueText(value,unit=''){return value===null||value===undefined||value===''?'Not Available':`${formatNumber(value)}${unit?` ${unit}`:''}`;}
...
FOODS.forEach(food => Object.assign(food, FOOD_METADATA[food.id] || {waterMl:food.waterMl??null,foodGroups:food.foodGroups||{}}));
...
// Curated and supplemental products use the same canonical catalogue contract.
FOODS.push(...clone(window.HECAustralianCatalogueData?.packagedProducts||[]));
...
function allFoods(){
  ...
  return [...nonRecipeFoods(),...recipes,...(window.HECOpenFoodFactsAU?.loadedFoods?.values?.()||[]),...(window.HECRetailerCatalogue?.loadedFoods?.values?.()||[]),...(window.HECBrandCatalogue?.loadedFoods?.values?.()||[])];
}
async function catalogueSearch633(query,options={}){
  if(window.HECBrandCatalogue?.recognise(query)){const result=await window.HECBrandCatalogue.search(query,options);if(result)rememberCanonicalFoods(result.foods);return result||{query,total:0,foods:[],hasMore:false};}
  const isCurrent=options.isCurrent||(()=>true),[external,retailer]=await Promise.allSettled([window.HECOpenFoodFactsAU.search(query,options),window.HECRetailerCatalogue.search(query,options)]);
  if(!isCurrent())return {query,total:0,foods:[],hasMore:false};
  const listed=retailer.status==='fulfilled'?retailer.value?.foods||[]:[];
  if(retailer.status==='rejected')throw retailer.reason;
  if(external.status==='rejected'&&!listed.length)throw external.reason;
  const result=external.status==='fulfilled'?external.value:{query,total:0,offset:options.offset||0,hasMore:false,foods:[]};
  const foods=C8.canonicaliseRecords([...listed,...result.foods]);rememberCanonicalFoods(foods);
  return {...result,foods,total:Math.max(result.total,foods.length),hasMore:result.hasMore||!!retailer.value?.hasMore,retailerMatches:retailer.value?.total||0};
}
async function catalogueBarcode633(value){
  const [external,retailer]=await Promise.all([window.HECOpenFoodFactsAU.lookupBarcode(value),window.HECRetailerCatalogue.search(value)]);
  const foods=C8.canonicaliseRecords([...(retailer?.foods||[]),...(external?[external]:[])]);rememberCanonicalFoods(foods);return foods[0]||null;
}
const canonicalCatalogueRecords=new Map();
function rememberCanonicalFoods(records){for(const food of records||[])if(!['private','recipe'].includes(C8?.recordType(food))){canonicalCatalogueRecords.set(food.id,food);for(const id of food.mergedRecordIds||[])canonicalCatalogueRecords.set(id,food);}return records;}
function getFood(id){ return canonicalCatalogueRecords.get(id)||allFoods().find(f => f.id === id)||window.HECOpenFoodFactsAU?.getLoaded?.(id); }
...
function friendlyUnitLabel(food,unit,amount=1){let label=cleanUserUnitLabel(unitLabel(food,unit));const a=Number(amount);if(a>0&&a<=1)return label;const simple={egg:'Egg',largeEgg:'Large Egg',mediumEgg:'Medium Egg',smallEgg:'Small Egg',xLargeEgg:'X-Large Egg',jumboEgg:'Jumbo Egg',kingEgg:'King-Size Egg',eggWhite:'Egg White',yolk:'Egg Yolk',sausage:'Sausage',thinSausage:'Long Thin Sausage',thickSausage:'Long Thick Sausage',cocktailSausage:'Cocktail Sausage',slice:'Slice',regularSlice:'Regular Slice',thickSlice:'Thick Slice',bar:'Bar',sachet:'Sachet',biscuit:'Biscuit',cracker:'Cracker',crispbread:'Crispbread',chip:'Chip',tbsp:'Tablespoon',tsp:'Teaspoon',mL:'mL',g:'g',piece:'Piece',item:'Item',serve:'Serve',bottle:'Bottle',can:'Can',tub:'Tub'};let base=/\([^)]*\)/.test(label)?label:(simple[unit]||label);if(/\([^)]*\)/.test(base)){const m=base.match(/^(.*?)\s*(\([^)]*\))$/);if(m){let word=m[1].trim();if(!/s$/i.test(word))word+='s';return `${word} ${m[2]}`;}}if(!/s$/i.test(base)&&!/^(g|mL|kg|L)$/i.test(base))base+='s';return base;}
...
function entryNaturalQuantity(entry){const unit=entry?.unit||'serving',amount=Number(entry?.amount),food=getFood(entry?.foodId)||{units:{[unit]:1},unitLabels:{[unit]:entry?.unitLabel||unit}};const names={egg:'Egg',largeEgg:'Large Egg',mediumEgg:'Medium Egg',smallEgg:'Small Egg',xLargeEgg:'X-Large Egg',jumboEgg:'Jumbo Egg',kingEgg:'King-Size Egg',eggWhite:'Egg White',yolk:'Egg Yolk',sausage:'Sausage',thinSausage:'Long Thin Sausage',thickSausage:'Long Thick Sausage',cocktailSausage:'Cocktail Sausage',slice:'Slice',regularSlice:'Regular Slice',thickSlice:'Thick Slice',bar:'Bar',sachet:'Sachet',biscuit:'Biscuit',cracker:'Cracker',crispbread:'Crispbread',chip:'Chip',tbsp:'Tablespoon',tsp:'Teaspoon',cup:'Cup',drizzle:'Small Drizzle',mL:'mL',g:'g',piece:'Piece',item:'Item',serve:'Serve',bottle:'Bottle',can:'Can',tub:'Tub',smallHandful:'Small Handful'};let label=names[unit]||cleanUserUnitLabel(entry?.unitLabel||unitLabel(food,unit)).replace(/\s*\([^)]*(?:g|mL)[^)]*\)\s*$/i,'').trim();if(amount>1&&!/^(g|mL|kg|L)$/i.test(label)&&!/s$/i.test(label))label+='s';const consumed=entry?.consumedPortion||entry?.foodSnapshot?.consumedPortion,storedMetric=consumed&&Number(consumed.baseQuantity)>0&&consumed.baseUnit&&consumed.baseUnit!=='count'?{value:Number(consumed.baseQuantity),unit:consumed.baseUnit}:null,metric=storedMetric||naturalMetricEquivalent(food,amount,unit)||entry?.metricEquivalent;const metricText=metric&&Number(metric.value)>0?(amount>1?` · ${formatNumber(metric.value,true)} ${metric.unit} total`:` (${formatNumber(metric.value,true)} ${metric.unit})`):'';return `${formatNaturalAmount(amount)} ${label}${metricText}`.trim();}

function foodMultiplier(food,amount,unit){const normalized=window.HECServingFoundation?.normalizeMassAmount?.(unit,amount)||{unit,amount},multiplier=Number(unitOptions(food)[normalized.unit]);return multiplier>0&&normalized.amount!==null?Math.max(0,n(normalized.amount))*multiplier:NaN;}
function scaledNutrients(food,amount,unit){
  const multiplier = foodMultiplier(food,amount,unit);
  if(!Number.isFinite(multiplier))return Object.fromEntries([...new Set([...NUTRIENT_KEYS,...Object.keys(food?.nutrients||{})])].map(key=>[key,null]));
  const result = {};
...
function scaledNutrientsForConsumedPortion(food,consumedPortion,amount,unit){
  const portionMultiplier=Number(consumedPortion?.amount)*Number(consumedPortion?.conversion?.nutritionMultiplier);if(!Number.isFinite(portionMultiplier)||portionMultiplier<=0)return scaledNutrients(food,amount,unit);const result={};NUTRIENT_KEYS.forEach(key=>{const value=food?.nutrients?.[key];result[key]=value===null||value===undefined||value===""?null:Number(value)*portionMultiplier;});Object.entries(food?.nutrients||{}).forEach(([key,value])=>{if(!(key in result))result[key]=value===null||value===undefined?null:Number(value)*portionMultiplier;});return result;
}
function scaledFoodGroups(food,amount,unit){
  const multiplier = foodMultiplier(food,amount,unit);
  if(food?.recordType==='private'&&food.captureEvidence?.confirmed)return Object.fromEntries(Object.entries(food.foodGroups||{}).map(([key,value])=>[key,value===null||value===undefined||value===''?null:Number(value)*multiplier]));
...
function scaledWaterMl(food,amount,unit){const value=food?.waterMl;return value===null||value===undefined||value===''?null:Math.max(0,Number(value)*foodMultiplier(food,amount,unit));}
...
function energyText(value,officialKj=null){
  ...
  const cal=Number(value),kj=Math.round(hasEnergyValue(officialKj)?Number(officialKj):cal*4.184);
...
function nutrientText(value,unit,detail=false){return value===null||value===undefined||value===''?'Not Available':`${formatNumber(value,detail)} ${unit}`;}
...
function variantSchema(food){
  const established=FOOD_VARIANT_SCHEMAS[food?.id];if(established)return established;
  if(!food?.optionalExtras?.length)return null;const defaults={},fields=food.optionalExtras.map((extra,index)=>{const key=`sourceExtra${index}`;defaults[key]='none';const energy=Number(extra.energyKj),known=extra.energyKj!==null&&extra.energyKj!==undefined&&extra.energyKj!==''&&Number.isFinite(energy);return {key,label:extra.name,options:[['none','No'],['added',known?`Add (+${formatNumber(energy)} kJ official)`:'Add (energy unavailable)']]};});return {fields,defaults,sourceOptionalExtras:true};
}
...
function resolveVariantFood(food,values){
  ...
  if(schema.sourceOptionalExtras){const selected=[];food.optionalExtras.forEach((extra,index)=>{if(v[`sourceExtra${index}`]!=='added')return;selected.push(extra.name);const energy=Number(extra.energyKj),known=extra.energyKj!==null&&extra.energyKj!==undefined&&extra.energyKj!==''&&Number.isFinite(energy);if(known){resolved.nutrients.energyKj=n(resolved.nutrients.energyKj)+energy;resolved.nutrients.calories=Math.round((n(resolved.nutrients.calories)+energy/4.184)*10)/10;}else{resolved.loggable=false;resolved.entryBlockedReason=`${extra.name} has no published fixed energy in the reviewed official source. Remove it or log supported components separately.`;}});if(selected.length){resolved.name+=` + ${selected.join(' + ')}`;
      // Source extras currently publish energy only. Base-only macros and
      // serving weights cannot describe the combined item.
      resolved.nutrients=Object.fromEntries(Object.entries(resolved.nutrients).filter(([key])=>['energyKj','calories'].includes(key)));resolved.nutritionStatus=resolved.loggable===false?'needs-review':'energy-only';resolved.nutritionPer100={};resolved.nutritionPer100g={};resolved.servingWeightG=null;resolved.servingVolumeMl=null;resolved.nutritionPer100Unit='';resolved.nutritionBasis={...(resolved.nutritionBasis||{}),perServing:{...resolved.nutrients},per100:{},servingAmount:null,servingUnit:''};
      for(const key of ['g','kg','mL','L','cup']){delete resolved.units?.[key];delete resolved.unitLabels?.[key];delete resolved.unitOrigins?.[key];}
    }
  } else if(food.id==="egg"){
...
function syncEntryFooterInset(){const footer=document.querySelector('#food-entry-editor .quick-action-row:has(#save-food-entry)'),height=footer&&getComputedStyle(footer).display!=='none'?Math.ceil(footer.getBoundingClientRect().height+24):112;document.documentElement.style.setProperty('--hec-entry-footer-height',`${height}px`);}
const entryFooter=document.querySelector('#food-entry-editor .quick-action-row:has(#save-food-entry)');if(typeof ResizeObserver==='function'&&entryFooter)new ResizeObserver(syncEntryFooterInset).observe(entryFooter);window.visualViewport?.addEventListener('resize',syncEntryFooterInset);window.addEventListener('orientationchange',syncEntryFooterInset,{passive:true});syncEntryFooterInset();
function editorFood(){return editorState?.reviewFood||getFood(editorState?.foodId);}
function editorMeasure(food,unit){return window.HECServingFoundation?.servingMeasureProfile?.(food)?.measures.find(item=>item.key===unit||item.id===unit)||null;}
function syncEditorConsumedPortion(food=editorFood(),amount=by("entry-amount")?.value,unit=by("entry-unit")?.value){if(!editorState||!food||!unit)return null;const measure=editorMeasure(food,unit),factory=window.HECServingFoundation?.consumedPortionState;if(!measure||!factory){editorState.consumedPortion=null;return null;}const prior=editorState.consumedPortion,portion=factory({food,measure,amount,sourceServing:prior?.sourceServing||food.manufacturerServing||null,nutritionBasis:prior?.nutritionBasis||food.nutritionBasis||null});if(portion)editorState.consumedPortion=portion;return portion;}
function prepareEntry(food,{entry=null,date=null,meal=null,status="eaten",source=null,amount=null,unit=null,consumedPortion=null,portionMeasures=null}={}){
  if(!food)return;
  ...
  const carriedPortion=clone(entry?.consumedPortion||entry?.foodSnapshot?.consumedPortion||consumedPortion||null),reviewFood=clone(food);reviewFood.units||={};reviewFood.unitLabels||={};reviewFood.unitOrigins||={};for(const measure of portionMeasures||[]){if(Number(measure?.multiplier)>0){reviewFood.units[measure.key]=Number(measure.multiplier);reviewFood.unitLabels[measure.key]=measure.displayLabel||measure.label||measure.key;reviewFood.unitOrigins[measure.key]={origin:measure.source||'',confidence:measure.confidence||'',sourceType:measure.sourceType||'',sourceReference:measure.sourceReference||'',sourceUrl:measure.sourceUrl||'',retrievedDate:measure.retrievedDate||'',derivation:measure.derivation||'',applicability:measure.applicability||''};}}if(carriedPortion?.measureId&&Number(carriedPortion.conversion?.nutritionMultiplier)>0){reviewFood.units[carriedPortion.measureId]=Number(carriedPortion.conversion.nutritionMultiplier);reviewFood.unitLabels[carriedPortion.measureId]=carriedPortion.measureLabel||carriedPortion.measureId;reviewFood.unitOrigins[carriedPortion.measureId]={origin:carriedPortion.conversionProvenance?.source||'',confidence:carriedPortion.conversionProvenance?.confidence||'',...(carriedPortion.conversionProvenance||{})};}
  const finalProfile=window.HECServingFoundation?.servingMeasureProfile?.(reviewFood);if(finalProfile){Object.assign(reviewFood,finalProfile.resolvedFood);portionMeasures=finalProfile.measures;if(!portionMeasures.length||finalProfile.nutritionBasisConflict){showActionToast(reviewFood.entryBlockedReason,null,8000);showFoodDetails(food.id);return;}if(carriedPortion&&!portionMeasures.some(item=>item.key===carriedPortion.measureId)){unit="";amount=null;}}
  ...
  editorState={foodId:food.id,entryId:entry?.id||null,returnTo:selectedMeal?"food-diary":"food-library",source:source||food.source,variantSelections:entry?.variantSelections||{},libraryOnly:!entry&&!selectedMeal,pendingDiarySave:false,reviewFood,portionMeasures:clone(portionMeasures||[]),consumedPortion:carriedPortion&&reviewFood.units[carriedPortion.measureId]?carriedPortion:null};
  by("entry-editor-title").textContent=entry?`Edit ${entry.name}`:`Review ${reviewFood.name}`;by("entry-date").value=entry?.date||defaultDate;by("entry-meal").value=selectedMeal;
  ...
  by("entry-unit").innerHTML=Object.keys(unitOptions(reviewFood)).filter(x=>unitOptions(reviewFood)[x]!==undefined).map(u=>`<option value="${esc(u)}">${esc(cleanUserUnitLabel(unitLabel(reviewFood,u)))}</option>`).join("");const requestedUnit=entry?.unit||unit||defaultUnit(reviewFood),requestedAmount=entry?.amount??amount??defaultAmount(reviewFood),normalized=window.HECServingFoundation?.normalizeMassAmount?.(requestedUnit,requestedAmount),chosenUnit=normalized?.unit||requestedUnit;by("entry-unit").value=chosenUnit;by("entry-amount").value=normalized?.amount??requestedAmount;renderEntryFractionChoices(reviewFood,chosenUnit);
  by('entry-saved-nutrition-action').innerHTML=!entry?savedNutritionReviewAction(reviewFood):'';
  renderVariantOptions(reviewFood,entry?.variantSelections||{});const safety=foodSafety(reviewFood);by("entry-source-warning").innerHTML=`<strong>${reviewFood.verified?"Verified Trial Source":"Review the Source"}</strong><p>${esc(reviewFood.source||"Source not supplied")}. ${safety.blocked?`<b class="danger-text">${esc(safety.message)}</b>`:"Check the quantity and details before adding."}</p>`;
  ...
  if(!entry)by("save-food-entry-and-food").textContent=editorState.libraryOnly?"Add to Diary & Save to My Foods":"Add & Save to My Foods";updateEntryPreview();openFeature("food-entry-editor");requestAnimationFrame(syncEntryFooterInset);
}
...
function updateEntryPreview(){
  if(!editorState)return;const baseFood=editorFood();if(!baseFood)return;const food=resolveVariantFood(baseFood,selectedVariantValues()),amount=by("entry-amount").value,unit=by("entry-unit").value,consumedPortion=syncEditorConsumedPortion(food,amount,unit),values=scaledNutrientsForConsumedPortion(food,consumedPortion,amount,unit);
  ...
  const selectionBlocked=window.HECFoodCatalogue&&!window.HECFoodCatalogue.canLog(food),selectionText=entryNaturalQuantity({foodId:food.id,amount,unit,unitLabel:unitLabel(food,unit),consumedPortion});if(by("entry-selection-summary"))by("entry-selection-summary").innerHTML=`<strong>${editorState?.libraryOnly?'Food To Save':'You Are Adding'}: ${esc(selectionText)}</strong><span>${selectionBlocked?esc(food.entryBlockedReason||'This selection cannot be saved with fixed nutrition.'):`${energyText(values.calories,values.energyKj)} · ${editorState?.libraryOnly?'Save this source record to My Foods for later use.':'Will be added only after you confirm Add to Diary.'}`}</span>`;
}
by("entry-amount")?.addEventListener("input",updateEntryPreview);
by("entry-unit")?.addEventListener("change",()=>{if(by("entry-amount"))by("entry-amount").value=1;const food=editorFood();if(food)renderEntryFractionChoices(food,by("entry-unit").value);updateEntryPreview();});
...
function saveEditorEntry(andSaveFood=false){
  ...
  const food=resolveVariantFood(editorFood()||baseFood,selectedVariantValues());
  if(window.HECFoodCatalogue&&!window.HECFoodCatalogue.canLog(food)){showActionToast(food.entryBlockedReason||'A selected component has no usable fixed nutrition.',null,8000);return;}
  const amount = n(by("entry-amount").value);
  const unit = by("entry-unit").value;
  if(!editorMeasure(food,unit)){showActionToast('Choose a supported measure for this food.');return;}
  ...
  const consumedPortion=syncEditorConsumedPortion(food,amount,unit),values=scaledNutrientsForConsumedPortion(food,consumedPortion,amount,unit);
...
function snapshotFood(entry){
  const stored=P8?.foodFromSnapshot?.(entry);if(stored)return {...stored,foodGroups:entry.foodGroups||{},waterMl:entry.waterMl??null,hydrationType:entry.hydrationType||"food",score:entry.score};
  return {id:entry.foodId || uid("snapshot"),name:entry.name,brand:entry.brand,defaultAmount:entry.amount,defaultUnit:entry.unit,units:{[entry.unit]:1/entry.amount},unitLabels:{[entry.unit]:entry.unitLabel},serving:`${entry.amount} ${entry.unitLabel}`,nutrients:entry.nutrients,foodGroups:entry.foodGroups||{},waterMl:entry.waterMl??null,hydrationType:entry.hydrationType||"food",score:entry.score,source:entry.source};
}
...
function canonicalCachedOnlinePolicy(query,minRank=620){
  if(!query||query.trim().length<3)return {primary:[],legacy:[]};
  // Canonicalise the full cache before matching: a nonmatching source may carry
  // conflicting evidence for a matching GTIN. Render the retained policy record.
  const online=ext.onlineFoods||[],ids=new Set(online.map(food=>food.id)),policy=C8.partitionSearchRecords([...FOODS,...online,...(window.HECRetailerCatalogue?.loadedFoods?.values?.()||[])]),fromOnline=food=>ids.has(food.id)||(food.mergedRecordIds||[]).some(id=>ids.has(id));
  const rank=items=>items.map(item=>({...item,rank:searchRank(item.food,query)})).filter(item=>fromOnline(item.food)&&item.rank>=minRank).sort((a,b)=>b.rank-a.rank||Number(b.food.country==='Australia')-Number(a.food.country==='Australia')||a.food.name.localeCompare(b.food.name));
  const primary=rank(policy.primary.map(food=>({food}))).map(item=>item.food),legacy=rank(policy.legacy);
  rememberCanonicalFoods([...primary,...legacy.map(item=>item.food)]);
  return {primary,legacy};
}
function cachedOnlineMatches(query){return canonicalCachedOnlinePolicy(query).primary;}
function cachedLegacyMatches(query){return canonicalCachedOnlinePolicy(query).legacy;}
...
function renderLibrary(){
  ...
  if(document.activeElement!==by("food-search"))by("food-search").value=ext.ui.foodSearch||"";
...
function resourceFoodRow(food){
  ...
  const blocked=C8?.addability?.(food)?.message||food.entryBlockedReason||'Enter energy and confirm a supported amount in Review Nutrition.';
...
by("food-search")?.addEventListener("compositionend",alpha0630HandleFoodSearchInput);
...
function renderOnlineLibrary(query=""){
  const {primary:items,legacy}=canonicalCachedOnlinePolicy(query,760),legacyHtml=legacy.length?`<details class="legacy-review-results"><summary>Legacy / conflicting results (${legacy.length})</summary><p>Details only — these cannot be quick-added.</p>${legacy.map(legacyFoodRow).join('')}</details>`:'';
...
let ss633SearchCleanup=null,retailerBrowse633=null; // Registered only after the query-owned state exists.
...
function alpha0630HandleFoodSearchInput(event){
  ...
  const revision=searchSession633.revision,committedRevision=searchSession633.committedRevision;
  // Coalesce preview construction, never native text or revision updates. A
  // timer works on iPhone Safari too; expensive loops use small task batches.
  alpha0630FoodSearchTimer=setTimeout(async()=>{
    alpha0630FoodSearchTimer=null;
    const isCurrent=()=>token===alpha0630FoodSearchUiToken&&ss633Current(revision,value)&&committedRevision===searchSession633.committedRevision;
    if(!isCurrent())return;
    if(C8.queryIntent(value).kind==='retailer'){s23RenderLive(value);keepLiveFoodResultsVisible();return;}
    if(C8.queryIntent(value).kind!=='brand-family'&&!rc5SearchContext(value).source&&s23LikelyProduct(value,s23Concept(value)))await s23PrepareProductMatches(value,isCurrent);
    if(!isCurrent())return;
    if(activeLibraryTab()==='all'&&document.activeElement===input){s23RenderLive(value);if(!au633BrandState||!au633BrandQueryCurrent(au633BrandState))psLargeSchedule(value);}
    else renderLibrary();
    keepLiveFoodResultsVisible();scheduleAllResourcesOnlineSearch();
  },120);
}
...
function alpha0631RenderOnlineProgress(query,loaded,checkingMore=false,revision=psSearchRevisionFor(query),arrivals=[]){
  if(!psSearchRevisionCurrent(revision,query)||(by("food-search")?.value||"")!==query||!q('#food-library.active'))return;
...
async function runOnlineFoodSearch({automatic=false}={}){
  const query=by("food-search")?.value||"",externalQuery=alpha0631ExternalSearchQuery(query),revision=psSearchRevisionFor(query),token=++onlineSearchToken;if(!query||query.length<3){if(!automatic)showActionToast("Enter at least three letters before searching online.",null,5000);return [];}
...
function scheduleAllResourcesOnlineSearch(){
  clearTimeout(allResourcesOnlineTimer);const query=by("food-search")?.value||"";if(!q('#food-library.active')||activeLibraryTab()!=="all"||query.length<3)return;if(query===lastAutoOnlineQuery&&cachedOnlineMatches(query).length)return;
  ...
  if(typeof rc5SearchContext==='function'&&rc5SearchContext(query).source){lastAutoOnlineQuery=query;return;}
  return (async()=>{
  const token=alpha0630FoodSearchUiToken,isCurrent=()=>token===alpha0630FoodSearchUiToken&&q('#food-library.active')&&(by('food-search')?.value||'')===query;let sourceMatch=false;
  if(!await C8.forEachSearchChunk(allFoods().filter(food=>food.foodSourceId),food=>{if(!sourceMatch&&searchRank(food,query)>=760)sourceMatch=true;},isCurrent))return;
  if(sourceMatch){lastAutoOnlineQuery=query;return;}
  // A data-backed generic concept owns its progressive identity flow. Online
  // packaged candidates remain available through the explicit online action,
  // but must not race or pollute the generic resolver while the user is typing.
  if(window.HECGuidedProductResolution?.genericSchemaForQuery?.(query)){lastAutoOnlineQuery=query;return;}
  allResourcesOnlineTimer=setTimeout(()=>{if(!q('#food-library.active')||(by("food-search")?.value||"")!==query)return;lastAutoOnlineQuery=query;runOnlineFoodSearch({automatic:true});},180);
  })();
}
...
function showFoodDetails(id){
  ...
  openModal(food.name,`${cleanMeasureText(food.serving)} · ${food.source}`,"Close",()=>{},`${fixed?`${nutritionCards(food.nutrients)}<p class="fine"><strong>Nutrition guidance:</strong> ${esc(rc3NutritionGuidance(food.nutrients))}</p>`:`<p class="danger-text"><strong>Cannot add to Diary:</strong> ${esc(C8?.addability?.(food)?.message||food.entryBlockedReason||'Enter energy and confirm a supported amount in Review Nutrition.')}</p>`}${(food.barcode||food.recordType==='private'||ext.savedFoodIds.includes(food.id)||(ext.customFoods||[]).some(item=>item.id===food.id))?`<button type="button" class="secondary wide" data-review-product-nutrition="${esc(food.id)}">Review / Update Nutrition</button>`:''}<p><strong>Food-group contribution:</strong> ${FOOD_GROUP_KEYS.filter(key=>groups[key]>0).map(key=>`${esc(FOOD_GROUP_LABELS[key])} ${formatNumber(groups[key],true)} serve`).join(" · ")||"Not yet classified"}</p><p><strong>Estimated water:</strong> ${availableValueText(food.waterMl,'mL')}${food.waterMl===null||food.waterMl===undefined?'':' per listed serving'}</p><p><strong>Ingredients:</strong> ${esc(food.ingredients||"Not Available")}</p>${licence}${identityConflictDetails633(food)}${(food.sourceAnomalies||[]).map(note=>`<p class="fine"><strong>Published-source note:</strong> ${esc(note)}</p>`).join('')}${safety.blocked?`<p class="danger-text"><strong>Profile warning:</strong> ${esc(safety.message)}</p>`:""}`);
...
async function lookupBarcodeProduct(code){
  ...
  const lookupOwner=captureEpoch,controller=new AbortController(),timer=setTimeout(()=>controller.abort(),12000);
  try{
    const lookupEpoch=captureEpoch;const localExternal=await catalogueBarcode633(clean);if(lookupEpoch!==captureEpoch)return null;
    if(localExternal){scanBarcodeFood=localExternal;const review=X8?.barcodeStatus?.(localExternal)||{canAddToDiary:hasEnergyValue(localExternal.nutrients?.calories),canSaveToMyFoods:true,missing:[]};by("scan-food-preview").innerHTML=`<div class="food-detail-title"><div><h3>${esc(localExternal.name)}</h3><p>${esc(localExternal.brand||"Brand Not Listed")} · Barcode ${esc(clean)} · ${esc(localExternal.packageSize||localExternal.serving)}</p></div></div>${nutritionCards(localExternal.nutrients)}`;const status=by("scan-review-status");if(status)status.innerHTML=review.canAddToDiary?'<strong>Australian Catalogue Match — Review Before Choosing</strong><p>Compare this catalogue record with the package.</p>':'<strong>Exact Product Found, But Nutrition Is Incomplete</strong><p>Use Read Nutrition Panel or enter nutrition manually before adding it to Diary.</p>';setCaptureActionState('barcode',X8?.actionsFor?.(localExternal)||{save:true,add:review.canAddToDiary,both:review.canAddToDiary});by("scan-review-card").classList.remove("hidden");by("barcode-status").textContent=`Found ${localExternal.name} in the Australian product catalogue.`;return localExternal;}
    ...
    if(!response.ok)throw new Error(`Lookup ${response.status}`);const data=await response.json();if(lookupEpoch!==captureEpoch)return null;if(data.status===0||!data.product)throw new Error("Product not found");const food=makeOpenFoodFactsFood(data.product),review=X8?.barcodeStatus?.(food)||{canAddToDiary:hasEnergyValue(food.nutrients?.calories),canSaveToMyFoods:true,missing:[]};
  ...
  }catch(error){if(lookupOwner!==captureEpoch)return null;by("barcode-status").innerHTML=`No Barcode Record Was Found For <strong>${esc(clean)}</strong>. Switch To <strong>Nutrition Panel</strong> to read the package directly, or create a custom food.`;showActionToast("Barcode Lookup Did Not Find This Product.",null,5000);return null;}finally{clearTimeout(timer);}
}
...
function alpha08UpdateOcrReview(){
  ...
  const comparison=X8.comparePanel(productCapture?.catalogueFood,productCapture?.extracted?.model||productCapture?.panelModel||model),box=by('capture-comparison'),savedReview=!!productCapture?.savedReview;
  if(productCapture?.catalogueFood){
    box.classList.remove('hidden');
    if(savedReview&&!productCapture.extracted){comparison.status='baseline';comparison.rows=[];}
    if(savedReview&&comparison.rows.some(row=>Math.abs(row.catalogue-row.panel)>.005))comparison.status='different';
    const heading={baseline:'Saved Nutrition Baseline',different:'Package Values Differ From '+(savedReview?'Saved Values':'Barcode Values'),compatible:'Package Values Match Closely',incomplete:'No Comparable Values Yet'}[comparison.status]||'Review Package Values';
    const signature=JSON.stringify({comparison,choice:productCapture.choice});
    if(box.dataset.signature!==signature){box.dataset.signature=signature;box.innerHTML='<strong>'+heading+'</strong><p>For your private My Food only. The catalogue stays unchanged.</p><div class="capture-comparison-rows">'+comparison.rows.map(row=>'<p>'+esc(({calories:'Energy (Cal)',satFat:'Saturated fat (g)',carbs:'Carbohydrate (g)',sugar:'Sugars (g)',sodium:'Sodium (mg)'})[row.key]||row.key+' (g)')+': '+(savedReview?'Saved Value ':'barcode ')+formatNumber(row.catalogue,true)+' → Panel Read Value '+formatNumber(row.panel,true)+((savedReview?Math.abs(row.catalogue-row.panel)>.005:row.different)?' · differs':'')+'</p>').join('')+'</div><label>Use For My Food<select id="capture-value-choice"><option value="">Choose values</option><option value="panel">Use Checked Package Values</option><option value="catalogue">'+(savedReview?'Keep Existing Saved Values':'Use Barcode Values')+'</option></select></label>';by('capture-value-choice').value=productCapture.choice||'';}
...
function fillOcrReview(parsed){
  ...
  if(parsed.ingredients)by('ocr-ingredients').value=parsed.ingredients;
  alpha08SetOcrValues(ALPHA0631_OCR_FIELDS,parsed.perServing);alpha08SetOcrValues(ALPHA08_OCR100_FIELDS,parsed.per100);
  ...
  by('ocr-review').classList.remove('hidden');
  if(productCapture.catalogueFood){
...
by("run-label-ocr")?.addEventListener("click",async()=>{
  if(!scanFile)return;const epoch=++captureEpoch,file=scanFile,box=by('ocr-progress'),button=by('run-label-ocr');let worker;
  button.disabled=true;box.classList.remove('hidden');box.textContent='Preparing The Photo For Nutrition Panel Reading…';
  try{
    const ok=await ensureOcrLibrary();if(epoch!==captureEpoch)return;if(!ok)throw Error('OCR library unavailable');
    const image=await prepareOcrImage(file);if(epoch!==captureEpoch)return;
...
function alpha08PanelFood(){
  ...
  if(ocrReviewedFood){ocrReviewedFood.captureEvidence.confirmedPanel=clone(productCapture.panelModel||review.model);ocrReviewedFood.captureEvidence.selectedModel=clone(review.model);if(productCapture.existingPrivate?.captureEvidence)ocrReviewedFood.captureEvidence.previous=clone(productCapture.existingPrivate.captureEvidence);for(const key of ['customNotes','createdAt','foodGroups','waterMl','hydrationType'])if(productCapture.existingPrivate?.[key]!==undefined)ocrReviewedFood[key]=clone(productCapture.existingPrivate[key]);}
...
function weightChartMarkup(model){
  ...
  const layout=WEIGHT_PROGRESS.chartLayout(model,{width:by("history-bars").clientWidth-2,height:Math.min(390,Math.max(280,innerHeight*.42))});
  const {width:W,height:H,left:L,right:R,top:T,bottom:B}=layout;
  const dense=layout.points.length>(R-L)/10;
  let grid="";for(let index=0;index<=4;index++){const value=model.domain.max-model.domain.span*index/4,yy=T+(B-T)*index/4;grid+=`<line x1="${L}" x2="${R}" y1="${yy}" y2="${yy}" class="stage6-weight-grid"/><text x="${L-10}" y="${yy+4}" text-anchor="end" class="stage6-axis-label">${value.toFixed(1)}</text>`;}
  const polyline=model.points.length>1?`<polyline points="${layout.points.map(point=>`${point.x.toFixed(1)},${point.y.toFixed(1)}`).join(" ")}" class="stage6-weight-line"/>`:"";
  const points=layout.points.map(point=>`<g class="stage6-weight-point ${point.selected?"selected":""}" data-weight-point-id="${esc(point.record.id||point.record.date)}" role="button" tabindex="0" aria-pressed="${point.selected}" aria-label="${esc(formatDate(point.record.date))}, ${n(point.record.weightKg).toFixed(1)} kilograms"><circle class="stage6-point-hit" cx="${point.x.toFixed(1)}" cy="${point.y.toFixed(1)}" r="12"/><circle class="stage6-point-dot" cx="${point.x.toFixed(1)}" cy="${point.y.toFixed(1)}" r="${point.selected?5.5:3.5}"/><title>${esc(formatDate(point.record.date))}: ${n(point.record.weightKg).toFixed(1)} kg</title></g>`).join("");
...
let weightChartResizeFrame=0;
if(typeof ResizeObserver==='function')new ResizeObserver(()=>{cancelAnimationFrame(weightChartResizeFrame);weightChartResizeFrame=requestAnimationFrame(()=>{if(!by('progress-history')?.classList.contains('active'))return;const model=WEIGHT_PROGRESS.chartModel(mainData().weightHistory||[],{period:currentPeriod(),today:isoToday(),selectedId:ext.ui.selectedWeightPointId||''});by('history-bars').innerHTML=weightChartMarkup(model);});}).observe(by('history-bars'));
...
document.addEventListener('click',event=>{
  const compare=event.target.closest('[data-compare-barcode-panel]');if(!compare)return;
  beginProductPanel(scanBarcodeFood||getFood(compare.dataset.compareBarcodePanel));
});
...
function alpha0633ResolvedItem(food,intent,rankInfo={}){let unit=defaultUnit(food),amount=defaultAmount(food),spokenUnit=alpha0633NormalUnit(intent.quantity?.unit||'');const options=unitOptions(food);let measureCompatible=true;if(intent.quantity?.explicit){amount=Number(intent.quantity.amount)||1;if(spokenUnit==='glass'&&food.id==='water'){amount*=250;unit='mL';}else if(spokenUnit){const request=window.HECServingFoundation?.resolveMeasureRequest?.(food,spokenUnit,amount);if(request?.ok){unit=request.measure.key;amount=request.amount;}else{measureCompatible=false;unit="";intent.unresolved||=[];intent.unresolved.push({field:"measure",message:"Choose a supported measure for this food."});}}}const catalogueRank=C8?.rank?.(food,intent.foodText)||{},exact=['exact-name','exact-alias'].includes(catalogueRank.tier);return {foodId:food.id,canonicalId:food.canonicalId||C8?.canonicalKey?.(food)||food.id,name:food.name,source:food.sourceDisplayName||food.brand||food.source||'',amount,unit,confidence:exact?'high':rankInfo.rank>=1400?'high':'moderate',resolverQuery:intent.foodText,loggable:measureCompatible&&(C8?.canLog?C8.canLog(food):hasEnergyValue(food?.nutrients?.calories)),provenance:C8?.provenance?.(food)||{label:food.source||''}};}
function alpha0633CountIdentityFood(intent){
  const count=Number(intent?.quantity?.amount),query=C8?.norm?.(intent?.foodText)||normalise(intent?.foodText);if(!intent?.quantity?.explicit||!Number.isInteger(count)||count<2||!query)return null;if(['g','kg','mL','L','cup','tsp','tbsp'].includes(window.HECServingFoundation?.normalizeMeasure?.(intent.quantity.unit)))return null;
  const matches=allFoods().filter(food=>food.semanticType==='counted-item'&&Number(food.productSemantics?.count||food.semanticCount)===count&&(C8?.norm?.(String(food.name||'').replace(/^\d+\s+/,''))||normalise(String(food.name||'').replace(/^\d+\s+/,'')))===query);
...
function alpha0634GuidedIntentFood(intent){
  ...
  if(session.exactProduct&&session.resolutionState===GUIDED_PRODUCTS.states.INCOMPLETE){intent.unresolved||=[];intent.unresolved.push({field:'food',message:session.identityIssue||'Complete this product’s nutrition before adding it.'});return {handled:true,items:[]};}
...
// Legacy menu-name and nutrition override tables were retired. Registered
// source records now supply every product preview and submitted identity.
document.addEventListener('click',e=>{const w=e.target.closest('[data-alpha0618-sausage-wizard]');if(w)alpha0618StartSausageWizard(by('food-search')?.value||'sausage');},true);
...
function alpha0630QueryContext(raw){
  const key=String(raw||'');if(!key)return{parsed:s23Parsed(raw),entities:[],residualTokens:[]};let ctx=alpha0630QueryContextCache.get(key);if(ctx)return ctx;
...
function s23LikelyProduct(raw,concept=null){
  const p=s23Parsed(raw),q=p.food;if(!q)return false;const cacheKey=`${alpha0630FoodSearchRevision}|${String(raw||'')}`;if(alpha0630ProductIntentCache.has(cacheKey))return alpha0630ProductIntentCache.get(cacheKey);
  ...
  if(!exactGeneric&&(window.HECFoodSources?.sourceForAlias?.(raw)||REG29?.primary?.(raw,['brand','retailer','restaurant'])||s23BrandMatch(raw)||allFoods().some(food=>food.foodSourceId&&C8?.rank?.(food,q)?.score>=900)))answer=true;
...
function s23ProductScorer(raw){
  const ctx=alpha0630QueryContext(raw),residual=ctx.residualTokens,entities=ctx.entities.filter(x=>['brand','retailer','restaurant'].includes(x.entity.type));
  return food=>{
...
function s23CacheProductMatches(cacheKey,scores){
    let scored=scores.filter(x=>x.score>0&&x.entityMatched&&x.missing===0).sort((a,b)=>b.score-a.score||Number(b.food.country==='Australia')-Number(a.food.country==='Australia')||a.food.name.localeCompare(b.food.name));
    if(C8?.dedupeRanked)scored=C8.dedupeRanked(scored.map(x=>({food:x.food,rank:x.score}))).map(x=>({food:x.food,score:x.rank}));
    let ranked=scored.map(x=>x.food);const policy=C8?.partitionSearchRecords?.(allFoods());if(policy){const primary=new Map(policy.primary.map(food=>[C8.canonicalKey(food),food]));ranked=ranked.map(food=>primary.get(C8.canonicalKey(food))).filter(Boolean);}
    rememberCanonicalFoods(ranked);alpha0630ProductMatchCache.set(cacheKey,ranked);if(alpha0630ProductMatchCache.size>120)alpha0630ProductMatchCache.delete(alpha0630ProductMatchCache.keys().next().value);
    return ranked;
}
async function s23PrepareProductMatches(raw,isCurrent){
  const cacheKey=`${alpha0630FoodSearchRevision}|${String(raw||'')}`;if(alpha0630ProductMatchCache.has(cacheKey))return;
  const score=s23ProductScorer(raw),scores=[],dataRevision=alpha0630FoodSearchRevision;
  if(await C8.forEachSearchChunk(allFoods().filter(s23ProductLike),food=>scores.push(score(food)),()=>isCurrent()&&dataRevision===alpha0630FoodSearchRevision))s23CacheProductMatches(cacheKey,scores);
}
function s23ProductMatches(raw,limit=16){
  if(!alpha0630QueryContext(raw).parsed.food)return[];
  const cacheKey=`${alpha0630FoodSearchRevision}|${String(raw||'')}`,ranked=alpha0630ProductMatchCache.get(cacheKey)||s23CacheProductMatches(cacheKey,allFoods().filter(s23ProductLike).map(s23ProductScorer(raw)));
...
function alpha0627StableProductMatches(raw,limit=20){
  const key=String(raw||'');if(!key)return[];const current=s23ProductMatches(raw,Math.max(limit,40));let cached=alpha0627StableSearchCache.get(key);
  ...
  const waveQuery=window.HECBrandCatalogue?.recognise(raw);
  return cached.map(id=>byId.get(id)||getFood(id)).filter(food=>food&&(!waveQuery||window.HECBrandCatalogue.ordinaryChoice(food))).slice(0,limit);
}
function alpha0627InvalidateSearch(raw){const key=String(raw||'');if(key)alpha0627StableSearchCache.delete(key);}
...
function alpha0627SourceModeFromQuery(raw,concept){
  ...
  if(s23BrandMatch(raw)||window.HECFoodSources?.sourceForAlias?.(raw))return'commercial';
  return'';
}
...
function s23StartWizard(raw,sourceMode=''){
  ...
  let candidates=s23ConceptCandidates(raw,concept);candidates=alpha0627SourceModeCandidates(candidates,sourceMode,concept);if(!candidates.length){if(typeof ss633PresentNoMatch==='function')ss633PresentNoMatch(raw,concept.label);else openModal(concept.label,'No reliable Australian nutrition records were found for that food concept yet.','Close',()=>{},'<p>Scan the barcode, read the Nutrition Panel, or create the food so HEC does not guess nutrition.</p>');return;}
...
function s23ApplyFullSurface(){
  ...
  if(window.HECFoodSources?.sourceForAlias?.(raw)||/\bburger king\b/.test(s23Norm(raw)))return;
...
by('run-front-ocr')?.addEventListener('click',async()=>{
  ...
  try{const ok=await ensureOcrLibrary();if(epoch!==captureEpoch)return;if(!ok)throw Error('OCR unavailable');const image=await prepareOcrImage(file);if(epoch!==captureEpoch)return;worker=await Tesseract.createWorker('eng',1);if(epoch!==captureEpoch)return;const result=await worker.recognize(image);if(epoch!==captureEpoch)return;const suggested=alpha0631FrontNameFromText(result.data?.text||'');if(suggested){by('ocr-food-name').value=suggested;by('ocr-package-confirmed').checked=false;status.textContent='Suggested name: '+suggested+'. Check it against the package and edit if needed.';alpha08UpdateOcrReview();}else status.textContent='Enter or correct the product name below.';}
  catch{if(epoch===captureEpoch)status.textContent='The front photo could not be read reliably. You can still enter the product name.';}
...
function rc3ClearSearchContext({preserveMeal=true}={}){
  ...
  if(by('food-search')){by('food-search').value='';ss633BeginTyping(by('food-search'));}saveExt();rc3NeutralSearch();
}
...
window.addEventListener('input',event=>{
  const input=event.target;if(input?.id!=='food-search')return;const value=input.value||'',previous=ext.ui.foodSearch||'',changed=previous!==value;
...
prepareEntry=function(food,opts={}){
  const options=unitOptions(food),next={...opts},locked=!(opts.consumedPortion||opts.entry?.consumedPortion||opts.entry?.foodSnapshot?.consumedPortion)&&food?.lockedServingUnit;
...
function rc4ExactProduct(raw){
  const parsed=s23Parsed(raw),parsedQuery=parsed.food,concept=S23?.conceptFromQuery?.(parsed),bareConcept=!!concept&&(concept.aliases||[]).some(alias=>S23?.singular?.(alias)===S23?.singular?.(parsedQuery));if(bareConcept)return null;const rawQuery=C8?.norm?.(raw)||normalise(raw),products=allFoods().filter(s23ProductLike);if(!parsedQuery&&!rawQuery)return null;let query=rawQuery,ranked=products.map(food=>{const result=C8?.rank?.(food,query,{exactOnly:true})||{score:searchRank(food,query),tier:''};return{food,score:Number(result.score)||0,tier:result.tier||''};}).filter(item=>['exact-name','exact-alias'].includes(item.tier));if(!ranked.length&&parsedQuery&&parsedQuery!==rawQuery){query=parsedQuery;ranked=products.map(food=>{const result=C8?.rank?.(food,query,{exactOnly:true})||{score:searchRank(food,query),tier:''};return{food,score:Number(result.score)||0,tier:result.tier||''};}).filter(item=>['exact-name','exact-alias'].includes(item.tier));}
...
function rc4NamedRestaurantSource(raw,options){
  const {only=false}=options||{};
  const registry=window.HECFoodSources,value=C8?.norm?.(raw)||normalise(raw);if(!registry||!value)return null;
  const exact=registry.sourceForAlias?.(raw);if(exact&&/restaurant|fast food/i.test(exact.sourceType||''))return exact;if(only)return null;
  const padded=` ${value} `,sources=(registry.allSources?.()||(registry.allCatalogues?.()||[]).map(catalogue=>catalogue.source)).filter(source=>/restaurant|fast food/i.test(source.sourceType||''));
  return sources.find(source=>(source.aliases||[]).map(alias=>C8?.norm?.(alias)||normalise(alias)).filter(alias=>alias.length>2).sort((a,b)=>b.length-a.length).some(alias=>padded.includes(` ${alias} `)))||null;
}
function rc4SourceOnly(raw){const source=window.HECFoodSources?.sourceForAlias?.(raw),value=C8?.norm?.(raw)||normalise(raw);return !!source&&/restaurant|fast food/i.test(source.sourceType||'')||new Set(['mcdonalds','mc donalds','maccas','macca','macca s']).has(value);}
...
function rc4RelatedProducts(raw,exact,limit=5){
  const intent=C8.queryIntent(raw),matches=allFoods().filter(food=>s23ProductLike(food)&&C8.explicitIdentityMatch(food,raw,intent)),model=C8.submittedResultModel(matches,raw),items=model.groups.flatMap(group=>group.items||[]).filter(item=>item.food&&item.id!==C8.canonicalKey(exact));
  return rememberCanonicalFoods(items.slice(0,limit).map(item=>({...item.food,...(!item.addability.normalLoggingAllowed?{loggable:false}:{} )})));
}
...
function rc4ApplySearchSurface(){const input=by('food-search'),raw=input?.value.trim()||'',exact=rc4ExactProduct(raw),state=ext.ui.foodSourceBrowse||null;if(state){if(state.query!==raw){state.query=raw;state.shown=20;}rc4RenderSourceSurface(state,raw);return;}if(exact)rc4RenderExactSurface(raw,exact);}
const rc4RenderLibraryBase=renderLibrary;renderLibrary=function(){rc4RenderLibraryBase();rc4ApplySearchSurface();};
// Explicit source navigation changes the filter while retaining the source.
// Begin a new raw revision before restoring that committed context, so late
// preview callbacks are retired without mistaking the action for free typing.
function rc4SetSourceFilter(query,state,reason){
  const input=by('food-search');if(input){input.value=query;if(String(query)!==searchSession633.rawQuery)ss633BeginTyping(input);input.blur?.();}
  ss633Commit(reason,{kind:'restaurant-source',sourceId:state.sourceId});ext.ui.foodSourceBrowse={...state,query,shown:20};ext.ui.foodSearch=query;saveExt();
}
...
document.addEventListener('click',event=>{
  const source=event.target.closest?.('[data-rc4-source-browse]');if(source){event.preventDefault();event.stopImmediatePropagation();const query=source.dataset.rc4SourceQuery||by('food-search')?.value||'';rc4SetSourceFilter(query,{sourceId:source.dataset.rc4SourceBrowse,label:source.dataset.rc4SourceLabel||'Food Source'},'source-tap');renderLibrary();if(typeof rc6ScrollSourceHub==='function')rc6ScrollSourceHub();return;}
  if(event.target.closest?.('[data-rc4-clear-source-filter]')){event.preventDefault();event.stopImmediatePropagation();if(ext.ui.foodSourceBrowse)rc4SetSourceFilter('',ext.ui.foodSourceBrowse,'source-filter-clear');renderLibrary();if(typeof rc6ScrollSourceHub==='function')rc6ScrollSourceHub();return;}
...
function rc5SearchContext(raw){const normal=C8?.norm?.(raw)||normalise(raw),source=typeof rc4NamedRestaurantSource==='function'?rc4NamedRestaurantSource(raw):/\b(?:mcdonalds|mc donalds|maccas|macca s|macca)\b/.test(normal)?{id:'mcdonalds-au',displayName:"McDonald's Australia"}:null,mcdonalds=source?.id==='mcdonalds-au',packaged=source?null:rc5PackagedBrand(raw);let product=String(REG29?.stripRecognisedEntities?.(raw)??raw).trim();if(packaged)product=(C8?.norm?.(product)||normalise(product)).replace(new RegExp(`\\b${packaged.brand.replace(/[.*+?^${}()|[\\]\\]/g,'\\$&')}\\b`,'g'),' ').replace(/\s+/g,' ').trim();product=s23Parsed(product).food||product;return {source,mcdonalds,packaged,product:C8?.corrected?.(product)||normalise(product)};}
function rc5ExactCandidates(query,predicate=()=>true){let ranked=allFoods().filter(food=>s23ProductLike(food)&&predicate(food)).map(food=>({food,result:C8?.rank?.(food,query,{exactOnly:true})||{score:searchRank(food,query),tier:''}})).filter(item=>['exact-name','exact-alias'].includes(item.result.tier));ranked=C8.canonicaliseRecords(ranked.map(item=>item.food)).map(food=>({food,result:C8.rank(food,query)}));ranked.sort((a,b)=>Number(C8.productEligibility(b.food).addability.normalLoggingAllowed)-Number(C8.productEligibility(a.food).addability.normalLoggingAllowed)||b.result.score-a.result.score||Number(!!b.food.foodSourceId)-Number(!!a.food.foodSourceId)||a.food.name.localeCompare(b.food.name));return ranked.map(item=>item.food);}
function rc5SourceFamilyChoice(context,query){
  const family=s23Parsed(query).food;if(!family||(!context.source&&(C8?.friesIntent?.(query)?.generic||window.HECGuidedProductResolution?.genericSchemaForQuery?.(query))))return null;let source=context.source,choices=source?rc4SourceFoods(source.id).filter(food=>[food.choiceFamily,...(food.choiceFamilyAliases||[])].some(value=>s23Parsed(String(value||'').replace(/-/g,' ')).food===family)):[];
  if(!source){const grouped=new Map();for(const food of allFoods().filter(item=>item.foodSourceId&&item.itemStatus!=='retired'&&[item.choiceFamily,...(item.choiceFamilyAliases||[])].some(value=>s23Parsed(String(value||'').replace(/-/g,' ')).food===family))){const group=grouped.get(food.foodSourceId)||[];group.push(food);grouped.set(food.foodSourceId,group);}const eligible=[...grouped.entries()].filter(([,foods])=>foods.length>1);if(eligible.length!==1)return null;const [sourceId,foods]=eligible[0],catalogue=window.HECFoodSources?.getCatalogue?.(sourceId);source=catalogue?.source||{id:sourceId,displayName:foods[0]?.sourceDisplayName||foods[0]?.brand||'Restaurant Source'};choices=foods;context={...context,source,product:family};}
  choices=choices.sort((a,b)=>(Number(a.choiceOrder)||0)-(Number(b.choiceOrder)||0)||a.name.localeCompare(b.name));if(choices.length<2)return null;return {kind:'choice',primary:null,choices,context,reason:`Choose the ${source.displayName} ${String(query).trim()} option that matches the serving you had.`};
}
function rc5SourceCountedExact(context,raw,query){
  if(!context.source)return null;const parsed=s23Parsed(String(REG29?.stripRecognisedEntities?.(raw)??raw)),count=Number(parsed.quantity),family=s23Parsed(query).food;
  if(!parsed.quantityExplicit||parsed.unit||!Number.isInteger(count)||count<1||!family)return null;
  const matches=rc4SourceFoods(context.source.id).filter(food=>food.semanticType==='counted-item'&&Number(food.productSemantics?.count||food.semanticCount)===count&&[food.choiceFamily,...(food.choiceFamilyAliases||[])].some(value=>s23Parsed(String(value||'').replace(/-/g,' ')).food===family));
  return matches.length===1?matches[0]:null;
}
function rc5ExactDecision(raw){
  ...
  if(!context.source&&!context.packaged&&C8?.friesIntent?.(raw)?.generic)return{kind:'none',primary:null,choices:[],context};
  const countedExact=rc5SourceCountedExact(context,raw,query);if(countedExact)return{kind:'exact',primary:countedExact,choices:[countedExact],context};
  const familyChoice=typeof rc5SourceFamilyChoice==='function'?rc5SourceFamilyChoice(context,query):null;if(familyChoice)return familyChoice;
  const registeredBrand=!context.source?REG29?.primary?.(raw,['brand','retailer']):null;if(registeredBrand){const fullIdentity=rc5ExactCandidates(raw,food=>REG29?.entityMatchesHay?.(registeredBrand.entity,`${food.brand||''} ${food.name||''} ${(food.aliases||[]).join(' ')}`));if(fullIdentity.length)return{kind:'exact',primary:fullIdentity[0],choices:fullIdentity.slice(0,1),context};}
  ...
  if(context.source){const official=rc5ExactCandidates(query,food=>food.foodSourceId===context.source.id);return official.length?{kind:'exact',primary:official[0],choices:official.slice(0,1),context}:{kind:'none',primary:null,choices:[],context};}
...
rc4SourceOnly=function(raw){return rc5SourceOnlyBase(raw)||!!rc4NamedRestaurantSource(raw,{only:true});};
...
function rc5CategoryGroups(sourceId){const foods=rc4SourceFoods(sourceId),source=window.HECFoodSources?.getCatalogue?.(sourceId)?.source,categories=source?.inventory?.browseCategories||RC5_SOURCE_CATEGORIES,groups=categories.map(name=>({name,foods:foods.filter(food=>(food.browseCategory||'Other')===name)})).filter(group=>group.foods.length);return {foods,groups};}
...
document.addEventListener('click',event=>{
  const category=event.target.closest?.('[data-rc5-source-category]');if(category){event.preventDefault();event.stopImmediatePropagation();if(ext.ui.foodSourceBrowse)rc4SetSourceFilter('',{...ext.ui.foodSourceBrowse,category:category.dataset.rc5SourceCategory},'source-category');renderLibrary();return;}
  if(event.target.closest?.('[data-rc5-back-categories]')){event.preventDefault();event.stopImmediatePropagation();if(ext.ui.foodSourceBrowse){const state={...ext.ui.foodSourceBrowse};delete state.category;rc4SetSourceFilter('',state,'source-categories');}renderLibrary();if(typeof rc6ScrollSourceHub==='function')rc6ScrollSourceHub();return;}
...
const searchSession633={...(C8?.newUniversalSearchSession?.()||{}),inputNode:by('food-search')||null,rawQuery:'',normalisedQuery:'',focused:false,mode:'idle',commitReason:'',intentKind:'none',revision:0,caretStart:0,caretEnd:0,stableTimer:0,errorOwner:null,submittedModel:null,sourceContext:null};
function ss633Intent(raw){const query=String(raw||'').trim(),context=rc5SearchContext(query),schema=GUIDED_PRODUCTS?.genericSchemaForQuery?.(query),intent=C8?.queryIntent?.(query)||{kind:'product'};if(context.source&&rc4SourceOnly(query))return{kind:'restaurant-source',sourceId:context.source.id,label:context.source.displayName};if(context.source)return{kind:'restaurant-product',sourceId:context.source.id,label:context.source.displayName};if(/^\d{8,14}$/.test((C8?.norm?.(query)||normalise(query)).replace(/\s/g,'')))return{kind:'barcode'};if(schema)return{kind:'generic-food',concept:schema.name};if(intent.kind==='consumer-brand'||intent.kind==='brand-family')return{kind:'consumer-brand'};return{kind:intent.kind||'product'};}
function ss633RetireOwnedError(){const owned=searchSession633.errorOwner,copy=by('a05-modal-copy');if(owned&&copy?.textContent===owned.message){copy.textContent='';q('#a05-modal')?.classList?.add('hidden');}searchSession633.errorOwner=null;}
function ss633PresentNoMatch(raw,title){const message='No reliable Australian nutrition records were found for that food concept yet.',query=String(raw||'');searchSession633.errorOwner={revision:searchSession633.revision,query,message};openModal(title,message,'Close',()=>{},'<p>Scan the barcode, read the Nutrition Panel, or create the food so HEC does not guess nutrition.</p>');}
function ss633BeginTyping(input){if(!input||input.id!=='food-search')return searchSession633;const raw=String(input.value||''),normal=C8?.corrected?.(raw)||normalise(raw),changed=raw!==searchSession633.rawQuery;searchSession633.inputNode ||= input;if(changed){searchSession633.revision+=1;searchSession633.previewRevision=Number(searchSession633.previewRevision||0)+1;searchSession633.normalisedQuery=normal;searchSession633.commitReason='';searchSession633.submittedModel=null;searchSession633.sourceContext=null;searchSession633.selectedResult=null;searchSession633.intent=null;searchSession633.quantity=null;searchSession633.preparationContext=null;searchSession633.ownedAsync=null;if(typeof au633BrandState!=='undefined')au633BrandState=null;if(typeof alpha0627StableSearchCache!=='undefined')alpha0627StableSearchCache.clear();const live=by('food-live-results'),results=by('food-results');if(live){live.innerHTML='';live.classList.add('hidden');}if(results)results.innerHTML='';psSearchBeginRevision(raw);ss633RetireOwnedError();const hadCommittedSource=!!ext.ui.foodSourceBrowse;delete ext.ui.foodSourceBrowse;if(hadCommittedSource)saveExt();if(typeof ps34GuidedQuery!=='undefined'&&ps34GuidedQuery&&ps34GuidedQuery!==raw){ps34GuidedSession=null;ps34GuidedQuery='';ps34GuidedFamily=null;}}searchSession633.rawQuery=raw;searchSession633.currentInput=raw;searchSession633.focused=document.activeElement===input;searchSession633.mode='typing-preview';searchSession633.submissionMode='preview';searchSession633.intentKind=ss633Intent(raw).kind;searchSession633.caretStart=Number.isFinite(input.selectionStart)?input.selectionStart:raw.length;searchSession633.caretEnd=Number.isFinite(input.selectionEnd)?input.selectionEnd:raw.length;if(searchSession633.stableTimer)clearTimeout(searchSession633.stableTimer);const revision=searchSession633.revision;searchSession633.stableTimer=setTimeout(()=>{if(revision===searchSession633.revision&&searchSession633.mode==='typing-preview'&&document.activeElement===input&&String(input.value||'')===raw)searchSession633.mode='stable-preview';},180);return searchSession633;}
function ss633Commit(reason,target={}){alpha0630CancelFoodSearchWork({invalidateOnline:true});clearTimeout(searchSession633.stableTimer);searchSession633.stableTimer=0;searchSession633.mode='explicit-committed';searchSession633.submissionMode=String(reason||'explicit-action');searchSession633.commitReason=String(reason||'explicit-action');searchSession633.committedRevision=Number(searchSession633.committedRevision||0)+1;searchSession633.intentKind=target.kind||searchSession633.intentKind;searchSession633.intent=target;searchSession633.focused=document.activeElement===searchSession633.inputNode;return searchSession633;}
function ss633Current(revision,raw){return revision===searchSession633.revision&&String(raw||'')===searchSession633.rawQuery&&String(by('food-search')?.value||'')===searchSession633.rawQuery;}
...
renderLibrary=function(){rc6RenderLibraryBase();const input=by('food-search'),raw=input?.value.trim()||'';if(!ext.ui.foodSourceBrowse)rc6GroupGenericFries(raw);rc6SyncFoodSearchViewport();};
window.addEventListener('input',event=>{const input=event.target;if(input?.id!=='food-search')return;ss633BeginTyping(input);},true);
...
function ps33ResultSurfaceOwner(owner){
  const live=by('food-live-results'),main=by('food-results');
  if(owner==='live'){
    if(main){main.innerHTML='';main.classList.add('result-surface-inactive');main.setAttribute('aria-hidden','true');}
    live?.classList.remove('result-surface-inactive','hidden');live?.removeAttribute('aria-hidden');
  }else if(owner==='main'){
    if(live){live.innerHTML='';live.classList.add('result-surface-inactive','hidden');live.setAttribute('aria-hidden','true');}
    main?.classList.remove('result-surface-inactive');main?.removeAttribute('aria-hidden');
  }
  return owner;
}
function ps33EnforceResultSurfaceOwnership(){const live=by('food-live-results'),main=by('food-results'),focused=document.activeElement===by('food-search'),liveReady=!!live?.innerHTML.trim()&&!live.classList.contains('hidden'),mainReady=!!main?.innerHTML.trim();return focused&&liveReady?ps33ResultSurfaceOwner('live'):mainReady?ps33ResultSurfaceOwner('main'):liveReady?ps33ResultSurfaceOwner('live'):'none';}
function ps33DecisionSourceLabel(decision){const sourceId=decision?.context?.source?.id,records=[decision?.primary,...(decision?.choices||[])].filter(Boolean),active=records.find(food=>food.foodSourceId&&(!sourceId||food.foodSourceId===sourceId))||records.find(food=>food.foodSourceId);return active?.sourceDisplayName||active?.brand||decision?.context?.source?.displayName||'Restaurant source';}
...
function ps33PreviewSource(raw,source){const box=by('food-live-results');if(!box||document.activeElement!==by('food-search'))return false;box.innerHTML=`<div class="live-match-heading"><span><strong>Restaurant source recognised</strong><small>Keep typing, or explicitly open the loaded menu.</small></span></div>${rc4SourceButton({sourceId:source.id,label:source.displayName},raw)}`;box.classList.remove('hidden');return true;}
function ps33PreviewFamily(raw,family){const box=by('food-live-results');if(!box||document.activeElement!==by('food-search'))return false;const generic=family.intent?.kind==='generic-category',label=family.entity?.name||raw,copy=generic?'Resolve the generic Australian food before choosing a measure or amount.':'Browse the recognised brand products before choosing a serving.';box.innerHTML=`<section class="guided-resolution-card guided-resolution-compact ps33-family-preview"><div class="guided-resolution-understanding"><strong>${generic?'Generic food':'Recognised brand'}: ${esc(label)}</strong><small>${esc(copy)}</small></div><button type="button" class="primary wide" data-ps33-commit-family>${generic?'Resolve This Food':'Browse Products'}</button><small>Recognition is a preview. Nothing is selected until you tap.</small></section>`;box.classList.remove('hidden');return true;}
let ps34GuidedSession=null,ps34GuidedQuery='',ps34GuidedFamily=null,ps34RestoredQuery='',ps34WasLocked=false,ps34PositionSignature='';
function ps34ResolutionLocked(){return !!ps34GuidedSession?.searchLock?.locked&&!!ps34GuidedSession?.exactNutritionalIdentity;}
function ps34EnsureGuided(raw,family){if(!GUIDED_PRODUCTS)return null;const query=String(raw||'').trim();if(!ps34GuidedSession||ps34GuidedQuery!==query){ps34GuidedQuery=query;ps34GuidedFamily=family;ps34PositionSignature='';ps34GuidedSession=GUIDED_PRODUCTS.createSession(family.products,query,{intent:family.intent,destination:{date:ext.ui.diaryDate||isoToday(),meal:ext.ui.pendingMeal||''}});}return ps34GuidedSession;}
function ps34StartCatalogueGuide(raw,foods,intent){const entity=intent?.brand||intent?.concept||intent?.product||{name:'Australian product',key:'catalogue'},family={intent,entity,products:foods,primary:foods,broader:[],activeTier:2,catalogue:true};ps34GuidedSession=null;ps34GuidedQuery='';ps34GuidedFamily=family;ps34EnsureGuided(raw,family);ps33RenderBrandFamilyLive(raw,family);ps33RenderBrandFamilySurface(raw,family);if(ps34GuidedSession?.stage===GUIDED_PRODUCTS?.stages?.AMOUNT)ps34FocusAmountEntry();return ps34GuidedSession;}
...
function ps34AnswerSummary(session){const answers=GUIDED_PRODUCTS.answerSummary?.(session)||[];return answers.length?`<div class="guided-resolution-answer-summary" aria-label="Your choices">${answers.map(answer=>`<span><small>${esc(answer.question)}</small><b>${esc(answer.label)}</b></span>`).join('')}</div>`:'';}
function ps34GuidedMarkup(raw,family,{compact=false}={}){
  const session=ps34EnsureGuided(raw,family);if(!session)return '';
  if(session.exactProduct&&!session.addability?.normalLoggingAllowed){const actions=(session.addability?.actions||[]).map(action=>action.id==='nutrition-panel'?`<button type="button" class="secondary" data-review-product-nutrition="${esc(session.exactProduct.id)}">Read Nutrition Panel</button>`:action.id==='barcode'?'<button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button>':action.id==='manual-nutrition'?`<button type="button" class="secondary" data-review-product-nutrition="${esc(session.exactProduct.id)}">Enter Nutrition Manually</button>`:action.id==='generic-alternative'?'<button type="button" class="secondary" data-gpr-generic-alternative>Use a Generic Alternative</button>':action.id==='details'?`<button type="button" class="secondary" data-food-details="${esc(session.exactProduct.id)}">View Details</button>`:'').join('');return `<section class="guided-resolution-card"><small>${esc(session.addability?.label||'Details only')}</small><strong>${esc(session.exactProduct.name)}</strong><p>${esc(session.identityIssue||session.addability?.message||'Complete this product’s nutrition and portion conversion.')}</p><div class="universal-search-handoffs">${actions}</div>${ps34GuidedNav(session)}</section>`;}
  const identityKind=family.intent?.kind==='generic-category'?'Generic food':family.intent?.kind==='consumer-brand'?'Recognised brand':'Product matches',answers=ps34AnswerSummary(session),recognised=`<div class="guided-resolution-understanding"><strong>${esc(identityKind)}: ${esc(family.entity.name)}</strong><small>Product identity is resolved before serving or amount.</small></div>`;
  if(session.stage===GUIDED_PRODUCTS.stages.IDENTITY){const question=session.nextQuestion,branded=session.genericConcept?'<button type="button" class="secondary wide" data-gpr-branded>Browse Branded Products</button>':'';if(session.resolutionState===GUIDED_PRODUCTS.states.INCOMPLETE||!question)return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${recognised}${answers}<div class="guided-resolution-question"><strong>Product identity needs more information</strong><small>A barcode or broad family label is not treated as the exact food you consumed.</small></div>${branded}${ps34GuidedNav(session)}<div class="guided-resolution-fallback"><small>Keep typing, scan for a better source name, read the Nutrition Panel, or complete the product name manually.</small></div></section>`;const choices=(question.options||[]).map(option=>`<button type="button" class="guided-resolution-choice" data-gpr-answer="${esc(question.key)}" data-gpr-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('');return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${recognised}${answers}<div class="guided-resolution-question"><strong>${esc(question?.question||'Which exact product?')}</strong><small>${question.key==='product'?'Choose the real product name. Optional narrowing is used only when it materially helps.':'Only the unresolved attributes needed to identify one nutrition record are asked.'}</small></div><div class="guided-resolution-choices">${choices}</div>${branded}${ps34GuidedNav(session)}<div class="guided-resolution-fallback"><small>Can’t find it? Keep typing, scan its barcode, read the Nutrition Panel, or create it manually.</small></div></section>`;}
  const product=session.exactProduct,identity=`<div class="guided-resolution-exact"><small>${session.identityKind==='generic-reference'?'Exact Australian nutrition reference':'Exact canonical product'}</small><strong>${esc(product?.name||'')}</strong><span>${esc(product?.brand||'')}</span></div>${answers}`;
  if([GUIDED_PRODUCTS.stages.CUSTOMISATION,GUIDED_PRODUCTS.stages.UNSUPPORTED].includes(session.stage)){const unsupported=session.stage===GUIDED_PRODUCTS.stages.UNSUPPORTED;return `<section class="guided-resolution-card">${identity}<div class="guided-resolution-question"><strong>${unsupported?'Customisation unavailable':'Standard or changes?'}</strong></div>${unsupported?'<p>Customisation nutrition is not yet available for this item in this test build. Extras and changes cannot be included.</p>':''}<div class="guided-resolution-choices"><button type="button" class="guided-resolution-choice" data-gpr-customisation="standard">${unsupported?'Go back to Standard':'Standard'}</button>${unsupported?'':'<button type="button" class="guided-resolution-choice" data-gpr-customisation="extras">Add Extras / Changes</button>'}</div>${ps34GuidedNav(session)}</section>`;}
  ...
  if(session.stage===GUIDED_PRODUCTS.stages.AMOUNT){const quick=(GUIDED_PRODUCTS.quickAmounts?.(session)||[]).map(value=>`<button type="button" class="secondary" data-gpr-quick-amount="${value}">${formatNaturalAmount(value)}</button>`).join('');return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${identity}<div class="guided-resolution-question"><strong>${esc(GUIDED_PRODUCTS.amountPrompt?.(session)||'How much?')}</strong><small>${esc(session.selectedMeasure?.label||session.selectedMeasure?.key||'')}</small></div><label class="guided-resolution-amount">Enter the amount<input type="number" inputmode="decimal" enterkeyhint="done" min="0.01" step="any" placeholder="Enter amount" required data-gpr-amount aria-describedby="gpr-amount-help gpr-amount-preview"></label><small id="gpr-amount-help" class="guided-resolution-amount-help">Use a positive number. Decimals are welcome.</small><small id="gpr-amount-preview" class="guided-resolution-amount-help" data-gpr-amount-preview aria-live="polite">Enter an amount to preview nutrition.</small>${quick?`<div class="guided-resolution-quick-amounts" aria-label="Quick fractions">${quick}</div>`:''}<button type="button" class="primary guided-resolution-continue" data-gpr-continue>Continue</button>${ps34GuidedNav(session)}</section>`;}
  return '';
}
...
function ps34PositionActiveCard(){const session=ps34GuidedSession,signature=session?`${session.stage}|${session.nextQuestion?.key||''}|${session.selectedMeasure?.key||''}|${session.transitionRevision||0}`:'';if(!signature||signature===ps34PositionSignature)return;ps34PositionSignature=signature;(window.requestAnimationFrame||((callback)=>callback()))(()=>{const card=document.querySelector?.('#food-results .guided-resolution-card:not(.guided-resolution-compact)')||document.querySelector?.('.guided-resolution-card'),viewport=window.visualViewport,rect=card?.getBoundingClientRect?.(),top=Number(viewport?.offsetTop)||0,bottom=top+(Number(viewport?.height)||window.innerHeight);if(rect&&(rect.top<top+8||rect.bottom>bottom-8))card.scrollIntoView?.({block:'nearest',behavior:'auto'});});}
function ps34RenderGuided(){const raw=by('food-search')?.value.trim()||ps34GuidedQuery,family=ps34GuidedFamily||ps33BrandFamily(raw)||ps34GenericFamily(raw);if(!family)return;ps34SyncGuidedUiState();ps33RenderBrandFamilyLive(raw,family);ps33RenderBrandFamilySurface(raw,family);rc6SyncFoodSearchViewport();ps34PositionActiveCard();}
function ps34FocusAmountEntry(){const schedule=typeof requestAnimationFrame==='function'?requestAnimationFrame:callback=>callback();schedule(()=>{const surfaces=[by('food-live-results'),by('food-results')].filter(Boolean),visible=surfaces.find(surface=>!surface.classList?.contains?.('hidden')),input=visible?.querySelector?.('[data-gpr-amount]')||surfaces.map(surface=>surface.querySelector?.('[data-gpr-amount]')).find(Boolean);if(input){input.value='';input.focus?.({preventScroll:true});input.scrollIntoView?.({block:'nearest'});}});}
function ps34UpdateAmountPreview(input){const target=input?.closest?.('.guided-resolution-card')?.querySelector?.('[data-gpr-amount-preview]'),amount=Number(input?.value),measure=ps34GuidedSession?.selectedMeasure,food=ps34GuidedSession?.exactProduct;if(!target)return;if(!(amount>0)||!measure||!food){target.textContent='Enter an amount to preview nutrition.';return;}const factor=Number(measure.multiplier)*amount,calories=food.nutrients?.calories,energyKj=food.nutrients?.energyKj;target.textContent=hasEnergyValue(calories)?`Nutrition preview · ${energyText(Number(calories)*factor,hasEnergyValue(energyKj)?Number(energyKj)*factor:null)}`:'Nutrition preview will be checked at Review.';}
function ps34SelectGuidedProductChoice(choiceId,answerKey){const session=ps34GuidedSession,question=session?.nextQuestion,key=String(answerKey||question?.key||''),value=String(choiceId||''),valid=session?.stage===GUIDED_PRODUCTS?.stages?.IDENTITY&&question?.key===key&&question.options?.some(option=>String(option.value)===value);if(!valid)return false;GUIDED_PRODUCTS.answerDistinction(session,key,value);if(session.exactNutritionalIdentity&&typeof ss633RetireOwnedError==='function')ss633RetireOwnedError();ps34RenderGuided();if(session.stage===GUIDED_PRODUCTS.stages.AMOUNT)ps34FocusAmountEntry();return true;}
function ps34OpenFinalReview(){const session=ps34GuidedSession;if(!session||session.stage!==GUIDED_PRODUCTS?.stages?.CONFIRMATION||!session.addability?.normalLoggingAllowed)return false;const food=GUIDED_PRODUCTS.reviewFood?.(session)||session.exactProduct;if(!food)return false;prepareEntry(food,{date:session.destination?.date||ext.ui.diaryDate||isoToday(),meal:session.destination?.meal||ext.ui.pendingMeal||'',amount:session.amount,unit:session.selectedMeasure?.key,consumedPortion:session.consumedPortion,portionMeasures:session.servingProfile?.measures||[]});return true;}
function ss633CommitCurrent(reason='enter'){const input=by('food-search'),raw=input?.value.trim()||'',source=rc4SourceOnly(raw)?rc4NamedRestaurantSource(raw,{only:true}):null;if(source){ss633Commit(reason,{kind:'restaurant-source',sourceId:source.id});ext.ui.foodSourceBrowse={sourceId:source.id,label:source.displayName,query:raw,shown:20};ext.ui.foodSearch=raw;input?.blur?.();saveExt();renderLibrary();rc6ScrollSourceHub();return true;}const family=ps33BrandFamily(raw)||ps34GenericFamily(raw);if(!family)return false;ss633Commit(reason,{kind:family.intent?.kind||'resolver'});input?.blur?.();ps34StartCatalogueGuide(raw,family.products,family.intent);return true;}
document.addEventListener('click',event=>{const commit=event.target.closest?.('[data-ps33-commit-family]');if(!commit)return;event.preventDefault();event.stopImmediatePropagation();ss633CommitCurrent('resolver-tap');},true);
by('food-search')?.addEventListener?.('keydown',event=>{if(event.key==='Enter'&&!event.isComposing&&ss633CommitCurrent('enter'))event.preventDefault();});
by('food-search')?.addEventListener?.('search',()=>{if(by('food-search')?.value.trim())ss633CommitCurrent('search-action');});
document.addEventListener('click',event=>{
  const customisation=event.target.closest('[data-gpr-customisation]');if(customisation){event.preventDefault();event.stopPropagation();GUIDED_PRODUCTS.chooseCustomisation(ps34GuidedSession,customisation.dataset.gprCustomisation);ps34RenderGuided();if(ps34GuidedSession.stage===GUIDED_PRODUCTS.stages.AMOUNT)ps34FocusAmountEntry();return;}
  const answer=event.target.closest('[data-gpr-answer]'),measure=event.target.closest('[data-gpr-measure]'),back=event.target.closest('[data-gpr-back]'),cancel=event.target.closest('[data-gpr-cancel]'),branded=event.target.closest('[data-gpr-branded]'),genericAlternative=event.target.closest('[data-gpr-generic-alternative]'),next=event.target.closest('[data-gpr-continue]'),quick=event.target.closest('[data-gpr-quick-amount]'),editAmount=event.target.closest('[data-gpr-edit-amount]'),editMeasure=event.target.closest('[data-gpr-edit-measure]'),editIdentity=event.target.closest('[data-gpr-edit-identity]');if(!answer&&!measure&&!back&&!cancel&&!branded&&!genericAlternative&&!next&&!quick&&!editAmount&&!editMeasure&&!editIdentity)return;
  event.preventDefault();if(!ps34GuidedSession)return;
  if(answer){ps34SelectGuidedProductChoice(answer.dataset.gprValue,answer.dataset.gprAnswer);return;}else if(measure){GUIDED_PRODUCTS.selectMeasure(ps34GuidedSession,measure.dataset.gprMeasure);ps34RenderGuided();ps34FocusAmountEntry();return;}else if(back){ps34GuidedSession=GUIDED_PRODUCTS.back(ps34GuidedSession);if(ps34GuidedSession.restoreSearchRequested){ps34RestoredQuery=ps34GuidedQuery;ps34GuidedSession=null;ps34GuidedFamily=null;ps34SyncGuidedUiState();renderLibrary();return;}}else if(cancel){GUIDED_PRODUCTS.cancel?.(ps34GuidedSession);ps34GuidedSession=null;ps34GuidedQuery='';ps34GuidedFamily=null;ps34RestoredQuery='';ps34PositionSignature='';ps34SyncGuidedUiState();const input=by('food-search');if(input)input.value='';ext.ui.foodSearch='';saveExt();renderLibrary();return;}else if(branded){const query=ps34GuidedQuery;ps34RestoredQuery=query;ps34GuidedSession=null;ps34GuidedFamily=null;ps34SyncGuidedUiState();psLargeSearch(query,{forceBranded:true});return;}else if(genericAlternative){const query=ps34GuidedSession.genericConcept?.name||ps34GuidedSession.exactProduct?.genericName||'';ps34GuidedSession=null;ps34GuidedFamily=null;ps34RestoredQuery='';ps34SyncGuidedUiState();const input=by('food-search');if(input&&query)input.value=query;renderLibrary();if(query)ss633CommitCurrent('generic-alternative');return;}else if(quick){const card=quick.closest('.guided-resolution-card'),input=card?.querySelector('[data-gpr-amount]');if(input){input.value=quick.dataset.gprQuickAmount;input.focus?.();input.dispatchEvent?.(new Event('input',{bubbles:true}));}return;}else if(editAmount)GUIDED_PRODUCTS.editAmount?.(ps34GuidedSession);else if(editMeasure)GUIDED_PRODUCTS.editMeasure?.(ps34GuidedSession);else if(editIdentity){ps34GuidedSession=GUIDED_PRODUCTS.editIdentity?.(ps34GuidedSession)||ps34GuidedSession;if(ps34GuidedSession.restoreSearchRequested){ps34RestoredQuery=ps34GuidedQuery;ps34GuidedSession=null;ps34GuidedFamily=null;ps34SyncGuidedUiState();renderLibrary();return;}}else if(next){const card=next.closest('.guided-resolution-card'),input=card?.querySelector('[data-gpr-amount]'),amount=input?.value;GUIDED_PRODUCTS.selectAmount(ps34GuidedSession,amount);if(ps34GuidedSession.stage===GUIDED_PRODUCTS.stages.AMOUNT){input?.setCustomValidity?.('Enter an amount greater than zero.');input?.reportValidity?.();return;}ps34OpenFinalReview();return;}ps34RenderGuided();
});
window.addEventListener?.('input',event=>{if(event.target?.matches?.('[data-gpr-amount]'))ps34UpdateAmountPreview(event.target);},true);
document.addEventListener('keydown',event=>{if(event.key!=='Enter'||!event.target.matches?.('[data-gpr-amount]'))return;event.preventDefault();event.target.closest('.guided-resolution-card')?.querySelector('[data-gpr-continue]')?.click();});
const ps33ExactProductBase=rc4ExactProduct;rc4ExactProduct=function(raw){if(ps33BrandFamily(raw))return null;const exact=ps33ExactProductBase(raw);if(!exact||C8?.recordType?.(exact)==='online-candidate')return null;const intent=C8.queryIntent(raw),matches=C8.canonicaliseRecords(allFoods().filter(food=>C8.explicitIdentityMatch(food,raw,intent))),model=C8.submittedResultModel(matches,raw),items=model.groups.flatMap(group=>group.items),selected=items.find(item=>item.id===C8.canonicalKey(exact));return selected&&C8.productEligibility(selected.food,{candidates:matches}).identityQuality.exactEligible?selected.food:null;};
const ps33RenderLiveBase=s23RenderLive;s23RenderLive=function(raw){const term=String(raw||'').trim();if(ps34RestoredQuery&&ps34RestoredQuery!==term)ps34RestoredQuery='';const source=rc4SourceOnly(term)?rc4NamedRestaurantSource(term,{only:true}):null,family=ps34RestoredQuery===term?null:ps33BrandFamily(term)||ps34GenericFamily(term);if(ps34GuidedQuery&&ps34GuidedQuery!==term){ps34GuidedSession=null;ps34GuidedQuery='';ps34GuidedFamily=null;ps34SyncGuidedUiState();}if(ps34ResolutionLocked()||ps34GuidedSession&&ps34GuidedQuery===term)ps34RenderGuided();else if(source&&document.activeElement===by('food-search'))ps33PreviewSource(term,source);else if(family&&document.activeElement===by('food-search'))ps33PreviewFamily(term,family);else ps33RenderLiveBase(term);ps33EnforceResultSurfaceOwnership();};renderFoodLiveMatches=s23RenderLive;
...
function ss633LeaveSearch(){
  clearTimeout(searchSession633.stableTimer);searchSession633.stableTimer=0;
  searchSession633.revision++;searchSession633.previewRevision++;searchSession633.ownedAsync=null;
  psLargeSearchToken++;psLargeState=null;au633BrandState=null;
  window.HECRetailerCatalogue?.cancel(retailerBrowse633);retailerBrowse633=null;
  psFederatedSearchState={...C8.newFederatedSearchState(),revision:psFederatedSearchState.revision+1,query:searchSession633.rawQuery};
}
...
function psSearchRevisionCurrent(revision,raw){return (C8?.revisionMatches?.(psFederatedSearchState,revision,raw)??false)&&String(by('food-search')?.value||'')===String(raw||'');}
...
function psCommitLocalResult(result,revision,{append=false,ownerQuery=result?.query}={}){if(!result)return null;const accepted=append?C8?.appendLocalSnapshot?.(psFederatedSearchState,revision,ownerQuery,result.foods):C8?.commitLocalSnapshot?.(psFederatedSearchState,revision,ownerQuery,result.foods,{intent:result.intent,total:result.total});if(!accepted)return null;return {...result,revision,ownerQuery,foods:append?[...(result.foods||[])]:[...(psFederatedSearchState.local||result.foods)]};}
function psLargeRows(result,foods=result.foods){return foods.map(food=>resourceFoodRow(food).replace(/<button class="resource-main"[^>]*>/,`<button class="resource-main" type="button" data-gpr-catalogue-product="${esc(food.id)}" aria-label="Choose ${esc(food.name)}">`)).join('');}
function psLargeRender(result,{append=false}={}){if(ps34GuidedSession)return;if(!psSearchRevisionCurrent(result.revision,result.ownerQuery))return;const target=by('food-results');if(!target)return;const kind=result.intent?.kind,primary=result.foods.filter(food=>food.searchGroup==='primary'),broader=result.foods.filter(food=>food.searchGroup!=='primary'),heading=kind==='consumer-brand'?`Brand · ${result.brand?.name||result.query}`:kind==='generic-category'?`Generic Food · ${result.concept?.name||result.query}`:kind==='exact-product'?'Best Exact Product Match':'Best Australian Product Matches',facetCopy=result.facets?.length?` · ${result.facets.slice(0,4).map(facet=>`${facet.label} (${facet.count})`).join(' · ')}`:'',rows=kind==='generic-category'&&primary.length?`<div class="catalogue-result-group"><strong>Primary category matches</strong>${psLargeRows(result,primary)}</div>${broader.length?`<details><summary>More Australian catalogue results</summary>${psLargeRows(result,broader)}</details>`:''}`:psLargeRows(result),section=`<section class="off-catalogue-results" data-off-results><div class="online-source-banner"><strong>${esc(heading)}</strong><p>${result.total.toLocaleString()} Australian catalogue product${result.total===1?'':'s'} · package size identifies the product, not consumption${esc(facetCopy)}.</p></div><div data-off-rows>${rows}</div>${result.hasMore?`<button type="button" class="secondary wide off-load-more" data-off-load-more="${result.offset+result.foods.length}">Load More Products</button>`:''}</section>`;const existing=target.querySelector('[data-off-results]');if(append&&existing){existing.querySelector('[data-off-rows]')?.insertAdjacentHTML('beforeend',rows);existing.querySelector('[data-off-load-more]')?.remove();if(result.hasMore)existing.insertAdjacentHTML('beforeend',`<button type="button" class="secondary wide off-load-more" data-off-load-more="${result.offset+result.foods.length}">Load More Products</button>`);}else{existing?.remove();target.insertAdjacentHTML('beforeend',section);}}
async function psLargeSearch(raw,{offset=0,append=false,forceBranded=false}={}){const api=window.HECOpenFoodFactsAU,query=String(raw||'').trim(),restaurant=rc5SearchContext(query).source;if(!api||query.length<2||restaurant||(!forceBranded&&ps34GuidedSession))return null;const ownerQuery=searchSession633.rawQuery;if(ownerQuery.trim()!==query)return null;const revision=psSearchRevisionFor(ownerQuery),sessionRevision=searchSession633.revision,token=++psLargeSearchToken;try{const preview=await catalogueSearch633(query,{offset,limit:24,isCurrent:()=>token===psLargeSearchToken&&ss633Current(sessionRevision,ownerQuery)});if(token!==psLargeSearchToken||(!forceBranded&&ps34GuidedSession)||!psSearchRevisionCurrent(revision,ownerQuery)||!ss633Current(sessionRevision,ownerQuery)||(by('food-search')?.value.trim()||'')!==query)return null;if(offset===0&&['consumer-brand','brand-product'].includes(preview.intent?.kind)&&preview.total>0&&preview.total<=500){const complete=await catalogueSearch633(query,{offset:0,limit:preview.total,isCurrent:()=>token===psLargeSearchToken&&ss633Current(sessionRevision,ownerQuery)});if(token!==psLargeSearchToken||(!forceBranded&&ps34GuidedSession)||!psSearchRevisionCurrent(revision,ownerQuery)||!ss633Current(sessionRevision,ownerQuery)||(by('food-search')?.value.trim()||'')!==query)return null;const stable=psCommitLocalResult(complete,revision,{ownerQuery});if(!stable)return null;psLargeState=stable;psLargeRender(stable,{append:false});return stable;}const stable=psCommitLocalResult(preview,revision,{append,ownerQuery});if(!stable)return null;psLargeState=stable;psLargeRender(stable,{append});return stable;}catch(error){console.warn('Australian product catalogue search unavailable',error);return null;}}
function psLargeSchedule(raw){const query=String(raw||'').trim();if(query.length<2||rc5SearchContext(query).source||ps34GuidedSession){psLargeSearchToken++;return;}const revision=searchSession633.revision,ownerQuery=searchSession633.rawQuery,committedRevision=searchSession633.committedRevision;setTimeout(()=>{if(committedRevision===searchSession633.committedRevision&&ss633Current(revision,ownerQuery)&&(by('food-search')?.value.trim()||'')===query&&!ps34GuidedSession)psLargeSearch(query);},40);}
...
document.addEventListener('click',event=>{const choice=event.target.closest('[data-gpr-catalogue-product]');if(!choice)return;const food=getFood(choice.dataset.gprCatalogueProduct);if(!food)return;event.preventDefault();event.stopPropagation();ps34StartCatalogueGuide(by('food-search')?.value.trim()||food.name,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});},true);
const ps33RenderLibraryBase=renderLibrary;renderLibrary=function(){ps33RenderLibraryBase();const input=by('food-search'),raw=input?.value.trim()||'',source=rc4SourceOnly(raw)?rc4NamedRestaurantSource(raw,{only:true}):null,family=ps34RestoredQuery===raw?null:ps33BrandFamily(raw)||ps34GenericFamily(raw);if(ps34GuidedSession&&ps34GuidedQuery===raw)ps34RenderGuided();else if(document.activeElement===input&&source)ps33PreviewSource(raw,source);else if(document.activeElement===input&&family)ps33PreviewFamily(raw,family);else if(psLargeState?.query===raw)psLargeRender(psLargeState);psLargeSchedule(raw);ps33EnforceResultSurfaceOwnership();rc6SyncFoodSearchViewport();};
by('food-search')?.addEventListener('blur',()=>setTimeout(()=>{if(q('#food-library.active')&&by('food-search')?.value.trim())renderLibrary();},0));

/* Universal deliberate submission owns one immutable result surface. Typing may
   preview, but it cannot select a food, source, serving or destination. */
function us633ResultRow(item){const food=item.food||{},decision=item.addability||C8?.addability?.(food)||{status:C8?.canLog?.(food)?'loggable-now':'details-only',label:'Details only'},status=decision.status==='loggable-now'?'Loggable now · choose an amount':decision.label||'Details only',parts=C8?.provenanceParts?.(food)||[food.brand,item.provenance?.label].filter(Boolean),meta=[...parts,...(food.brandAdmission?[food.sourceBrands!==food.brand?food.sourceBrands:null,food.packageSize?'Pack: '+food.packageSize:null,food.barcode?'Barcode '+food.barcode:null].filter(Boolean):[]),cleanMeasureText(food.serving?`Source reference: ${food.serving}`:''),status].filter(Boolean).join(' · '),action=decision.status==='loggable-now'?'Choose':decision.status==='needs-nutrition-completion'?'Complete':'Details';return `<button type="button" class="universal-search-result" data-universal-result="${esc(item.recordId||item.id)}"><span><strong>${esc(item.name||food.name||'Food')}</strong><small>${esc(meta)}</small></span><b>${action}</b></button>`;}
function us633SubmittedMarkup(model){if(!model?.groups?.length)return `<section class="universal-search-empty"><strong>No reliable match is ready to select.</strong><p>HEC will not guess nutrition. Scan the barcode, read the Nutrition Panel, or create the food.</p><div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button><button type="button" class="secondary" data-open-feature="custom-food">Create Food</button></div></section>`;return `<section class="universal-search-submitted" aria-label="Submitted food search results"><header><strong>Results for “${esc(model.rawQuery)}”</strong><small>Choose a result to continue</small></header>${model.groups.map(group=>`<section class="universal-search-group" data-universal-group="${esc(group.key)}"><h3>${esc(group.label)}</h3>${group.items.slice(0,24).map(item=>item.kind==='generic-concept'?`<button type="button" class="universal-search-result universal-generic-result" data-universal-generic="${esc(item.conceptKey)}"><span><strong>${esc(item.name)}</strong><small>Generic food · choose where it came from</small></span><b>Choose</b></button>`:us633ResultRow(item)).join('')}</section>`).join('')}</section>`;}
function us633AppendOnlineGroup(){const target=by('food-results'),surface=target?.querySelector?.('.universal-search-submitted'),group=searchSession633.submittedModel?.groups?.find(item=>item.key==='online');if(!surface||!group||searchSession633.sourceContext)return false;const markup=`<section class="universal-search-group" data-universal-group="online"><h3>${esc(group.label)}</h3>${group.items.slice(0,24).map(us633ResultRow).join('')}</section>`,existing=surface.querySelector('[data-universal-group="online"]');if(existing)existing.outerHTML=markup;else surface.insertAdjacentHTML('beforeend',markup);return true;}
function us633AppendPackagedRecords(records=[]){const target=by('food-results'),surface=target?.querySelector?.('.universal-source-results');if(!surface||searchSession633.sourceContext?.sourceKey!=='packaged-frozen')return false;const seen=new Set([...surface.querySelectorAll?.('[data-universal-result]')||[]].map(node=>String(node.dataset.universalResult||''))),foods=(records||[]).filter(food=>['packaged','external-catalogue','online-candidate'].includes(C8?.recordType?.(food))&&!seen.has(String(food.id||''))).slice(0,24);if(!foods.length)return false;const rows=foods.map(food=>us633ResultRow({recordId:food.id,name:food.name,food,provenance:C8?.provenance?.(food)})).join(''),existing=surface.querySelector?.('[data-universal-packaged-online]');if(existing)existing.insertAdjacentHTML('beforeend',rows);else surface.querySelector?.('.universal-search-handoffs')?.insertAdjacentHTML('beforebegin',`<section class="universal-search-group" data-universal-packaged-online><h3>Australian Packaged Catalogue Results</h3>${rows}</section>`);return true;}
function us633RenderSubmitted(){const model=searchSession633.submittedModel,target=by('food-results'),live=by('food-live-results');if(!model||!target)return false;target.className='resource-list universal-search-owned';target.innerHTML=us633SubmittedMarkup(model);target.classList.remove('result-surface-inactive');target.removeAttribute('aria-hidden');if(live){live.innerHTML='';live.classList.add('hidden','result-surface-inactive');live.setAttribute('aria-hidden','true');}return true;}
function us633SourceMarkup(item){const choices=item?.sourcePlan?.choices||S23?.sourceContextPlan?.(S23?.concepts?.find(concept=>concept.key===item?.conceptKey),searchSession633.rawQuery)?.choices||[];return `<section class="universal-source-branch"><header><button type="button" class="secondary" data-universal-back-results>← Results</button><span><strong>${esc(item?.name||'Generic food')}</strong><small>Where did it come from?</small></span></header><div class="universal-source-choices">${choices.map(choice=>`<button type="button" class="guided-resolution-choice" data-universal-source="${esc(choice.key)}"><span><strong>${esc(choice.label)}</strong><small>${choice.key==='home-prepared'?'Recipe or home-prepared record':choice.key==='ready-to-eat'?'Verified loaded restaurant menus':choice.key==='packaged-frozen'?'Brand, barcode or Nutrition Panel':'Use a safe generic record when available'}</small></span><b>›</b></button>`).join('')}</div></section>`;}
function us633OpenGeneric(conceptKey){const generic=searchSession633.submittedModel?.groups?.flatMap(group=>group.items).find(item=>item.kind==='generic-concept'&&item.conceptKey===conceptKey);if(!generic)return false;searchSession633.selectedResult=generic;const choices=generic.sourcePlan?.choices||[],explicit=generic.sourcePlan?.explicitSource||'',direct=!choices.length?'typical':({home:'home-prepared',restaurant:'ready-to-eat',commercial:'packaged-frozen',bakery:'typical'}[explicit]||'');if(direct){searchSession633.sourceContext={conceptKey,sourceKey:direct,stage:'source-results',direct:true};return us633OpenSource(direct);}searchSession633.sourceContext={conceptKey,stage:'choose-source'};const target=by('food-results');if(target)target.innerHTML=us633SourceMarkup(generic);return true;}
function us633SourceBack(){return searchSession633.sourceContext?.direct?'<button type="button" class="secondary" data-universal-back-results>← Results</button>':'<button type="button" class="secondary" data-universal-back-source>← Source choices</button>';}
function us633SafeHandoff(title,copy,{recipe=false,packaged=false}={}){const target=by('food-results');if(!target)return;target.innerHTML=`<section class="universal-search-empty">${us633SourceBack()}<strong>${esc(title)}</strong><p>${esc(copy)}</p><div class="universal-search-handoffs">${recipe?'<button type="button" class="secondary" data-open-feature="recipe-builder">Build From Ingredients</button>':''}${packaged?'<button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button>':''}<button type="button" class="secondary" data-open-feature="custom-food">Create Food</button></div></section>`;}
function us633SourceRecords(sourceKey){const query=searchSession633.submittedModel?.identityQuery||searchSession633.rawQuery,ranked=allFoods().map(food=>({food,rank:C8?.rank?.(food,query)?.score||0})).filter(item=>item.rank>0).sort((a,b)=>b.rank-a.rank||a.food.name.localeCompare(b.food.name));if(sourceKey==='ready-to-eat')return ranked.filter(item=>item.food.foodSourceId&&['mcdonalds-au','kfc-au'].includes(item.food.foodSourceId)&&item.food.verified!==false).map(item=>item.food);if(sourceKey==='packaged-frozen')return ranked.filter(item=>['packaged','external-catalogue','online-candidate'].includes(C8?.recordType?.(item.food))).map(item=>item.food);return ranked.filter(item=>C8?.recordType?.(item.food)==='afcd').map(item=>item.food);}
function us633OpenSource(sourceKey){const target=by('food-results'),item=searchSession633.selectedResult,query=searchSession633.rawQuery;if(!target||!item)return false;searchSession633.sourceContext={conceptKey:item.conceptKey,sourceKey,stage:'source-results',direct:!!searchSession633.sourceContext?.direct};const family=ps34GenericFamily(query);if(sourceKey==='home-prepared'){us633SafeHandoff('Home-Prepared',family?'Choose Not Sure / Typical to use the Australian generic resolver, or build the home recipe from its real ingredients.':'No complete home-prepared nutrition record is available for this concept. Build it from ingredients or create a reviewed food; HEC will not invent nutrition.',{recipe:true});return true;}if(sourceKey==='packaged-frozen'){const foods=us633SourceRecords(sourceKey);target.innerHTML=`<section class="universal-source-results">${us633SourceBack()}<h3>Purchased Packaged / Frozen</h3><p>Choose the exact loaded product, or use its barcode or Nutrition Panel.</p>${foods.slice(0,24).map(food=>us633ResultRow({recordId:food.id,name:food.name,food,provenance:C8?.provenance?.(food)})).join('')}<div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button></div></section>`;psLargeSearch(query,{forceBranded:true});return true;}const foods=us633SourceRecords(sourceKey);if(sourceKey==='typical'&&family){ps34StartCatalogueGuide(query,family.products,family.intent);return true;}if(!foods.length){us633SafeHandoff(sourceKey==='ready-to-eat'?'Restaurant / Ready-to-Eat':'Not Sure / Typical','No reliable loaded Australian nutrition record matches this source and food. Try another source, scan a package, or create a reviewed food.',{packaged:true});return true;}target.innerHTML=`<section class="universal-source-results">${us633SourceBack()}<h3>${sourceKey==='ready-to-eat'?'Restaurant / Ready-to-Eat':'Not Sure / Typical'}</h3><p>${sourceKey==='ready-to-eat'?'Only currently verified loaded restaurant sources are shown.':'Choose the closest reliable Australian generic record.'}</p>${foods.slice(0,24).map(food=>us633ResultRow({recordId:food.id,name:food.name,food,provenance:C8?.provenance?.(food)})).join('')}</section>`;return true;}
const us633CommitCurrentBase=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){const input=by('food-search'),raw=input?.value.trim()||'';if(!raw)return false;const parsed=S23?.parseQuery?.(raw),concept=S23?.conceptFromQuery?.(parsed),bare=!!concept&&(concept.aliases||[]).some(alias=>S23?.singular?.(alias)===S23?.singular?.(parsed?.food));if(reason==='resolver-tap'||!bare&&(rc4SourceOnly(raw)||ps33BrandFamily(raw)||ps34GenericFamily(raw)))return us633CommitCurrentBase(reason);ss633Commit(reason,{kind:'universal-search'});const loadedCatalogue=psLargeState&&psSearchRevisionCurrent(psLargeState.revision,psLargeState.ownerQuery)?psLargeState.foods||[]:[],submittedExternal=[...(psFederatedSearchState?.online||[]),...loadedCatalogue];searchSession633.quantity=S23?.parseQuantityLanguage?.(raw,{candidates:allFoods()})||null;searchSession633.submittedModel=C8?.submittedResultModel?.(allFoods(),raw,{savedIds:ext.savedFoodIds||[],online:submittedExternal})||null;searchSession633.ownedAsync={previewRevision:searchSession633.previewRevision,committedRevision:searchSession633.committedRevision,query:searchSession633.rawQuery};searchSession633.sourceContext=null;searchSession633.selectedResult=null;ext.ui.foodSearch=raw;saveExt();input?.blur?.();us633RenderSubmitted();return true;};
const us633BeginTypingBase=ss633BeginTyping;ss633BeginTyping=function(input){const state=us633BeginTypingBase(input),raw=state.rawQuery||'';state.suppliedSourceWords=S23?.sourceModeFromQuery?.(raw)||'';state.suppliedPreparationWords=S23?.classifyText?.(raw)?.prep||'';state.destination={date:ext.ui.diaryDate||isoToday(),meal:ext.ui.pendingMeal||''};return state;};
const us633CommitCurrentWithContext=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){const input=by('food-search');if(input&&String(input.value||'')!==searchSession633.rawQuery)ss633BeginTyping(input);const state=us633CommitCurrentWithContext(reason);if(state){searchSession633.suppliedSourceWords=S23?.sourceModeFromQuery?.(searchSession633.rawQuery)||'';searchSession633.suppliedPreparationWords=S23?.classifyText?.(searchSession633.rawQuery)?.prep||'';searchSession633.destination={date:ext.ui.diaryDate||isoToday(),meal:ext.ui.pendingMeal||''};}return state;};
by('food-search-form')?.addEventListener('submit',event=>{event.preventDefault();ss633CommitCurrent('search-button');});
function us633UniversalControl(target){return target?.closest?.('[data-universal-generic],[data-universal-source],[data-universal-back-results],[data-universal-back-source],[data-universal-result],[data-fc-answer],[data-fc-back],[data-fc-cancel],[data-fc-base]')||null;}
function us633ActivateControl(control){if(!control||!control.isConnected)return false;if(control.hasAttribute('data-universal-generic'))return us633OpenGeneric(control.dataset.universalGeneric);if(control.hasAttribute('data-universal-source'))return us633OpenSource(control.dataset.universalSource);if(control.hasAttribute('data-universal-back-results')){searchSession633.sourceContext=null;searchSession633.selectedResult=null;return us633RenderSubmitted();}if(control.hasAttribute('data-universal-back-source'))return us633OpenGeneric(searchSession633.sourceContext?.conceptKey);if(control.hasAttribute('data-universal-result')){const recordId=control.dataset.universalResult,model=au633BrandState&&au633BrandQueryCurrent(au633BrandState)&&searchSession633.mode!=='explicit-committed'?au633BrandState.previewModel:searchSession633.submittedModel,food=model?.groups?.flatMap(group=>group.items||[]).find(item=>item.recordId===recordId)?.food||au633BrandState?.records.find(food=>food.id===recordId)||getFood(recordId)||window.HECOpenFoodFactsAU?.getLoaded?.(recordId);if(!food)return false;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(searchSession633.rawQuery||food.name,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}return false;}
let us633PendingPointer=null,us633ConsumedPointerControl=null;
window.addEventListener('pointerdown',event=>{const control=us633UniversalControl(event.target);if(!control||event.button!==0)return;us633PendingPointer={control,pointerId:event.pointerId,x:event.clientX,y:event.clientY};},true);
window.addEventListener('pointerup',event=>{const pending=us633PendingPointer;us633PendingPointer=null;if(!pending||pending.pointerId!==event.pointerId||Math.hypot(event.clientX-pending.x,event.clientY-pending.y)>12)return;event.preventDefault();event.stopImmediatePropagation();us633ConsumedPointerControl=pending.control;us633ActivateControl(pending.control);},true);
window.addEventListener('pointercancel',()=>{us633PendingPointer=null;},true);
window.addEventListener('click',event=>{const control=us633UniversalControl(event.target);if(!control)return;event.preventDefault();event.stopImmediatePropagation();if(control===us633ConsumedPointerControl){us633ConsumedPointerControl=null;return;}us633ConsumedPointerControl=null;us633ActivateControl(control);},true);
const us633RenderLibraryBase=renderLibrary;renderLibrary=function(){const owns=searchSession633.mode==='explicit-committed'&&searchSession633.submittedModel&&!ps34GuidedSession&&!ext.ui.foodSourceBrowse,stable=owns&&by('food-results')?.querySelector?.('.universal-search-submitted,.universal-source-branch,.universal-source-results,.universal-search-empty');if(stable)return;us633RenderLibraryBase();if(owns){if(searchSession633.sourceContext?.stage==='choose-source')us633OpenGeneric(searchSession633.sourceContext.conceptKey);else if(searchSession633.sourceContext?.stage==='source-results')us633OpenSource(searchSession633.sourceContext.sourceKey);else us633RenderSubmitted();}};
const us633LargeRenderBase=psLargeRender;psLargeRender=function(result,options={}){if(searchSession633.mode==='explicit-committed'&&searchSession633.submittedModel)return;return us633LargeRenderBase(result,options);};
const us633LargeSearchBase=psLargeSearch;psLargeSearch=async function(raw,options={}){const ticket={...(searchSession633.ownedAsync||{})},result=await us633LargeSearchBase(raw,options);if(result&&searchSession633.submittedModel&&C8?.ownsUniversalAsync?.(searchSession633,ticket)){if(searchSession633.sourceContext?.sourceKey==='packaged-frozen')us633AppendPackagedRecords(result.foods||[]);else{searchSession633.submittedModel=C8.appendSubmittedOnline(searchSession633.submittedModel,result.foods||[]);us633AppendOnlineGroup();}}return result;};

/* Submitted search shows a shortlist; selecting its base owns identity narrowing.
   The existing guided controller remains the measure/amount/Review authority. */
let fc633Session=null,fc633Revision=0;
// A deliberate preview Review of a configurable product uses the same entry.
const fc633FullReviewBase=rc6OpenFullReview;rc6OpenFullReview=function(food){
  if(GUIDED_PRODUCTS.customisationProfile(food)&&q('#food-library.active')){const raw=by('food-search')?.value.trim()||food.name;fc633Session=null;fc633Revision++;ss633Commit('product-tap',{kind:'exact-product'});by('food-search')?.blur?.();ps34StartCatalogueGuide(raw,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return {opened:true,stage:GUIDED_PRODUCTS.stages.CUSTOMISATION};}
  return fc633FullReviewBase(food);
};
function fc633RenderShortlist(){
  const session=fc633Session,target=by('food-results');if(!session||!target)return false;
  const signature=JSON.stringify([session.known,session.records.length,session.loading,'shortlist']);if(session.renderSignature===signature&&target.querySelector('[data-fc-base]'))return true;session.renderSignature=signature;
  const model=C8.conceptShortlist(session.records,session);searchSession633.submittedModel=model;session.question=null;
  us633RenderSubmitted();const base=target.querySelector('[data-universal-generic]');if(base){base.removeAttribute('data-universal-generic');base.setAttribute('data-fc-base','');base.querySelector('small').textContent='Choose this to narrow down your food';}
  if(session.loading)target.insertAdjacentHTML('beforeend','<small role="status">Checking the Australian packaged catalogue…</small>');return true;
}
async function fc633StartExplicit(raw,intent,reason){
  fc633Session=null;const revision=++fc633Revision,ownerRevision=searchSession633.revision,ownerQuery=searchSession633.rawQuery;ps34GuidedSession=null;ps34GuidedFamily=null;ps34SyncGuidedUiState();ss633Commit(reason,intent);searchSession633.sourceContext=null;searchSession633.selectedResult=null;ext.ui.foodSearch=raw;saveExt();by('food-search')?.blur?.();
  const identity=REG29.preserveSearchSpelling(raw,intent.quantity?.identityQuery||intent.identityQuery||raw),sourceIntent=C8.queryIntent(identity),entity=sourceIntent.entity,records=allFoods();
  const waveQuery=window.HECBrandCatalogue?.recognise(identity);
  const render=()=>{const matches=C8.canonicaliseRecords(records.filter(food=>C8.explicitIdentityMatch(food,identity,sourceIntent,intent)&&(!waveQuery||window.HECBrandCatalogue.ordinaryChoice(food))));const model=C8.submittedResultModel(matches,raw);searchSession633.submittedModel={...model,groups:model.groups.filter(group=>group.key!=='generic')};us633RenderSubmitted();if(!matches.length)by('food-results').innerHTML='<section class="universal-search-empty" role="status"><p>This item isn’t currently in HEC’s verified catalogue.</p><p>Try another search, or cancel.</p></section>';};
  if(entity?.type==='restaurant'){render();return true;}
  by('food-results').innerHTML='<section class="universal-search-empty" role="status">Checking the Australian catalogue…</section>';
  searchSession633.submittedModel={rawQuery:raw,groups:[],total:0};
  try{const result=await catalogueSearch633(identity,{limit:500,isCurrent:()=>revision===fc633Revision&&ss633Current(ownerRevision,ownerQuery)});if(revision!==fc633Revision||!ss633Current(ownerRevision,ownerQuery))return false;records.push(...(result?.foods||[]));render();}catch(error){if(revision===fc633Revision&&ss633Current(ownerRevision,ownerQuery))by('food-results').innerHTML='<section class="universal-search-empty" role="status">The catalogue could not be checked. Please try Search again.</section>';}
  return true;
}
function fc633Candidates(){
  const session=fc633Session;if(!session)return [];
  return session.records.filter(food=>S23.conceptCompatibility(food,session).compatible).filter(food=>{
    const type=C8.recordType(food),reference=type==='afcd',source=session.known.sourceContext;
    if(session.known.breadOrigin==='home'&&!reference)return false;
    if(session.known.breadSource&&C8.commercialSourceClass(food)!==session.known.breadSource)return false;
    if(session.known.identitySource==='generic'&&!reference)return false;
    if(session.known.identitySource==='products'&&reference)return false;
    if(source==='ready-to-eat')return C8.productEligibility(food).restaurantEligible;
    if(source==='packaged-frozen')return ['packaged','external-catalogue','online-candidate','local'].includes(type)&&!food.foodSourceId;
    if(source==='typical'||source==='home-prepared')return reference;
    return true;
  });
}
function fc633Snapshot(){
  if(!fc633Session)return null;const session=fc633Session;
  return {conceptId:session.conceptId,intent:session.kind,known:{...session.known},question:session.question?.question||'',nextFacet:session.question?.key||'',reason:session.question?.reason||'specific-identity-choice',stage:session.stage,loading:session.loading,catalogueTotals:session.catalogueTotals||[],catalogueError:session.catalogueError||'',quantity:session.quantity,candidateCount:fc633Candidates().length,excluded:session.records.filter(food=>!S23.conceptCompatibility(food,session).compatible).map(food=>({id:food.id,...S23.conceptCompatibility(food,session)}))};
}
function fc633Render(){
  if(retailerBrowse633&&retailerCurrent633(retailerBrowse633)&&!ps34GuidedSession)return retailerRender633(retailerBrowse633);
  if(au633BrandState?.returnToConcept&&au633BrandQueryCurrent(au633BrandState)&&!ps34GuidedSession)return au633RenderBrand(au633BrandState);
  const session=fc633Session,target=by('food-results');if(!session||!target||ps34GuidedSession)return false;
  if(session.stage==='shortlist')return fc633RenderShortlist();
  const definition=S23.foodConceptRegistry[session.conceptId],candidates=fc633Candidates();
  session.question=window.HECRetailerCatalogue.conceptSourceQuestion(session)||S23.nextConceptQuestion(session,candidates);session.stage='identity';
  if(!session.question&&!session.loading&&candidates.length===1&&C8.recordType(candidates[0])==='afcd'&&session.history.length){const food=candidates[0];session.stage='resolved';session.selectedId=food.id;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(session.rawQuery,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}
  const signature=JSON.stringify([session.known,session.records.length,session.loading]);if(session.renderSignature===signature&&target.querySelector('.food-concept-resolution'))return true;session.renderSignature=signature;
  const answers=session.history.map(item=>`<span>${esc(item.label)}</span>`).join(''),nav='<div class="guided-resolution-nav"><button type="button" class="secondary" data-fc-back>Back</button><button type="button" class="secondary" data-fc-cancel>Cancel</button></div>';
  let content='';
  if(session.known.sourceContext==='home-prepared'){session.question=null;content='<p>Build the home recipe from its ingredients, or choose Typical for an available Australian reference.</p><button type="button" class="secondary" data-open-feature="recipe-builder">Build From Ingredients</button>';}
  else if(session.question){const question=session.question;content=`<div class="food-concept-question"><strong>${esc(question.question)}</strong></div>${!question.options.length?'<p>No supported retailer products have been added for this food yet.</p>':''}<div class="guided-resolution-choices">${question.options.map(option=>`<button type="button" class="guided-resolution-choice" data-fc-answer="${esc(question.key)}" data-fc-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('')}</div>`;}
  else if(candidates.length){
    // Source/brand groups contain every compatible loaded identity. Neither a
    // lexical rank nor a display limit can silently remove another source.
    const groups=new Map();for(const food of candidates){const display=C8.recordType(food)==='afcd'?'Generic Australian reference':food.sourceDisplayName||food.brand||'Brand not listed',label=[...groups.keys()].find(key=>C8.brandKey(key)===C8.brandKey(display))||display,group=groups.get(label)||[];group.push(food);groups.set(label,group);}
    const ordered=[...groups].sort(([a],[b])=>a==='Generic Australian reference'?-1:b==='Generic Australian reference'?1:a.localeCompare(b));
    content=`<div class="food-concept-question"><strong>Which ${esc(definition.label.toLowerCase())} did you have?</strong></div>${ordered.map(([label,foods])=>`<section class="food-concept-product-group"><h3>${esc(label)}</h3>${foods.sort((a,b)=>S23.semanticProductExactness(b,session.rawQuery).priority-S23.semanticProductExactness(a,session.rawQuery).priority||a.name.length-b.name.length||a.name.localeCompare(b.name)).map(food=>us633ResultRow({recordId:food.id,name:food.name,food})).join('')}</section>`).join('')}`;
  }else content=`<p>${session.loading?'Finding supported products…':'No reliable loaded Australian nutrition record matches this food and source. Choose another option, scan the package or enter the food.'}</p><div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-feature="custom-food">Create Food</button></div>`;
  if(session.known.sourceContext==='packaged-frozen')content+='<div class="universal-search-handoffs"><button type="button" class="secondary" data-open-scan-mode="barcode">Scan Barcode</button><button type="button" class="secondary" data-open-scan-mode="label">Read Nutrition Panel</button></div>';
  target.className='resource-list universal-search-owned';target.innerHTML=`<section class="food-concept-resolution guided-resolution-card" data-food-concept="${esc(session.conceptId)}"><header><strong>${esc(definition.label)}</strong></header><div class="guided-resolution-answer-summary">${answers}</div>${content}${session.loading?'<small role="status">Checking the Australian packaged catalogue…</small>':''}${nav}</section>`;
  target.classList.remove('result-surface-inactive');target.removeAttribute('aria-hidden');const live=by('food-live-results');if(live){live.innerHTML='';live.classList.add('hidden','result-surface-inactive');live.setAttribute('aria-hidden','true');}
  const position=()=>{if(fc633Session!==session||ps34GuidedSession)return;const card=target.querySelector('.food-concept-resolution'),rect=card?.getBoundingClientRect(),viewport=window.visualViewport,top=Number(viewport?.offsetTop)||0,bottom=top+(Number(viewport?.height)||innerHeight);if(rect&&(rect.top<top+8||rect.bottom>bottom-8))card.scrollIntoView({block:'nearest',behavior:'auto'});};position();requestAnimationFrame(()=>requestAnimationFrame(position));return true;
}
async function fc633Enrich(session,revision){
  const external=window.HECOpenFoodFactsAU;if(!external?.search){session.loading=false;fc633Render();return;}
  try{
    const queries=S23.conceptSearchQueries(session),firstFacet=S23.conceptFacets[S23.foodConceptRegistry[session.conceptId].facets[0]];
    // Query each missing source/type from declarative facet metadata. This finds
    // supported minority types without downloading the whole product universe.
    if(S23.foodConceptRegistry[session.conceptId].facets[0]==='milkSource')for(const [label] of firstFacet.rules)if(!session.records.some(food=>S23.conceptAttributes(food,session.conceptId).milkSource===label))queries.push(`${label} ${S23.foodConceptRegistry[session.conceptId].label}`);
    const results=await Promise.all(queries.map(query=>catalogueSearch633(query,{limit:500,isCurrent:()=>fc633Session===session&&revision===fc633Revision&&ss633Current(session.ownerRevision,session.ownerQuery)})));
    if(fc633Session!==session||revision!==fc633Revision||!ss633Current(session.ownerRevision,session.ownerQuery))return;
    const seen=new Set(session.records.map(food=>food.id));for(const result of results)for(const food of result.foods||[])if(!seen.has(food.id)&&(S23.conceptCompatibility(food,session.conceptId).compatible||(S23.foodConceptRegistry[session.conceptId].shortlist?.related||[]).includes(S23.foodConceptEvidence(food).conceptId))){seen.add(food.id);session.records.push(food);}
    session.records=C8.canonicaliseRecords(session.records);rememberCanonicalFoods(session.records);
    session.catalogueTotals=results.map((result,index)=>({query:queries[index],total:result.total,loaded:result.foods.length,hasMore:result.hasMore}));
  }catch(error){if(fc633Session===session&&ss633Current(session.ownerRevision,session.ownerQuery))session.catalogueError=String(error?.message||error);}
  finally{if(fc633Session===session&&revision===fc633Revision&&ss633Current(session.ownerRevision,session.ownerQuery)){session.loading=false;fc633Render();}}
}
function fc633Start(raw,intent,reason){
  fc633Revision++;ps34GuidedSession=null;ps34GuidedFamily=null;ps34GuidedQuery='';ps34SyncGuidedUiState();ss633Commit(reason,intent);
  fc633Session={...intent,ownerRevision:searchSession633.revision,ownerQuery:searchSession633.rawQuery,known:{...intent.known},records:allFoods(),history:[],loading:true,question:null,stage:'shortlist'};
  searchSession633.sourceContext=null;searchSession633.selectedResult=null;
  searchSession633.submittedModel={rawQuery:raw,identityQuery:intent.identityQuery,conceptIntent:intent,groups:[],total:0};
  ext.ui.foodSearch=raw;saveExt();by('food-search')?.blur?.();fc633Render();void fc633Enrich(fc633Session,fc633Revision);return true;
}
const fc633CommitBase=ss633CommitCurrent;ss633CommitCurrent=function(reason='enter'){
  const input=by('food-search'),raw=input?.value.trim()||'';if(!raw)return false;if(String(input.value||'')!==searchSession633.rawQuery)ss633BeginTyping(input);
  if(C8.queryIntent(raw).kind==='brand-family')return au633SubmitBrand(raw,reason);
  if(C8.queryIntent(raw).kind==='retailer')return retailerSubmit633(raw,reason);
  const intent=S23.interpretFoodIntent(raw,{records:allFoods(),sourceIntent:C8.queryIntent(raw)});
  if(C8.isExplicitProductRequest(raw,intent))return fc633StartExplicit(raw,intent,reason);
  if(intent.generic){if(String(by('food-search')?.value||'')!==searchSession633.rawQuery)ss633BeginTyping(input);return fc633Start(raw,intent,reason);}
  fc633Session=null;fc633Revision++;return fc633CommitBase(reason);
};
const fc633TypingBase=ss633BeginTyping;ss633BeginTyping=function(input){if(String(input?.value||'')!==searchSession633.rawQuery){fc633Session=null;fc633Revision++;window.HECRetailerCatalogue?.cancel(retailerBrowse633);retailerBrowse633=null;}return fc633TypingBase(input);};
const fc633LibraryBase=renderLibrary;renderLibrary=function(){if(fc633Session&&!ps34GuidedSession&&ss633Current(fc633Session.ownerRevision,fc633Session.ownerQuery)){fc633Render();return;}return fc633LibraryBase();};
function fc633Activate(control){
  if(!fc633Session||!control||!control.isConnected)return false;
  if(control.hasAttribute('data-fc-cancel')){fc633Session=null;fc633Revision++;const input=by('food-search');if(input){input.value='';ss633BeginTyping(input);}ext.ui.foodSearch='';saveExt();renderLibrary();return true;}
  if(control.hasAttribute('data-fc-base')){fc633Session.stage='identity';fc633Session.renderSignature='';fc633Render();return true;}
  if(control.hasAttribute('data-fc-back')){const previous=fc633Session.history.pop();if(previous)fc633Session.known=previous.before;else fc633Session.stage='shortlist';fc633Session.renderSignature='';fc633Render();return true;}
  const key=control.dataset.fcAnswer,value=control.dataset.fcValue;if(!key)return false;if(fc633Session.known[key]===value)return true;
  if(key==='retailerIdentity'&&fc633Session.question?.options.some(option=>option.value===value)){retailerOpen633(value,{scope:'commercial-identity',conceptId:fc633Session.conceptId});return true;}
  if(key==='brandIdentity'&&fc633Session.question?.options.some(option=>option.value===value)){const brand=window.HECBrandCatalogue.brands.get(value);au633LoadBrand(brand.name,{conceptId:fc633Session.conceptId,returnToConcept:true});return true;}
  fc633Session.history.push({key,value,label:control.querySelector('span')?.innerText||value,before:{...fc633Session.known}});fc633Session.known[key]=value;if(key==='catalogueOrigin')fc633Session.known.identitySource=value==='commercial'?'products':'generic';fc633Render();return true;
}
const fc633ActivateBase=us633ActivateControl;us633ActivateControl=function(control){if(au633BrandState?.returnToConcept&&au633BrandQueryCurrent(au633BrandState)&&control?.hasAttribute("data-universal-result"))return fc633ActivateBase(control);if(retailerBrowse633&&retailerCurrent633(retailerBrowse633)&&control?.hasAttribute('data-universal-result'))return fc633ActivateBase(control);if(fc633Session&&control?.matches('[data-fc-answer],[data-fc-back],[data-fc-cancel],[data-fc-base]'))return fc633Activate(control);if(fc633Session&&control?.hasAttribute('data-universal-result')){const food=(fc633Session.stage==='shortlist'?fc633Session.records:fc633Candidates()).find(item=>item.id===control.dataset.universalResult);if(!food)return false;fc633Session.stage='resolved';fc633Session.selectedId=food.id;searchSession633.selectedResult={kind:'exact-product',recordId:food.id};ps34StartCatalogueGuide(fc633Session.rawQuery,[food],{kind:'exact-product',product:{id:food.id,name:food.name}});return true;}return fc633ActivateBase(control);};
window.HEC_FOOD_CONCEPT_TEST={loading:()=>!!fc633Session?.loading,state:fc633Snapshot,candidates:()=>fc633Candidates().map(food=>({id:food.id,name:food.name,brand:food.brand,source:food.foodSourceId,concept:S23.foodConceptEvidence(food),attributes:S23.conceptAttributes(food,fc633Session?.conceptId)}))};

// Indexed brand discovery owns its query revision. Hydrated product records may
...
function au633BrandModel(state,{preview=false}={}){
  if(!state.wave)return C8.brandResultModel(state.records,state.query,{limit:preview?6:20,offset:preview?0:state.offset});
  const records=state.records.filter(f=>!state.categoryId||state.categoryId==='*'||au633BrandCategory(f)===state.categoryId),start=preview?0:state.offset,limit=preview?6:state.categoryId===null?0:20,items=records.slice(start,start+limit).map(food=>({id:C8.canonicalKey(food),recordId:food.id,kind:'exact-product',name:C8.consumerDisplayName(food),food,recordType:C8.recordType(food),provenance:C8.provenance(food),addability:C8.productEligibility(food).addability}));
  return {rawQuery:state.ownerQuery,identityQuery:state.query,intent:{kind:'brand-family',entity:state.entity},brand:state.wave,groups:items.length?[{key:'brand-products',label:state.wave.name+' products',items}]:[],total:records.length,loaded:records.length,offset:start,hasMore:start+limit<records.length};
}
function au633BrandCategory(food){return food.categoryId||window.HECBrandCatalogue.index.categories.find(c=>C8.norm(c.label)===C8.norm(food.category))?.id||'existing';}
function au633ConceptMember(food,conceptId){if(!conceptId)return true;const entry=window.HECBrandCatalogue?.entryFor(food);return entry?entry.conceptIds.includes(conceptId):S23.conceptCompatibility(food,conceptId).compatible;}
function au633RenderBrand(state){
  if(!au633BrandQueryCurrent(state))return false;
  const box=by('food-live-results'),target=by('food-results'),committed=searchSession633.mode==='explicit-committed';
  if(committed){
    if(state.loading){target.innerHTML='<section class="universal-search-empty" role="status">Finding the brand’s Australian product range…</section>';return true;}
    if(state.wave){
      const model=au633BrandModel(state),categories=window.HECBrandCatalogue.index.categories.map(c=>({...c,count:state.records.filter(f=>au633BrandCategory(f)===c.id).length})).filter(c=>c.count),existing=state.records.filter(f=>au633BrandCategory(f)==='existing').length;
      if(existing)categories.push({id:'existing',label:'Other accepted products',count:existing});
      searchSession633.submittedModel=model;
      const content=state.categoryId===null?`<div class="guided-resolution-choices">${[...categories,{id:'*',label:'All Items',count:state.records.length}].map(c=>`<button type="button" class="guided-resolution-choice" data-au-brand-category="${esc(c.id)}"><span>${esc(c.label)} (${c.count})</span><b>›</b></button>`).join('')}</div>`:`${model.groups.flatMap(g=>g.items).map(us633ResultRow).join('')}${state.offset?'<button type="button" class="secondary" data-au-brand-start>First Products</button>':''}${model.hasMore?'<button type="button" class="secondary wide" data-au-brand-more>More Products</button>':''}`;
      target.className='resource-list universal-search-owned';target.innerHTML=`<section class="guided-resolution-card brand-catalogue"><header><strong>${esc(state.wave.name)}</strong></header><p>${esc(window.HECBrandCatalogue.index.notice)}</p>${state.error?'<p role="status">Some products could not be loaded. Search again to retry.</p>':''}${content}<div class="guided-resolution-nav"><button type="button" class="secondary" data-au-brand-back>← Back</button></div></section>`;target.classList.remove('result-surface-inactive');target.removeAttribute('aria-hidden');box.innerHTML='';box.classList.add('hidden','result-surface-inactive');box.setAttribute('aria-hidden','true');return true;
    }
    const model=au633BrandModel(state);searchSession633.submittedModel=model;us633RenderSubmitted();
    target.insertAdjacentHTML('beforeend',`<p class="fine">${model.loaded} catalogue identities loaded · choose a product to continue.${state.error?' Some catalogue pages could not be loaded. Try Search again.':''}</p>${model.hasMore||state.catalogueLoaded<state.catalogueTotal?'<button type="button" class="secondary wide" data-au-brand-more>More Products</button>':''}${state.offset?'<button type="button" class="secondary" data-au-brand-start>First Products</button>':''}`);
  }else if(document.activeElement===by('food-search')){
    const model=state.loading?null:au633BrandModel(state,{preview:true});state.previewModel=model;
    box.innerHTML=`<section class="guided-resolution-card ps33-family-preview"><strong>Brand: ${esc(C8.queryIntent(state.query).entity.name)}</strong><small>${state.loading?'Finding the Australian product range…':'Choose a product, or Search to browse the range.'}</small>${model?.groups.flatMap(group=>group.items).map(item=>us633ResultRow(item)).join('')||''}</section>`;box.classList.remove('hidden','result-surface-inactive');box.removeAttribute('aria-hidden');
    if(target){target.classList.add('result-surface-inactive');target.setAttribute('aria-hidden','true');}
  }
  return true;
}
function au633LoadBrand(query,{conceptId='',returnToConcept=false,returnToSource=false}={}){
  if(au633BrandState&&au633BrandState.query===query&&au633BrandState.conceptId===conceptId&&au633BrandQueryCurrent(au633BrandState)){au633RenderBrand(au633BrandState);return au633BrandState;}
  const wave=window.HECBrandCatalogue?.brands.get(C8.brandKey(query)),entity=wave?{...wave,type:'brand',id:wave.key}:C8.queryIntent(query).entity;
  const state={query,ownerQuery:searchSession633.rawQuery,revision:searchSession633.revision,records:[],loading:true,offset:0,wave,entity,conceptId,returnToConcept,returnToSource,categoryId:conceptId?'*':null};au633BrandState=state;au633RenderBrand(state);
  const existing=()=>allFoods().filter(food=>wave?window.HECBrandCatalogue.isBrandMember(food,wave.key)&&C8.recordType(food)!=='external-catalogue'&&C8.productEligibility(food).addability.normalLoggingAllowed:C8.consumerBrandMembership(entity,food).matches);
  state.promise=(async()=>{
    const isCurrent=()=>au633BrandQueryCurrent(state);
    try{const result=wave?await window.HECBrandCatalogue.search(query,{brandKey:wave.key,limit:500,isCurrent}):await catalogueSearch633(query,{limit:500,isCurrent});if(!isCurrent())return;state.records=C8.sortedAustralianProducts(C8.canonicaliseRecords([...result.foods,...existing()]).filter(food=>(!wave||C8.productEligibility(food).addability.normalLoggingAllowed)&&au633ConceptMember(food,conceptId)));rememberCanonicalFoods(state.records);state.catalogueTotal=result.total;state.catalogueLoaded=result.foods.length;}
    catch(error){if(isCurrent()){state.error=String(error?.message||error);state.records=wave?[]:existing().filter(food=>au633ConceptMember(food,conceptId));}}
    finally{if(isCurrent()&&(wave||await C8.prepareBrandResultModel(state.records,query,{isCurrent}))){state.loading=false;au633RenderBrand(state);}}
  })();return state;
}
function au633SubmitBrand(raw,reason){
  if(String(by('food-search')?.value||'')!==searchSession633.rawQuery)ss633BeginTyping(by('food-search'));
  fc633Session=null;fc633Revision++;ps34GuidedSession=null;ps34GuidedFamily=null;ps34GuidedQuery='';ps34SyncGuidedUiState();searchSession633.sourceContext=null;searchSession633.selectedResult=null;searchSession633.quantity=null;
  ss633Commit(reason,C8.queryIntent(raw));ext.ui.foodSearch=raw;saveExt();by('food-search')?.blur();au633LoadBrand(raw);return true;
}
const au633LiveBase=s23RenderLive;s23RenderLive=function(raw){const query=String(raw||'').trim();if(C8.queryIntent(query).kind==='brand-family'&&!ps34GuidedSession){au633LoadBrand(query);return;}return au633LiveBase(raw);};renderFoodLiveMatches=s23RenderLive;
const au633LibraryBase=renderLibrary;renderLibrary=function(){if(au633BrandState?.returnToSource&&au633BrandQueryCurrent(au633BrandState)&&!ps34GuidedSession)return au633RenderBrand(au633BrandState);const input=by('food-search');if(input&&String(input.value||'')!==searchSession633.rawQuery)ss633BeginTyping(input);const raw=input?.value.trim()||'';if(C8.queryIntent(raw).kind==='brand-family'&&!ps34GuidedSession&&!['recent','saved','recipes','meals','combined'].includes(activeLibraryTab())){au633LoadBrand(raw);return;}return au633LibraryBase();};
const au633LargeRenderBase=psLargeRender;psLargeRender=function(result,options){if(C8.queryIntent(result.query).kind==='brand-family'){if(au633BrandState)au633RenderBrand(au633BrandState);return;}return au633LargeRenderBase(result,options);};
document.addEventListener('click',async event=>{const more=event.target.closest('[data-au-brand-more],[data-au-brand-start]'),state=au633BrandState;if(!more||!state||state.loading||!au633BrandQueryCurrent(state))return;event.preventDefault();state.offset=more.hasAttribute('data-au-brand-start')?0:state.offset+20;if(state.offset+20>state.records.length&&state.catalogueLoaded<state.catalogueTotal){state.loading=true;au633RenderBrand(state);try{const result=await catalogueSearch633(state.query,{limit:500,offset:state.catalogueLoaded,isCurrent:()=>au633BrandQueryCurrent(state)});if(!au633BrandQueryCurrent(state))return;state.records=C8.canonicaliseRecords([...state.records,...result.foods]);state.catalogueLoaded+=result.foods.length;state.catalogueTotal=result.total;}catch(error){if(au633BrandQueryCurrent(state)){state.error=String(error.message||error);state.offset=Math.max(0,state.offset-20);}}finally{if(au633BrandQueryCurrent(state))state.loading=false;}}au633RenderBrand(state);},true);
document.addEventListener('click',event=>{const control=event.target.closest('[data-au-brand-category],[data-au-brand-back]'),state=au633BrandState;if(!control||!state?.wave||!au633BrandQueryCurrent(state))return;event.preventDefault();if(control.hasAttribute('data-au-brand-category')){state.categoryId=control.dataset.auBrandCategory;state.offset=0;au633RenderBrand(state);}else if(state.categoryId!==null&&!state.returnToConcept&&!state.returnToSource){state.categoryId=null;state.offset=0;au633RenderBrand(state);}else if(state.returnToSource){au633BrandState=null;us633OpenSource('packaged-frozen');}else if(state.returnToConcept){au633BrandState=null;searchSession633.submittedModel=null;fc633Session.renderSignature='';fc633Render();}else{au633BrandState=null;const input=by('food-search');input.value='';ss633BeginTyping(input);ext.ui.foodSearch='';saveExt();renderLibrary();}},true);
window.HEC_AU_CATALOGUE_TEST={status:()=>au633BrandState?{query:au633BrandState.query,loading:au633BrandState.loading}:null,brandState:()=>au633BrandState?{query:au633BrandState.query,ownerQuery:au633BrandState.ownerQuery,revision:au633BrandState.revision,loading:au633BrandState.loading,offset:au633BrandState.offset,model:au633BrandState.loading?null:au633BrandModel(au633BrandState)}:null,foods:()=>allFoods(),eligibility:id=>C8.productEligibility(getFood(id))};
// Existing universal concepts (e.g. Soup and Coffee) reuse the same scoped brand view.
const brandSourceBase633=us633OpenSource;us633OpenSource=function(sourceKey){const result=brandSourceBase633(sourceKey),conceptId=searchSession633.sourceContext?.conceptKey,brands=sourceKey==='packaged-frozen'?window.HECBrandCatalogue?.conceptBrands(conceptId)||[]:[];if(brands.length){const target=by('food-results');target.querySelector('[data-brand-concept-choices]')?.remove();target.querySelector('.universal-source-results > p')?.insertAdjacentHTML('afterend','<section data-brand-concept-choices><h3>Brand Name</h3><div class="guided-resolution-choices">'+brands.map(b=>'<button type="button" class="guided-resolution-choice" data-au-concept-brand="'+esc(b.key)+'" data-au-concept="'+esc(conceptId)+'"><span>'+esc(b.name)+' ('+b.count+')</span><b>›</b></button>').join('')+'</div></section>');}return result;};
document.addEventListener('click',event=>{const control=event.target.closest('[data-au-concept-brand]');if(!control)return;const brand=window.HECBrandCatalogue.brands.get(control.dataset.auConceptBrand);if(brand){event.preventDefault();au633LoadBrand(brand.name,{conceptId:control.dataset.auConcept,returnToSource:true});}},true);
window.HEC_CANONICAL_CACHE_TEST={matches:cachedOnlineMatches,legacy:cachedLegacyMatches,render:renderOnlineLibrary,food:getFood};

// Retailer browse is a query-owned view of explicit listing evidence. It never
// uses the OFF brand directory or writes a retailer name into a consumer brand.
function identityConflictDetails633(food){
  return (food.evidenceConflicts||[]).filter(item=>item.code==='same-gtin-identity-conflict'&&item.resolution==='unresolved').map(item=>`<section class="identity-source-conflict"><h3>Product identity conflict</h3><p>These records report the same barcode (${esc(food.barcode)}). Their product details disagree.</p>${[item.evidence.left,item.evidence.right].map(record=>{const pack=C8.metricEvidence(record.pack),url=record.source?.url;return `<article><strong>${esc(record.brand)} · ${esc(record.name)}</strong><p>${pack?`Pack: ${esc(pack.amount)} ${esc(pack.unit)} · `:''}Form: ${esc(record.physicalForm)}${Object.values(record.variantAttributes||{}).length?` · Variant: ${esc(Object.values(record.variantAttributes).join(', '))}`:''}</p><small>Source record: ${esc(record.source?.recordId||record.recordId)}</small>${/^https:\/\//.test(url||'')?`<p><a href="${esc(url)}" target="_blank" rel="noopener noreferrer">View source evidence</a></p>`:''}</article>`;}).join('')}</section>`).join('');
}
function retailerCurrent633(state){return retailerBrowse633===state&&state.active&&ss633Current(state.ownerRevision,state.ownerQuery);}
function retailerRender633(state){
  if(!retailerCurrent633(state)||ps34GuidedSession)return false;
  const target=by('food-results'),live=by('food-live-results');if(!target)return false;
  const R=window.HECRetailerCatalogue,model=state.result||R.directory(state.retailerId,state),retailer=model?.retailer||R.entity(state.retailerId),commercial=state.scope==='commercial-identity',collection=['source-declared-brand','source-declared-store','private-testing-evidence'].includes(retailer?.collectionMode);
  const items=rememberCanonicalFoods(model?.foods||[]).map(food=>({id:C8.canonicalKey(food),recordId:food.id,name:food.name,food,addability:C8.productEligibility(food).addability}));
  searchSession633.submittedModel={rawQuery:state.ownerQuery,identityQuery:state.ownerQuery,intent:{kind:commercial?'commercial-identity':'retailer',entity:retailer},retailerId:state.retailerId,groups:items.length?[{key:'retailer-products',label:model.categoryLabel||'Products',items}]:[],total:model?.total||0};
  searchSession633.sourceContext={kind:'retailer',retailerId:state.retailerId,scope:state.scope,categoryId:state.categoryId};
  let content='';
  ...
  else content=`<h3>${esc(commercial?'Supported products':model.brandName||model.categoryLabel)}</h3>${items.map(us633ResultRow).join('')}${model.offset?'<button type="button" class="secondary" data-retailer-previous>Previous Products</button>':''}${model.hasMore?'<button type="button" class="secondary wide" data-retailer-more>More Products</button>':''}`;
  target.className='resource-list universal-search-owned';target.innerHTML=`<section class="guided-resolution-card retailer-catalogue" data-retailer-id="${esc(state.retailerId)}" data-retailer-scope="${esc(state.scope)}"><header><strong>${esc(retailer?.name||'Retailer')} · ${collection?'Private-testing collection':commercial?'Brand identity':'Retailer food catalogue'}</strong></header><p>${collection?esc(retailer.collectionNotice):commercial?'Verified house/private-label foods for your selected food.':'Products retain their own brands. Listing evidence does not guarantee stock at every store.'}</p>${content}<div class="guided-resolution-nav"><button type="button" class="secondary" data-retailer-back>← Back</button><button type="button" class="secondary" data-retailer-exit>Cancel</button></div></section>`;
  target.classList.remove('result-surface-inactive');target.removeAttribute('aria-hidden');if(live){live.innerHTML='';live.classList.add('hidden','result-surface-inactive');live.setAttribute('aria-hidden','true');}return true;
}
async function retailerLoad633(state,options={}){const work=window.HECRetailerCatalogue.load(state,{...options,isCurrent:()=>retailerCurrent633(state)});retailerRender633(state);await work;if(retailerCurrent633(state))retailerRender633(state);}
function retailerOpen633(retailerId,{scope='retailer',conceptId=''}={}){
  const R=window.HECRetailerCatalogue;R.cancel(retailerBrowse633);retailerBrowse633=R.createSession(retailerId,{ownerQuery:searchSession633.rawQuery,ownerRevision:searchSession633.revision,scope,conceptId});
  au633BrandState=null;psLargeSearchToken++;psLargeState=null;searchSession633.selectedResult=null;searchSession633.quantity=null;
  void retailerLoad633(retailerBrowse633);return true;
}
function retailerSubmit633(raw,reason){
  const intent=C8.queryIntent(raw);if(intent.kind!=='retailer')return false;
  fc633Session=null;fc633Revision++;ps34GuidedSession=null;ps34GuidedFamily=null;ps34GuidedQuery='';ps34SyncGuidedUiState();ss633Commit(reason,intent);by('food-search')?.blur();ext.ui.foodSearch=raw;saveExt();return retailerOpen633(intent.entity.id);
}
function retailerExit633({back=false}={}){
  const state=retailerBrowse633,restore=back&&state?.scope==='commercial-identity'&&fc633Session&&ss633Current(fc633Session.ownerRevision,fc633Session.ownerQuery);
  window.HECRetailerCatalogue.cancel(state);retailerBrowse633=null;searchSession633.sourceContext=null;searchSession633.submittedModel=null;searchSession633.selectedResult=null;
  if(restore){fc633Session.renderSignature='';return fc633Render();}
  const input=by('food-search');if(input){input.value='';ss633BeginTyping(input);}ext.ui.foodSearch='';saveExt();renderLibrary();return true;
}
const retailerLiveBase633=s23RenderLive;s23RenderLive=function(raw){const intent=C8.queryIntent(raw);if(intent.kind!=='retailer'||ps34GuidedSession)return retailerLiveBase633(raw);const box=by('food-live-results'),collection=['source-declared-brand','source-declared-store','private-testing-evidence'].includes(window.HECRetailerCatalogue?.entity(intent.entity.id)?.collectionMode);if(!box)return;box.innerHTML=`<section class="guided-resolution-card"><strong>${esc(intent.entity.name)} · ${collection?'Private-testing collection':'Retailer food catalogue'}</strong><small>${collection?'Reviewed records from Open Food Facts. Current availability is unknown.':'Search to browse foods with verified retailer listing evidence.'}</small></section>`;box.classList.remove('hidden','result-surface-inactive');box.removeAttribute('aria-hidden');};renderFoodLiveMatches=s23RenderLive;
const retailerLibraryBase633=renderLibrary;renderLibrary=function(){const input=by('food-search');if(input&&String(input.value)!==searchSession633.rawQuery)ss633BeginTyping(input);if(retailerBrowse633&&retailerCurrent633(retailerBrowse633)&&!ps34GuidedSession)return retailerRender633(retailerBrowse633);if(C8.queryIntent(input?.value||'').kind==='retailer'&&searchSession633.mode!=='explicit-committed'&&!ps34GuidedSession){s23RenderLive(input.value);return;}return retailerLibraryBase633();};
const retailerLargeSearchBase633=psLargeSearch;psLargeSearch=function(raw,options){if(C8.queryIntent(raw).kind==='retailer'||retailerBrowse633&&retailerCurrent633(retailerBrowse633))return Promise.resolve(null);return retailerLargeSearchBase633(raw,options);};
document.addEventListener('click',event=>{
  const control=event.target.closest('[data-retailer-brands],[data-retailer-brand],[data-retailer-category],[data-retailer-more],[data-retailer-previous],[data-retailer-back],[data-retailer-exit],[data-retailer-retry]'),state=retailerBrowse633;
  if(!control||!state||!control.isConnected||!retailerCurrent633(state))return;event.preventDefault();
  if(control.hasAttribute('data-retailer-exit'))return retailerExit633();
  if(control.hasAttribute('data-retailer-back')){if(state.scope==='commercial-identity')return retailerExit633({back:true});if(state.categoryId!==null)return void retailerLoad633(state,{categoryId:null,brandKey:''});if(state.view==='brands'){state.view='categories';return retailerRender633(state);}return retailerExit633({back:true});}
  if(state.loading)return;
  if(control.hasAttribute('data-retailer-brands')){state.view='brands';return retailerRender633(state);}
  if(control.hasAttribute('data-retailer-brand'))return void retailerLoad633(state,{categoryId:'*',brandKey:control.dataset.retailerBrand});
  if(control.hasAttribute('data-retailer-category')){state.view='categories';return void retailerLoad633(state,{categoryId:control.dataset.retailerCategory,brandKey:''});}
  const offset=control.hasAttribute('data-retailer-more')?state.offset+window.HECRetailerCatalogue.pageSize:control.hasAttribute('data-retailer-previous')?Math.max(0,state.offset-window.HECRetailerCatalogue.pageSize):state.offset;
  void retailerLoad633(state,{offset});
},true);
window.HEC_RETAILER_TEST={state:()=>retailerBrowse633?{retailerId:retailerBrowse633.retailerId,scope:retailerBrowse633.scope,categoryId:retailerBrowse633.categoryId,brandKey:retailerBrowse633.brandKey,view:retailerBrowse633.view,ownerQuery:retailerBrowse633.ownerQuery,ownerRevision:retailerBrowse633.ownerRevision,loading:retailerBrowse633.loading,error:retailerBrowse633.error,result:retailerBrowse633.result}:null};
...
window.HEC_SEARCH_SESSION_TEST={state:()=>({rawQuery:searchSession633.rawQuery,normalisedQuery:searchSession633.normalisedQuery,focused:searchSession633.focused,mode:searchSession633.mode,commitReason:searchSession633.commitReason,intentKind:searchSession633.intentKind,revision:searchSession633.revision,caretStart:searchSession633.caretStart,caretEnd:searchSession633.caretEnd,inputStable:searchSession633.inputNode===by('food-search'),sourceCommitted:!!ext.ui.foodSourceBrowse,guidedCommitted:!!ps34GuidedSession,universalStage:searchSession633.sourceContext?.stage||'',universalSource:searchSession633.sourceContext?.sourceKey||''}),intent:ss633Intent,begin:input=>ss633BeginTyping(input),commit:ss633Commit,current:ss633Current};
window.HEC_GUIDED_PRODUCT_TEST={food:()=>clone(ps34GuidedSession?.exactProduct||null),profile:()=>clone(ps34GuidedSession?.servingProfile||null),start:(records,query,options)=>GUIDED_PRODUCTS?.createSession?.(records,query,options),summary:session=>GUIDED_PRODUCTS?.summary?.(session),separation:session=>GUIDED_PRODUCTS?.separationAudit?.(session),selectChoice:ps34SelectGuidedProductChoice,ui:()=>({query:ps34GuidedQuery,stage:ps34GuidedSession?.stage||'',state:ps34GuidedSession?.resolutionState||'',product:ps34GuidedSession?.exactProduct?.name||'',selectedMeasure:ps34GuidedSession?.selectedMeasure?.key||'',measureAutoSelected:!!ps34GuidedSession?.measureAutoSelected,destination:clone(ps34GuidedSession?.destination||null)})};
window.HEC_LARGE_CATALOGUE_TEST={search:(query,options)=>window.HECOpenFoodFactsAU?.search?.(query,options),voiceSearch:(query,options)=>window.HECOpenFoodFactsAU?.search?.(query,options),barcode:value=>catalogueBarcode633(value),progression:(previous,next)=>window.HECOpenFoodFactsAU?.queryProgression?.(previous,next),state:()=>window.HECOpenFoodFactsAU?.cacheState?.(),federatedState:()=>({revision:psFederatedSearchState.revision,query:psFederatedSearchState.query,local:[...(psFederatedSearchState.local||[])],online:[...(psFederatedSearchState.online||[])]})};
...
// Product capture owns one in-memory draft. Navigation offers resume/new;
// completed drafts and asynchronous work can never reopen an old product.
...
function beginProductPanel(food=null,{force=false,savedReview=false}={}){
  const barcode=validBarcodeValue(food?.barcode||productCapture?.barcode||by('scan-barcode-input').value);
  ...
  alpha08UpdateOcrReview();ext.ui.scanMode='label';saveExt();updateScanModeUI();
}
...
updateScanModeUI=function(){
  captureModeUI();by('capture-controls').classList.toggle('hidden',ext.ui.scanMode==='food');
  const paused=!!productCapture?.suspended;by('capture-resume').classList.toggle('hidden',!paused);qa('[data-scan-mode]').forEach(button=>button.disabled=paused);
  if(paused){by('capture-controls').classList.add('hidden');['barcode-tools','label-tools','scan-photo-capture','scan-review-card','photo-tools'].forEach(id=>by(id)?.classList.add('hidden'));}
  else if(productCapture?.panelStarted)by('scan-review-card').classList.add('hidden');
  by('take-scan-photo').textContent=productCapture?.savedReview?'Photograph Current Nutrition Panel':'📷 Take Photo';
  by('capture-new-active').textContent=productCapture?.savedReview?'Start New Product Capture':'Start New Capture';
  if(productCapture?.savedReview)by('scan-mode-copy').textContent='Review / Update Nutrition — '+by('ocr-food-name').value+'. Your saved nutrition is the baseline. Photograph Current Nutrition Panel below, or check and edit the saved values.';
};
...
by('capture-cancel').addEventListener('click',()=>{const savedReview=productCapture?.savedReview;resetProductCapture();if(savedReview){ext.ui.libraryTab='saved';ext.ui.foodSearch='';}saveExt();openFeature(savedReview?'food-library':'home');});
