...
function retailerRender633(state){
  ...
  const R=window.HECRetailerCatalogue,model=state.result||R.directory(state.retailerId,state),retailer=model?.retailer||R.entity(state.retailerId),commercial=state.scope==='commercial-identity';
  ...
  let content='';
  if(state.loading)content='<p role="status">Loading verified foods…</p>';
  ...
  target.className='resource-list universal-search-owned';target.innerHTML=`<section class="guided-resolution-card retailer-catalogue" data-retailer-id="${esc(state.retailerId)}" data-retailer-scope="${esc(state.scope)}"><header><strong>${esc(retailer?.name||'Retailer')} · ${commercial?'Brand identity':'Retailer food catalogue'}</strong></header><p>${commercial?'Verified house/private-label foods for your selected food.':'Products retain their own brands. Listing evidence does not guarantee stock at every store.'}</p>${content}<div class="guided-resolution-nav"><button type="button" class="secondary" data-retailer-back>← Back</button><button type="button" class="secondary" data-retailer-exit>Cancel</button></div></section>`;
