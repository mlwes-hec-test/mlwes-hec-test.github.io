...
async function psLargeSearch(raw,{offset=0,append=false}={}){const api=window.HECOpenFoodFactsAU,query=String(raw||'').trim();if(!api||query.length<2)return null;const token=++psLargeSearchToken;try{const preview=await api.search(query,{offset,limit:24});if(token!==psLargeSearchToken||(by('food-search')?.value.trim()||'')!==query)return null;psLargeState=preview;if(offset===0&&['consumer-brand','brand-product'].includes(preview.intent?.kind)&&preview.total>0&&preview.total<=500){const complete=await api.search(query,{offset:0,limit:preview.total});if(token!==psLargeSearchToken)return null;ps34StartCatalogueGuide(query,complete.foods,complete.intent);return complete;}psLargeRender(preview,{append});return preview;}catch(error){console.warn('Australian product catalogue search unavailable',error);return null;}}
