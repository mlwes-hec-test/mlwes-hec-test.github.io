...
(function(global){
  ...
  const DIMENSIONS=Object.freeze(['category','family','productLine','variant','flavour','fatLevel','saltLevel','texture','grain','format','dietaryVariant','packSize','size']);
  ...
  function getNextProductDistinction(candidates,knownAttributes={}){
    ...
    for(const key of DIMENSIONS){if(knownAttributes[key])continue;const values=candidates.map(food=>productAttributes(food)[key]),present=values.filter(Boolean),unique=[...new Map(present.map(value=>[norm(value),value])).values()];if(unique.length>1&&unique.length<=12&&present.length/candidates.length>=.45){const options=unique.map(value=>optionFor(key,value,candidates));if(present.length<candidates.length)options.push(optionFor(key,'__other__',candidates));return {key,label:LABELS[key],question:`Which ${LABELS[key]}?`,options};}}
  ...
  function createSession(records,query,{knownAttributes={},consumption=null,destination=null,intent=null}={}){
    let parsed=consumption?{...parseConsumption(query),...consumption,identityQuery:consumption.identityQuery||parseConsumption(query).identityQuery}:parseConsumption(query),pool=candidatePool(records,parsed.identityQuery,intent);const packageIdentity=parsed.explicit&&parsed.measure&&pool.candidates.length===1&&norm(productAttributes(pool.candidates[0]).packSize)===norm(parsed.phrase);if(packageIdentity){parsed={...parsed,identityQuery:norm(query),amount:null,measure:'',explicit:false,phrase:''};pool=candidatePool(records,parsed.identityQuery,intent);}const initial=pool.candidates,known=knownAttributesFromQuery(initial,parsed.identityQuery,knownAttributes),session={version:VERSION,query:String(query||''),identityQuery:parsed.identityQuery,intent:pool.intent,initialCandidates:initial,candidates:initial,initialResolutionState:pool.state,resolutionState:pool.state,stage:STAGES.IDENTITY,knownAttributes:known,answers:[],nextQuestion:null,exactProduct:null,servingProfile:null,selectedMeasure:null,amount:null,nutrition:null,nutritionIntegrity:null,destination:clone(destination||null),consumption:parsed};
