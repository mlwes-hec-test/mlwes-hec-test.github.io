...
test('Flora ProActiv exposes only the manufacturer serve and grams with correct scaling',()=>{
  ...
  const applied=foundation.applyToFood(JSON.parse(JSON.stringify(food)));assert.deepEqual(Object.keys(applied.units).sort(),['g','serve']);assert.equal(applied.defaultUnit,'serve');assert.equal(applied.defaultAmount,1);assert.equal(applied.nutrients.calories*applied.units.serve,37);assert.equal(applied.nutrients.calories*10*applied.units.g,37);assert.doesNotMatch(JSON.stringify(applied),/vegetable serve|standard vegetable|teaspoon|"kg"/i);
});
