...
(function(global){
  ...
  function filterKnown(candidates,known={}){return candidates.filter(food=>{const attrs=productAttributes(food);return Object.entries(known).every(([key,value])=>!value||key==='product'?true:norm(attrs[key])===norm(value));});}
  ...
  function answerDistinction(session,key,value){if(!session||session.stage!==STAGES.IDENTITY)return session;const answer={key,value};session.answers.push(answer);if(key==='product'){session.candidates=session.candidates.filter(food=>canonicalProductKey(food)===value);session.knownAttributes.product=value;}else session.knownAttributes[key]=value;return workflowState(session);}
