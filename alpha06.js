...
function rc5SourceFamilyChoice(context,query){
  const family=s23Parsed(query).food;if(!family||(!context.source&&(C8?.friesIntent?.(query)?.generic||window.HECGuidedProductResolution?.genericSchemaForQuery?.(query))))return null;let source=context.source,choices=source?rc4SourceFoods(source.id).filter(food=>s23Parsed(String(food.choiceFamily||'').replace(/-/g,' ')).food===family):[];
  if(!source){const grouped=new Map();for(const food of allFoods().filter(item=>item.foodSourceId&&item.itemStatus!=='retired'&&s23Parsed(String(item.choiceFamily||'').replace(/-/g,' ')).food===family)){const group=grouped.get(food.foodSourceId)||[];group.push(food);grouped.set(food.foodSourceId,group);}const eligible=[...grouped.entries()].filter(([,foods])=>foods.length>1);if(eligible.length!==1)return null;const [sourceId,foods]=eligible[0],catalogue=window.HECFoodSources?.getCatalogue?.(sourceId);source=catalogue?.source||{id:sourceId,displayName:foods[0]?.sourceDisplayName||foods[0]?.brand||'Restaurant Source'};choices=foods;context={...context,source,product:family};}
...
function rc5SourceCountedExact(context,raw,query){
  ...
  const matches=rc4SourceFoods(context.source.id).filter(food=>food.semanticType==='counted-item'&&Number(food.productSemantics?.count||food.semanticCount)===count&&s23Parsed(String(food.choiceFamily||'').replace(/-/g,' ')).food===family);
  return matches.length===1?matches[0]:null;
}
