...
let ps34GuidedSession=null,ps34GuidedQuery='';
function ps34EnsureGuided(raw,family){if(!GUIDED_PRODUCTS)return null;const query=String(raw||'').trim();if(!ps34GuidedSession||ps34GuidedQuery!==query){ps34GuidedQuery=query;ps34GuidedSession=GUIDED_PRODUCTS.createSession(family.products,query,{destination:{date:ext.ui.diaryDate||isoToday(),meal:ext.ui.pendingMeal||''}});}return ps34GuidedSession;}
...
function ps34GuidedMarkup(raw,family,{compact=false}={}){
  ...
  const recognised=`<div class="guided-resolution-understanding"><strong>Recognised brand: ${esc(family.entity.name)}</strong><small>Product identity is resolved before serving or amount.</small></div>`;
  if(session.stage===GUIDED_PRODUCTS.stages.IDENTITY){const question=session.nextQuestion,choices=(question?.options||[]).map(option=>`<button type="button" class="guided-resolution-choice" data-gpr-answer="${esc(question.key)}" data-gpr-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('');return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${recognised}<div class="guided-resolution-question"><strong>${esc(question?.question||'Which exact product?')}</strong><small>Only choices that narrow the loaded products are shown.</small></div><div class="guided-resolution-choices">${choices}</div>${ps34GuidedNav(session)}<div class="guided-resolution-fallback"><small>Can’t find it? Keep typing, scan its barcode, read the Nutrition Panel, or create it manually.</small></div></section>`;}
  const product=session.exactProduct,identity=`<div class="guided-resolution-exact"><small>Exact canonical product</small><strong>${esc(product?.name||'')}</strong><span>${esc(product?.brand||'')}</span></div>`;
  if(session.stage===GUIDED_PRODUCTS.stages.MEASURE){const choices=(session.servingProfile?.measures||[]).map(measure=>`<button type="button" class="guided-resolution-choice" data-gpr-measure="${esc(measure.key)}"><span>${esc(measure.label)}</span><b>›</b></button>`).join('');return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${identity}<div class="guided-resolution-question"><strong>Choose the serving measure</strong><small>No amount has been assumed from the nutrition reference serving or package size.</small></div><div class="guided-resolution-choices">${choices||'<p>Only a verified metric measure can be offered for this product.</p>'}</div>${ps34GuidedNav(session)}</section>`;}
  if(session.stage===GUIDED_PRODUCTS.stages.AMOUNT)return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${identity}<div class="guided-resolution-question"><strong>Amount</strong><small>${esc(session.selectedMeasure?.label||session.selectedMeasure?.key||'')}</small></div><label class="guided-resolution-amount">How many?<input type="number" inputmode="decimal" min="0.01" step="0.01" value="1" data-gpr-amount></label><button type="button" class="primary guided-resolution-continue" data-gpr-continue>Continue</button>${ps34GuidedNav(session)}</section>`;
...
function ps33RenderBrandFamilyLive(raw,family){const box=by('food-live-results');if(!box)return;box.innerHTML=ps34GuidedMarkup(raw,family,{compact:true});box.classList.toggle('hidden',!box.innerHTML);ps34SyncGuidedUiState();}
function ps33RenderBrandFamilySurface(raw,family){const results=by('food-results');if(!results)return;results.className='food-results brand-family-results';results.innerHTML=ps34GuidedMarkup(raw,family);ps34SyncGuidedUiState();}
function ps34SyncGuidedUiState(){document.body.classList.toggle('guided-product-unresolved',!!ps34GuidedSession&&ps34GuidedSession.stage===GUIDED_PRODUCTS?.stages?.IDENTITY&&!ps34GuidedSession.exactProduct);}
function ps34RenderGuided(){const raw=by('food-search')?.value.trim()||ps34GuidedQuery,family=ps33BrandFamily(raw);if(!family)return;ps33RenderBrandFamilyLive(raw,family);ps33RenderBrandFamilySurface(raw,family);ps34SyncGuidedUiState();rc6SyncFoodSearchViewport();}
...
document.addEventListener('click',event=>{
  const answer=event.target.closest('[data-gpr-answer]'),measure=event.target.closest('[data-gpr-measure]'),back=event.target.closest('[data-gpr-back]'),cancel=event.target.closest('[data-gpr-cancel]'),next=event.target.closest('[data-gpr-continue]'),review=event.target.closest('[data-gpr-review]');if(!answer&&!measure&&!back&&!cancel&&!next&&!review)return;
  event.preventDefault();event.stopPropagation();if(!ps34GuidedSession)return;
  if(answer){ps34SelectGuidedProductChoice(answer.dataset.gprValue,answer.dataset.gprAnswer);return;}else if(measure)GUIDED_PRODUCTS.selectMeasure(ps34GuidedSession,measure.dataset.gprMeasure);else if(back)ps34GuidedSession=GUIDED_PRODUCTS.back(ps34GuidedSession);else if(cancel){ps34GuidedSession=null;ps34GuidedQuery='';ps34SyncGuidedUiState();const input=by('food-search');if(input)input.value='';ext.ui.foodSearch='';saveExt();renderLibrary();return;}else if(next){const card=next.closest('.guided-resolution-card'),amount=Number(card?.querySelector('[data-gpr-amount]')?.value);GUIDED_PRODUCTS.selectAmount(ps34GuidedSession,amount);}else if(review){const selected=ps34GuidedSession.exactProduct,food=getFood(selected?.id)||selected;if(food)prepareEntry(food,{date:ps34GuidedSession.destination?.date||ext.ui.diaryDate||isoToday(),meal:ps34GuidedSession.destination?.meal||ext.ui.pendingMeal||'',amount:ps34GuidedSession.amount,unit:ps34GuidedSession.selectedMeasure?.key});return;}ps34RenderGuided();
},true);
const ps33ExactProductBase=rc4ExactProduct;rc4ExactProduct=function(raw){return ps33BrandFamily(raw)?null:ps33ExactProductBase(raw);};
const ps33RenderLiveBase=s23RenderLive;s23RenderLive=function(raw){const term=String(raw||'').trim(),family=ps33BrandFamily(term);if(ps34GuidedQuery&&ps34GuidedQuery!==term){ps34GuidedSession=null;ps34GuidedQuery='';ps34SyncGuidedUiState();}if(family&&document.activeElement===by('food-search')){ps33RenderBrandFamilyLive(term,family);ps34SyncGuidedUiState();return;}ps33RenderLiveBase(term);};renderFoodLiveMatches=s23RenderLive;
let psLargeSearchToken=0,psLargeState=null;
function psLargeRows(result){return result.foods.map(resourceFoodRow).join('');}
function psLargeRender(result,{append=false}={}){const target=by('food-results');if(!target)return;const section=`<section class="off-catalogue-results" data-off-results><div class="online-source-banner"><strong>Australian Product Catalogue</strong><p>${result.total.toLocaleString()} real product${result.total===1?'':'s'} found · Open Food Facts · package size is product identity, not amount consumed.</p></div><div data-off-rows>${psLargeRows(result)}</div>${result.hasMore?`<button type="button" class="secondary wide off-load-more" data-off-load-more="${result.offset+result.foods.length}">Load More Products</button>`:''}</section>`;const existing=target.querySelector('[data-off-results]');if(append&&existing){existing.querySelector('[data-off-rows]')?.insertAdjacentHTML('beforeend',psLargeRows(result));existing.querySelector('[data-off-load-more]')?.remove();if(result.hasMore)existing.insertAdjacentHTML('beforeend',`<button type="button" class="secondary wide off-load-more" data-off-load-more="${result.offset+result.foods.length}">Load More Products</button>`);}else{existing?.remove();target.insertAdjacentHTML('beforeend',section);}}
async function psLargeSearch(raw,{offset=0,append=false}={}){const api=window.HECOpenFoodFactsAU,query=String(raw||'').trim();if(!api||query.length<2)return null;const token=++psLargeSearchToken;try{const preview=await api.search(query,{offset,limit:24});if(token!==psLargeSearchToken||(by('food-search')?.value.trim()||'')!==query)return null;psLargeState=preview;if(preview.brand&&offset===0&&preview.total<=500){const complete=await api.search(query,{offset:0,limit:preview.total});if(token!==psLargeSearchToken)return null;const family=ps33BrandFamily(query);if(family&&complete.foods.length){ps34GuidedSession=null;ps34GuidedQuery='';const expanded=ps33BrandFamily(query);if(expanded){ps33RenderBrandFamilyLive(query,expanded);ps33RenderBrandFamilySurface(query,expanded);return complete;}}}psLargeRender(preview,{append});return preview;}catch(error){console.warn('Australian product catalogue search unavailable',error);return null;}}
function psLargeSchedule(raw){const query=String(raw||'').trim();if(query.length<2){psLargeSearchToken++;return;}setTimeout(()=>{if((by('food-search')?.value.trim()||'')===query)psLargeSearch(query);},40);}
...
const ps33RenderLibraryBase=renderLibrary;renderLibrary=function(){ps33RenderLibraryBase();const raw=by('food-search')?.value.trim()||'',family=ps33BrandFamily(raw);if(family)ps33RenderBrandFamilySurface(raw,family);psLargeSchedule(raw);rc6SyncFoodSearchViewport();};
...
window.HEC_LARGE_CATALOGUE_TEST={search:(query,options)=>window.HECOpenFoodFactsAU?.search?.(query,options),voiceSearch:(query,options)=>window.HECOpenFoodFactsAU?.search?.(query,options),barcode:value=>window.HECOpenFoodFactsAU?.lookupBarcode?.(value),state:()=>window.HECOpenFoodFactsAU?.cacheState?.()};
