...
function s23ProductMatches(raw,limit=16){
  ...
  const cacheKey=`${alpha0630FoodSearchRevision}|${String(raw||'')}`;let ranked=alpha0630ProductMatchCache.get(cacheKey);
...
by('food-search')?.addEventListener?.('keydown',event=>{if(event.key==='Enter'&&ss633CommitCurrent('enter'))event.preventDefault();});
...
function au633LoadBrand(query){
  ...
  state.promise=(async()=>{try{const result=await window.HECOpenFoodFactsAU.search(query,{limit:500});if(!au633BrandQueryCurrent(state))return;state.records=C8.canonicaliseRecords([...allFoods().filter(food=>C8.consumerBrandMembership(entity,food).matches),...result.foods]);state.catalogueTotal=result.total;state.catalogueLoaded=result.foods.length;}catch(error){if(au633BrandQueryCurrent(state)){state.error=String(error?.message||error);state.records=allFoods().filter(food=>C8.consumerBrandMembership(entity,food).matches);}}finally{if(au633BrandQueryCurrent(state)){state.loading=false;au633RenderBrand(state);}}})();return state;
}
