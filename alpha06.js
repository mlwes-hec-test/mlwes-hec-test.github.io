...
let ps34GuidedSession=null,ps34GuidedQuery='',ps34GuidedFamily=null;
...
function ps34GuidedMarkup(raw,family,{compact=false}={}){
  ...
  if(session.stage===GUIDED_PRODUCTS.stages.IDENTITY){const question=session.nextQuestion;if(session.resolutionState===GUIDED_PRODUCTS.states.INCOMPLETE||!question)return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${recognised}<div class="guided-resolution-question"><strong>Product identity needs more information</strong><small>A barcode or broad family label is not treated as the exact food you consumed.</small></div>${ps34GuidedNav(session)}<div class="guided-resolution-fallback"><small>Keep typing, scan for a better source name, read the Nutrition Panel, or complete the product name manually.</small></div></section>`;const choices=(question.options||[]).map(option=>`<button type="button" class="guided-resolution-choice" data-gpr-answer="${esc(question.key)}" data-gpr-value="${esc(option.value)}"><span>${esc(option.label)}</span><b>›</b></button>`).join('');return `<section class="guided-resolution-card ${compact?'guided-resolution-compact':''}">${recognised}<div class="guided-resolution-question"><strong>${esc(question?.question||'Which exact product?')}</strong><small>${question.key==='product'?'Choose the real product name. Optional narrowing is used only when it materially helps.':'This shopper-facing distinction materially reduces the remaining products.'}</small></div><div class="guided-resolution-choices">${choices}</div>${ps34GuidedNav(session)}<div class="guided-resolution-fallback"><small>Can’t find it? Keep typing, scan its barcode, read the Nutrition Panel, or create it manually.</small></div></section>`;}
...
function ps34RenderGuided(){const raw=by('food-search')?.value.trim()||ps34GuidedQuery,family=ps34GuidedFamily||ps33BrandFamily(raw);if(!family)return;ps33RenderBrandFamilyLive(raw,family);ps33RenderBrandFamilySurface(raw,family);ps34SyncGuidedUiState();rc6SyncFoodSearchViewport();}
...
const ps33RenderLiveBase=s23RenderLive;s23RenderLive=function(raw){const term=String(raw||'').trim(),family=ps33BrandFamily(term);if(ps34GuidedQuery&&ps34GuidedQuery!==term){ps34GuidedSession=null;ps34GuidedQuery='';ps34GuidedFamily=null;ps34SyncGuidedUiState();}if(family&&document.activeElement===by('food-search')){ps34GuidedFamily=family;ps33RenderBrandFamilyLive(term,family);ps34SyncGuidedUiState();return;}ps33RenderLiveBase(term);};renderFoodLiveMatches=s23RenderLive;
...
const ps33RenderLibraryBase=renderLibrary;renderLibrary=function(){ps33RenderLibraryBase();const raw=by('food-search')?.value.trim()||'',family=ps33BrandFamily(raw);if(ps34GuidedFamily&&ps34GuidedQuery===raw)ps33RenderBrandFamilySurface(raw,ps34GuidedFamily);else if(family){ps34GuidedFamily=family;ps33RenderBrandFamilySurface(raw,family);}else if(psLargeState?.query===raw)psLargeRender(psLargeState);psLargeSchedule(raw);rc6SyncFoodSearchViewport();};
